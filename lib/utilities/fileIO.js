...
const readParsedAsync = async (file) => {
    return parse(await promises.readFile(file))
}
...
const read = (file, relative = false) => {
...
const stringify = (data, oneline = false) => {
    return oneline ? fastStringify(data) : fastStringify(data, null, "\t");
};
