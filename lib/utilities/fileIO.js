...
const readParsed = async (file, relative = false) => {
    return parse(await read(relative ? await getAbsolutePathFrom(file) : file));
}
 ...
 */
const readParsedSync = (file, relative = false) => {
    return parse(readSync(relative ? getAbsolutePathFromSync(file) : file));
}
...
const fileExist = async (file, relative = false) => {
    const path = (relative)  ? await getAbsolutePathFrom(file) : file;
    try {
        await promises.access(path, fs.constants.R_OK);
        return true;
    } catch (e) {
        return false;
    }
...
*/
const getAbsolutePathFromSync = (path) => {
...
* Retrieve absolute path using shortened path.
* @param {string} path 
* @returns {<Promise>string} absolutePath
*/
const getAbsolutePathFrom = async (path) => {
    const startsWithSlash = path[0] == "/";
    return startsWithSlash
        ? `${process.cwd()}${path}`
        : `${process.cwd()}/${path}`;
...
const getDirectoriesFrom = async (path, relative = true) => {
    const path = (relative) ? await getAbsolutePathFrom(path) : path;
...
const getFilesFrom = async (path, relative = true) => {
    const tempPath = (relative) ? await getAbsolutePathFrom(path) : path;
