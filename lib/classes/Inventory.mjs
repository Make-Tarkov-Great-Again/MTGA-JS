...
export class Inventory {
    ...
    */
    ...
    static async addItemToInventory(character, container, itemData, amountToBeAdded = 1, children = false) {
        ...
        while (amountToBeAdded > 0) {
            ...
            await this.adjustItemForPurchase(currentIterItemsAdded, item, itemData, children); //done outside
    ...
    static async splitItem(characterInventory, characterChanges, moveAction) {
        ...
        characterInventory.items.push(await structuredClone(newItem));
    ...
    static removeInventoryItemByID(characterInventory, itemId) {
        const item = this.getInventoryItemByID(characterInventory, itemId);
    ...
    static async examineItem(character, moveAction) {
        ...
        if (moveAction.fromOwner) {
            switch (moveAction.fromOwner.type) {
                ...
                case "RagFair":
                    const offer = Ragfair.getOfferById(moveAction.fromOwner.id);
            ...
            const item = Inventory.getInventoryItemByID(character.Inventory, moveAction.item);
            if (item) {
    ...
    static async openRandomLootContainer(character, characterChanges, moveAction) {
        ...
        const stash = Inventory.getInventoryItemByID(character.Inventory, await Inventory.getStashContainer(characterInventory))
