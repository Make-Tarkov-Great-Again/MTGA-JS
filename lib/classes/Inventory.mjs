...
import { logger, stringify, cloneDeep, round, min } from "../utilities/_index.mjs";
...
export class Inventory {
    ...
    static async addItemToInventory(character, container, parentId, itemData, amountPurchased = 1, children = false) {
        const itemsAdded = [];

        while (amountPurchased > 0) {
            ...
            amountPurchased = await this.adjustStackSize(item, itemData, amountPurchased);
            ...
            await this.adjustItemForPurchase(itemsAdded, character.Inventory.items, item, itemData, preparedChildren); //done outside
        }
    ...
    static async createItemForPurchase(itemData, container) {
        const { trading: { tradePurchasedIsFoundInRaid } } = database.core.gameplay;
        ...
        item.upd.SpawnedInSession = (tradePurchasedIsFoundInRaid) ? true : false;
        return item;
    }
    ...
    static async adjustItemForPurchase(itemsAdded, inventory, item, itemData, children = false) {
        ...
        if (itemData._parent === ITEM_NODES.AMMO_CONTAINER) {
            ...
            itemsAdded.push(...ammoInAmmoBox);
            inventory.push(...ammoInAmmoBox);
        }
        ...
        if (children) {
            for (const child of children) {
                ...
                itemsAdded.push(...childrenAdded);
                inventory.push(...childrenAdded);
            }
        }
    ...
    static async moveItems(characterInventory, moveAction) {
        if (moveAction.fromOwner && moveAction.fromOwner.type === "Mail") {
            const redeemed = await this.retrieveRewardItems(moveAction);
            if (!redeemed)
                return;
            await this.moveItemIntoProfile(characterInventory, moveAction, redeemed);
            return;
        }
        await this.moveItem(characterInventory, moveAction);
    }
    ...
    static async examineItem(character, moveAction) {
        ...
        if (templateItem) {
            if (await Character.examineItem(character, templateItem._id)) {
                await Character.addExperience(character, templateItem._props.ExamineExperience);
                return logger.info(`[examineItem] Examined item ${templateItem._id} successfully!`)
            } else {
    ...
    static async openRandomLootContainer(character, characterChanges, moveAction) {
        const { core: { globals: { seasonalEvents: { loot } } } } = database;
