...
import { generateMongoID } from "../utilities/utility.mjs";
...
export class Location {
    ...
    static async generateLocationData(mapName) {
        ...
        await Promise.all([
            // loot generation
            this.generateStaticLoot(output),
            this.generateLooseLoot(output),
            this.generateQuestLoot(output),
            this.generateWeaponsLoot(output)
        ]);
        return output;
    }
    /**
    ...
    static async generateContainerContents(data) {
        ...
        const containerTemplate = Item.get(containerId);
        let SpawnList = {}
        ...
        for (let spawningTpl of containerTable.SpawnList) {
            SpawnList[spawningTpl] = Item.get(spawningTpl);
        }
        for (let i = 0; i < itemToSpawn; i++) {
            ...
            if (smallList.length > 0) {
                ...
                let item = await Item.createAsNewItem(itemToAdd);
