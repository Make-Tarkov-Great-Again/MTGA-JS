...
import { generateMongoID, getRandomInt, logger } from "../utilities/_index.mjs";
...
export class Location {
    ...
    static async _GetItemRarityType(itemTemplate) {
        const { BackgroundColor, LootExperience, ExamineExperience, Unlootable, Name } = itemTemplate._props;

        if (Unlootable) {
            return "NOT_EXIST";
        }

        if (LootExperience >= 45 || BackgroundColor === "violet" || ExamineExperience >= 20 || Name.toLowerCase().includes("key")) {
            return "SUPERRARE";
        }

        if (LootExperience >= 40 || ExamineExperience >= 15) {
            return "RARE";
        }

        if (LootExperience >= 20 || ExamineExperience >= 8) {
            return "UNCOMMON";
        }
    ...
    static async generateContainerContents(data) {
        ...
        for (let i = 0; i < itemToSpawn; i++) {
            ...
            if (smallList.length > 0) {
                ...
                const item = Item.createAsNewItemWithParent(itemToAdd, data.Items[0]._id);
    ...
    static async generateStaticLoot(location, output) {
        ...
        const containers = location.lootSpawns.containers;

        const spawnPromises = containers.map(container => {
            return this.gsl_loopWalker(container, output);
        });
    ...
    static async generateQuestLoot(location, output) {
        ...
        const quests = location.lootSpawns.quests;
        const spawnedPromises = quests.map(quest => this.ql_loopWalker(quest, output));
        await Promise.all(spawnedPromises);
    }

    static async ql_loopWalker(quest, output) {
        const newId = generateMongoID();
        const data = await this.BSG_DefaultLootStructure();

        const { worldId, questItmTpl, Position, Rotation } = quest;
        data.Id = worldId;
        ...
        data.Items.push({ _id: newId, _tpl: questItmTpl });
        data.Root = newId;
    ...
    static async generateWeaponsLoot(location, output) {
        ...
        const lootGenData = Location.get("lootGen");
        const weapons = location.lootSpawns.weapons;

        const spawnedPromises = weapons.map(weapon => this.gwl_loopWalker(weapon, output, lootGenData));
        await Promise.all(spawnedPromises);
        /*         const lootGenData = Location.get("lootGen");
    ...
    static async gll_loopWalker(spawnTable, lootSpawn, output) {
        ...
        if (typeof spawnTable[generateSimpleName] == "undefined") {
            logger.warn(`SpawnTable not found for ${generateSimpleName} with WorldId: ${lootSpawn.worldId}, skipping...`);
            logger.warn(`SNIPPET CODE FOR QUICK ADDING INTO THE availableSpawns.json for map ${output.Name}\n"${generateSimpleName}": [],`)
            return;
        }
    ...
    static async generateLooseLoot(location, output) {
        ...
        const spawnedPromises = looseLoot.map(lootSpawn => this.gll_loopWalker(location.dynamicAvailableSpawns, lootSpawn, output));
        await Promise.all(spawnedPromises);
        /*         const looseLoot = location.lootSpawns.dynamic;
