...
export class Hideout {
    ...
    static async takeItemFromAreaSlot(character, moveAction, characterChanges) {
        ...
        for (const slot of moveAction.slots) {
            ...
            const itemsAdded = await Inventory.addItemToInventory(character, stashContainer, itemData);
            if (!itemsAdded) {
                logger.error(`[Hideout.takeItemFromAreaSlot] Unable to find hideout area type ${moveAction.areaType}.`);
                return;
            }
    ...
    static async takeProduction(character, moveAction, characterChanges) {
        ...
        for (const product of production.Products) {
            ...
            if (itemPresets) {
                ...
                const itemChilds = await Item.prepareChildrenForAddItem(itemPreset._items[0]._id, itemPreset._items);
                const itemsAdded = await Inventory.addItemToInventory(character, stashContainer, item, product.count, itemChilds);
                if (!itemsAdded) {
                ...
                const itemsAdded = await Inventory.addItemToInventory(character, stashContainer, item, product.count);
                if (!itemsAdded) {
                    logger.error(`[Hideout.takeProduction] Unable to find hideout area type ${moveAction.areaType}.`);
                    return;
                }
