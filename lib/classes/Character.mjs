...
import {
    ...
    write, getFileUpdatedDate, getCurrentTimestamp, logger
...
export class Character {
    ...
    static async create(template, request) {
        ...
        template.aid = Response.getSessionID(request);
    ...
    */
     ...
     * @returns {Promise<object>}
     */
    static getChangesTemplate(character) {
    ...
    static async save(sessionID) {
        ...
        if (!await fileExist(characterPath)) {
            await write(characterPath, currentCharacter);
        ...
        if (currentCharacter !== savedCharacter) {
            ...
            logger.debug(`[CHARACTER SAVE] Character file for profile ${sessionID} save skipped!`);
    }
    ...
    static getLoyalty(character, traderID) {
    ...
    * @returns
    */
    static updateBackendCounters(character, conditionId, qid, counter) {
    ...
    static getQuest(character, questId) {
        return character.Quests.find(quest => quest.qid === questId);
    }

    static updateQuest(character, id, status) {
    ...
    static getPlayerSkill(character, skillId) {
    ...
    static getPlayerSkillLevel(character, skillId) {
        const playerSkill = this.getPlayerSkill(character, skillId);
        let level = 0;

        if (playerSkill.Progress < 550) {
            level = Math.floor(1 / 2 * (-1 + Math.sqrt(0.8 * playerSkill.Progress + 1)));
        } else {
            level = 10 + Math.floor((playerSkill.Progress - 550) / 100);
            if (level > 51) {
                level = 51;
            }
        }
    ...
    static filterWishList(character, wishToRemove) {
        character.WishList = character.WishList.filter(wish => wish !== wishToRemove);
        logger.info(`${wishToRemove} removed from Wish List`);
    }

    static addHealthToBodyPart(character, bodyPart, health) {
        character.Health.BodyParts[bodyPart].Health.Current = Math.round(character.Health.BodyParts[bodyPart].Health.Current + health);
 ...
 */
    ...
    static setExperience(character, experiencePoints) {
        character.Info.Experience = experiencePoints;
    }
    ...
    static setHealth(character, healthData) {
        ...
        for (const bodyPart in character.Health.BodyParts) {
            character.Health.BodyParts[bodyPart].Health.Current = healthData.Health[bodyPart].Current <= 0
                ? 0
                : Math.round(healthData.Health[bodyPart].Current);
