    ...
    */
    static async sendNotification(sessionID, notification) {
        ...
        if ((webSocket[sessionID] !== undefined && webSocket[sessionID].socket.readyState === 1)) {
            ...
            const check = await this.getMailFromMailbox(sessionID, notification)
            if (check) {
        ...
        if (notifications) {
            ...
            for (const notification of notifications) {
                if (currentTime >= notification.message.dt) {
                    const check = await this.sendNotification(sessionID, notification);
                    if (check) await this.removeNotificationFromQueue(sessionID);
                }
            }
        }
    }
    ...
    static async processInsuranceReturn(sessionID) {
        const check = this.checkInsuranceNotifications()
        if (check) {
