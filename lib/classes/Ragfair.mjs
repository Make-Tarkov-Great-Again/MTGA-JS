...
export class Ragfair {
    ...
    static async generateOffersBasedOnRequest(request, playerProfile) {
        ...
        if (request.body.neededSearchId) {
            ...
            output.categories = await Ragfair.generateAmountOffersPerCategories(output.offers, Object.keys(database.flea.categories)); // we want all categories to show
    ...
    static async loadTradersOffers(playerProfile) {
        ...
        for (const traderId in traders) {
            ...
            if (!await Trader.isFence(trader) && !await Trader.isRagfair(trader)) {
          ...
          //  const activity = DiscordRPC.GenerateActivityTemplate();
           // activity.state = "Browsing the Flea-Market";
           // activity.details = `Shit. What was the name of that flashhider..`
           // activity.startTimestamp = time.now()
           // activity.largeImageKey = "logo"
          //  activity.largeImageText = "Ragfair"
