...
import {
    getCurrentTimestamp, generateMongoID, cloneDeep
...
export class Ragfair {
    ...
    static async generateOffersBasedOnRequest(request, playerProfile) {
        ...
        const output = { offers: await structuredClone(database.flea.offers) };
        ...
        for (const traderId in traders) {
            ...
            if (!Trader.isFence(trader) && !Trader.isRagfair(trader)) {
                ...
                if (trader.nextResupply < timestamp || !database.flea.offersCount) {
                    database.flea.offers = database.flea.offers.concat(Trader.getRagfairOffers(trader));
    ...
    static async getTplFromHandbook(handbookId) {
        ...
        const children = await Templates.childrenCategories(handbookId);

        if (bastards.includes(handbookId)) {
            for (let c = 0, cLength = children.length; c < cLength; c++) {
                const child = children[c];
                const grandchildren = await Templates.childrenCategories(child);
                for (let gc = 0, gcLength = grandchildren.length; gc < gcLength; gc++) {
                    const grandchild = grandchildren[gc];
                    const templates = await Templates.templatesWithParent(grandchild);
        ...
        if (!await Templates.isCategory(handbookId)) { // its a specific item searched then
            return handbookId;
        }

        // list all item of the category
        result = result.concat(await Templates.templatesWithParent(handbookId));
        for (let c = 0, cLength = children.length; c < cLength; c++) {
            const child = children[c];
            const templates = await Templates.templatesWithParent(child);
            result = result.concat(templates);
        }
        return result;
    }
    ...
    static async sortOffers(request, offers) {
        ...
        switch (request.body.sortType) {
            ...
            case 4: // Offer (title)
                const { lang } = await Account.get(Response.getSessionID(request));
