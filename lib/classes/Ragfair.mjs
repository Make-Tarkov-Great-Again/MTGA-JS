...
import { getCurrentTimestamp, generateMongoID } from "../utilities/_index.mjs";
...
export class Ragfair {
    ...
    static async generateOffersBasedOnRequest(request, playerProfile) {
        ...
        const output = { offers: { ...database.flea.offers } };
    ...
    static async getTplFromHandbook(handbookId) {
        ...
        if (!Templates.isCategory(handbookId)) { // its a specific item searched then
            return handbookId;
        }

        // list all item of the category
        result = result.concat(Templates.templatesWithParent(handbookId));
        for (let c = 0, cLength = children.length; c < cLength; c++) {
            const child = children[c];
            const templates = Templates.templatesWithParent(child);
            result = result.concat(templates);
        }
        return result;
    }
