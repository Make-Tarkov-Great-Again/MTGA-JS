...
export class Quest {
    ...
    static async getQuestsForPlayer(sessionID) {
        ...
        for (const questId in quests) {
            ...
            if (await this.questForOtherFaction(character.Info.Side, questId))
                continue;
            ...
            if (await this.checkQuestsRequirements(character, questRequirements, loyaltyRequirements)) {
     ...
     */
    static async checkQuestsRequirements(character, questRequirements, loyaltyRequirements) {
        ...
        for (const condition of loyaltyRequirements) {
            if (!await this.checkLoyaltyReq(character, condition))
                return false;
        }
        return true;
    }
    ...
    */
    ...
    static async failQuest(characterChanges, character, quest, failure) {
        ...
        const questReward = await this.processQuestRewards(characterChanges, quest, character, "Fail");
