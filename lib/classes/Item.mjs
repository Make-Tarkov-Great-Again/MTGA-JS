...
export class Item {
    ...
    static createWeightedList(items) {
        const weights = this.generateItemWeights(items);
        return this.generateWeightedList(weights);
    }

    static generateItemWeights(items) {
        ...
        if (!Array.isArray(items)) {
            ...
            for (const c in items) {
                ...
                output[c] = this.generateItemsWeight(category);
            }
            return output;
        }
    ...
    static generateItemsWeight(items) {
    ...
    */
    static generateWeightedList(weights) {
        const object = {};
        const array = [];
    ...
    static createAsNewItemWithParent(itemID, parentId) {
        const id = generateMongoID();
        return {
            _id: id,
            _tpl: itemID,
    ...
    static getParentAndChildren(itemId, items) {
        const children = [];
        for (let i = 0, length = items.length; i < length; i++) {
            ...
            if (child.parentId === itemId
                ...
                children.push(...this.getParentAndChildren(child._id, items));
        }
        return children;
    }
    ...
    */
    static findAndReturnChildrenAsIds(itemId, items) {
        const output = [];
        ...
        for (let i = 0, length = items.length; i < length; i++) {
            ...
            if (child.parentId === itemId) {
                output.push(...this.findAndReturnChildrenAsIds(child._id, items));
            }
        }
    ...
    */
    static handleAmmoBoxes(item, parentId) {
        const output = [];
        ...
        while (remainder > 0) {
            const cartridges = this.createAsNewItemWithParent(ammoId, parentId);
    ...
    */
    static getSizesOfChildItems(children, size, forced) {
    ...
    */
    static createContainerMap(container) {
        ...
        for (const { _name, _props: { cellsV: height, cellsH: width } } of containerTemplate._props.Grids) {
            containerMap.map[_name] = { // define the cell
                height, // set cell height (props vertical)
                width, // set cell width (props horizontal)
                rows: Array.from({ length: height }, () => Array.from({ length: width }, () => null)) // create a 2d array of nulls
            };
        }
    ...
    /*     static createContainerMap(container) {
            ...
            for (const grid of containerTemplate._props.Grids) {
                const { cellsV: height, cellsH: width } = grid._props;
    
                containerMap.map[grid._name] = { // define the cell
                    height: height, // set cell height (props vertical)
                    width: width, // set cell width (props horizontal)
                    rows: Array.from({ length: height }, () => Array.from({ length: width }, () => null)) // create a 2d array of nulls
                };
            }
            return containerMap;
    ...
    */
    ...
    static positionItemsInMap(containerMap, containerItems) {
        const items = containerItems.filter(item => item?.parentId === containerMap._id && item?.location);

        // If no items are found, return
        if (items.length === 0) {
            logger.error(`No items found to be positioned. Container id: ${containerMap['_id']}`);
            return;
        }
        ...
        for (const item of items) {
            ...
            const { y: startY, x: startX } = item.location;
            const endY = startY + itemSize.height;
            const endX = startX + itemSize.width;

            const grid = containerMap.map[item.slotId];
            if (!grid || !Array.isArray(grid?.rows)) {
                logger.error(`Invalid container map data in slot ${item.slotId}. Container id: ${containerMap['_id']}`);
                return;
            }

            // Verify that all indexes are within the bounds of the array.
            const isIndexesOutOfBounds = endY > grid['height'] || endX > grid['width'] ||
                startY < 0 || startX < 0 || itemSize.width <= 0 || itemSize.height <= 0;

            if (isIndexesOutOfBounds) {
                logger.error(`Inventory item occupies an invalid position. _id: ${item._id} _tpl: ${item._tpl} parentId: ${item.parentId} slotId: ${item.slotId} - width: ${itemSize.width} height: ${itemSize.height}`);
                continue;
            }

            for (let row = startY; row < endY; row++) {
                for (let column = startX; column < endX; column++) {
                    if (typeof grid.rows[row][column] !== "undefined") {
                        grid.rows[row][column] = item._id;
                    } else {
        ...
        logger.info(`All inventory items have been positioned correctly. Container id: ${containerMap['_id']}`);
    }
    /*     static positionItemsInMap(containerMap, containerItems) {
            const items = [];
    
            for (const item of containerItems) {
                if (item?.parentId === containerMap._id && item?.location) {
                    items.push(item);
                }
            }
            ...
            for (const item of items) {
                ...
                const grid = containerMap.map[item.slotId];
                if (!grid) {
                    logger.error(`Inventory item occupies invalid slot: _id: ${item._id} _tpl: ${item._tpl} parentId: ${item.parentId} slotId: ${item.slotId} - width: ${itemSize.width} height: ${itemSize.height}`);
                    return;
                }
    
                for (let row = startY; row < endY; row++) {
                    for (let column = startX; column < endX; column++) {
                        if (typeof grid[row][column] !== "undefined") {
                            grid[row][column] = item._id;
                        } else {
                            logger.error(`Inventory item occupies invalid slot: _id: ${item._id} _tpl: ${item._tpl} parentId: ${item.parentId} slotId: ${item.slotId} y: ${row} x: ${column} - width: ${itemSize.width} height: ${itemSize.height}`);
                            return;
                        }
    ...
    */
    static getFreeSlot(containerMap, dimensions) {
        let freeSlot = false;

        findSlot: for (const grid in containerMap.map) {
            // Check if the dimensions fit in the slot horizontally
            if (containerMap.map[grid].width >= dimensions.width && containerMap.map[grid].height >= dimensions.height) {
                freeSlot = this.checkDimensionsFit(containerMap, grid, dimensions);
                if (freeSlot) {
                    break findSlot;
                } else {
                    freeSlot = this.checkSwappedDimensionsFit(containerMap, grid, dimensions);
                    if (freeSlot) {
                        freeSlot.r = 1;
                        break findSlot;
                    }
    ...
    static checkDimensionsFit(containerMap, grid, dimensions) {
        for (let row = 0, size = (containerMap.map[grid].height - dimensions.height); row <= size; row++) {
            for (let column = 0, size = (containerMap.map[grid].width - dimensions.width); column <= size; column++) {
                let canFit = true;
                // Check if the dimensions fit in the current row and column
                for (let searchRow = row, endRow = row + dimensions.height; searchRow < endRow; searchRow++) {
                    for (let searchColumn = column, endColumn = column + dimensions.width; searchColumn < endColumn; searchColumn++) {
                        if (containerMap.map[grid].rows[searchRow][searchColumn] != null) {
                            canFit = false;
                            break;
                        }
                    }
                ...
                if (canFit) {
                    return { x: column, y: row, r: 0, slotId: grid };
                }
            }
        }
    ...
    static checkSwappedDimensionsFit(containerMap, grid, dimensions) {
        if (containerMap.map[grid].width >= dimensions.height && containerMap.map[grid].height >= dimensions.width) {
            for (let row = 0, size = (containerMap.map[grid].height - dimensions.width); row <= size; row++) {
                for (let column = 0, size = (containerMap.map[grid].width - dimensions.height); column <= size; column++) {
                    let canFit = true;
                    // Check if the swapped dimensions fit in the current row and column
                    for (let searchRow = row, endRow = row + dimensions.width; searchRow < endRow; searchRow++) {
                        for (let searchColumn = column, endColumn = column + dimensions.height; searchColumn < endColumn; searchColumn++) {
                            if (containerMap.map[grid].rows[searchRow][searchColumn] != null) {
                                canFit = false;
                                break;
                            }
                    ...
                    if (canFit) {
                        return { x: column, y: row, r: 1, slotId: grid };
                    }
                }
            }
        }
    ...
    //original code just inccase the above breaks during testing
    static _getFreeSlot(containerMap, dimensions) {
        ...
        for (const grid in containerMap.map) {
            ...
            if (containerMap.map[grid].width === dimensions.width && containerMap.map[grid].height === dimensions.height) {
                if (containerMap.map[grid].rows[0][0] != null)
                    continue;
