    ...
    */
    ...
    static async getSizeableItems(item, itemTemplate, items) {
        ...
        for (const child of items) {
            ...
            if ((itemTemplate._props.Foldable
                ...
                && (await this.isFolded(item) || await this.isFolded(child)))
                || (childItem._props.Foldable && await this.isFolded(item) && await this.isFolded(child))) {
                continue;
            }
            output.push(child);
        }
    ...
    static async createContainerMap(container) {
        ...
        for (const grid of containerTemplate._props.Grids) {
            ...
            for (let row = 0; row < containerMap.map[grid._name].height; row++) {
                containerMap.map[grid._name][row] = {}; // Define this row
                for (let column = 0; column < containerMap.map[grid._name].width; column++) {
    ...
    static async positionItemsInMap(containerMap, containerItems) {
        ...
        for (const item of items) {
            ...
            for (let row = item.location.y; row < (item.location.y + itemSize.height); row++) {
                // Iterate item height in relation to inventory squares
                for (let column = item.location.x; column < (item.location.x + itemSize.width); column++) {
    ...
    static async getFreeSlot(containerMap, dimensions) {
        ...
        for (const grid in containerMap.map) {
            ...
            if (containerMap.map[grid].width >= dimensions.width && containerMap.map[grid].height >= dimensions.height) {
                ...
                for (let row = 0; row <= (containerMap.map[grid].height - dimensions.height); row++) {
                    columnSearch: for (let column = 0; column <= (containerMap.map[grid].width - dimensions.width); column++) {
                        for (let searchRow = row; searchRow < (row + dimensions.height); searchRow++) {
                            // Search the surrounding squares in the shape of this item
                            for (let searchColumn = column; searchColumn < (column + dimensions.width); searchColumn++) {
            ...
            if (containerMap.map[grid].width >= dimensions.height && containerMap.map[grid].height >= dimensions.width) { // Check slots that are larger than the size we need, horizontally
                for (let row = 0; row <= (containerMap.map[grid].height - dimensions.width); row++) {
                    columnSearch: for (let column = 0; column <= (containerMap.map[grid].width - dimensions.height); column++) {
                        for (let searchRow = row; searchRow < (row + dimensions.width); searchRow++) { // Search the surrounding squares in the shape of this item
                            for (let searchColumn = column; searchColumn < (column + dimensions.height); searchColumn++) {
                                if (containerMap.map[grid][searchRow][searchColumn] != null) {
                                    continue columnSearch; // Search the next column to the right
                                }
                            }
