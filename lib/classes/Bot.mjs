...
export class Bot {
    ...
    constructor() {
        Object.assign(this, { ...database.core.botTemplate });
    }
    ...
    */
    static async generateBots(request = null, preload = null) {
        ...
        const generatedBots = await Promise.all(
            botsParameters.flatMap(({ Role, Limit, Difficulty }) => {
                return Array.from({ length: Limit }, () => {
                    const newBot = new Bot();

                    newBot.setIds(generateMongoID());
                    return Promise.all([
    ...
    async generateInfo(botInfo, role, difficulty) {
        ...
        info.Nickname = await this.generateNickname(botInfo, role);
        info.Side = "Savage";
        info.Voice = await this.generateVoice(botInfo, role);
        info.Settings = await this.generateSettings(info.Settings, role, difficulty);
        ...
        if (botInfo.names[role]) {
            ...
            switch (role) {
                ...
                case "followerStormtrooper":
                    return getRandomFromArray(botInfo.names.generalFollower);

                case "usec": case "bear":
                    return getRandomFromArray(botInfo.names.normal);

                default:
                    return getRandomFromArray(botInfo.names.scav);
            }
        }
    }
    ...
    async generateVoice(botInfo, role) {
        if (botInfo.appearance[role]) {
            if (typeof botInfo.appearance[role].Voice !== "string")
                return getRandomFromObject(botInfo.appearance[role].Voice);
            else
                return botInfo.appearance[role].Voice;
        } else {
            switch (role) {
                case "assault":
                case "cursedAssault":
                case "marksman":
                    return getRandomFromObject(botInfo.appearance.scav.Voice);

                case "followerGluharSecurity":
                case "followerGluharAssault":
                case "followerGluharScout":
                    return getRandomFromObject(botInfo.appearance.followerGluhar.Voice);

                default:
                    return getRandomFromObject(botInfo.appearance.random.Voice);
            }
        }
    }
    ...
    async generateCustomization(botInfo, role) {
        ...
        if (botInfo.appearance[role]) {
            customization.Head = typeof botInfo.appearance[role].Head !== "string"
                ? getRandomFromArray(botInfo.appearance[role].Head)
                : botInfo.appearance[role].Head;

            customization.Body = typeof botInfo.appearance[role].Body !== "string"
                ? getRandomFromArray(botInfo.appearance[role].Body)
                : botInfo.appearance[role].Body;

            customization.Hands = typeof botInfo.appearance[role].Hands !== "string"
                ? getRandomFromArray(botInfo.appearance[role].Hands)
                : botInfo.appearance[role].Hands;

            customization.Feet = typeof botInfo.appearance[role].Feet !== "string"
                ? getRandomFromArray(botInfo.appearance[role].Feet)
                : botInfo.appearance[role].Feet;

            return this.setCustomization(customization);
        }
        else if (["assault", "cursedAssault", "marksman"].includes(role)) {
            customization.Head = getRandomFromArray(botInfo.appearance.scav.Head);
            customization.Body = getRandomFromArray(botInfo.appearance.scav.Body);
            customization.Hands = getRandomFromArray(botInfo.appearance.scav.Hands);
            customization.Feet = getRandomFromArray(botInfo.appearance.scav.Feet);

            return this.setCustomization(customization);
        }
        else if (["followerGluharSecurity", "followerGluharAssault", "followerGluharScout"].includes(role)) {
            customization.Head = getRandomFromArray(botInfo.appearance.followerGluhar.Head);
            customization.Body = getRandomFromArray(botInfo.appearance.followerGluhar.Body);
            customization.Hands = botInfo.appearance.followerGluhar.Hands;
            customization.Feet = getRandomFromArray(botInfo.appearance.followerGluhar.Feet);

            return this.setCustomization(customization);
        }
        else {
            //logger.error(`Role [${role}] customization not handled, randomizing`);
            customization.Head = getRandomFromArray(botInfo.appearance.random.Head);
            customization.Body = getRandomFromArray(botInfo.appearance.random.Body);
            customization.Hands = getRandomFromArray(botInfo.appearance.random.Hands);
            customization.Feet = getRandomFromArray(botInfo.appearance.random.Feet);
    ...
    async generateInventory(botInfo, role) {
        ...
        const equipment = generateMongoID();
        inventory.equipment = equipment;

        const stash = generateMongoID();
        inventory.stash = stash;

        const sortingTable = generateMongoID();
        inventory.sortingTable = sortingTable;

        const questRaidItems = generateMongoID();
        inventory.questRaidItems = questRaidItems;

        const questStashItems = generateMongoID();
        inventory.questStashItems = questStashItems;

        items[0]._id = inventory.equipment;
        items[1]._id = inventory.stash;
        items[2]._id = inventory.sortingTable;
        items[3]._id = inventory.questRaidItems;
        items[4]._id = inventory.questStashItems;

        const botDefaultInventory = await this.generateInventoryItems(botInfo,
            inventory.equipment, role);
        items.push(...botDefaultInventory);

        this.setInventory(inventory);
    }

    async generateInventoryItems(botInfo, parentId, role) {
        const { bot } = database.core.gameplay
        const output = [];
...
export class BotItemGeneration {
    /**
    ...
    static async generateBotEarpiece(botInfo, parentId, role, conflictIds, earpieceChance) {
        const earpieces = await BotUtilities.cleanPartList(botInfo.bots[role].loadoutWeights.earpiece, conflictIds);

        if (earpieces && earpieces.length > 0 && getRandomInt() <= earpieceChance) {
            const choice = getRandomFromArray(earpieces);
            const earpieceTemplate = Item.get(choice);
    ...
    static async generateBotFacecover(botInfo, parentId, role, conflictIds, facecoverChance) {
        const facecovers = await BotUtilities.cleanPartList(botInfo.bots[role].loadoutWeights.facecover, conflictIds);

        if (facecovers && facecovers.length > 0 && getRandomInt() <= facecoverChance) {
            const choice = getRandomFromArray(facecovers);
            const facecoverTemplate = Item.get(choice);
    ...
    static async generateBotBodyArmor(botInfo, parentId, role) {
        const bodyArmor = botInfo.bots[role].loadoutWeights.bodyArmor;
        if (bodyArmor && bodyArmor.length > 0) {
            const choice = getRandomFromArray(bodyArmor);
            const armorTemplate = Item.get(choice);
    ...
    static async generateBotMelee(botInfo, parentId, role) {
        const melee = botInfo.bots[role].loadoutWeights.melee;
    ...
    static async generateBotWeapon(botInfo, parentId, role, slotId) {
        let weightedGear = []
        weightedGear = botInfo.bots[role].loadoutWeights;
    ...
    static async assembleWeapon(botInfo, choice, slotId, parentId) {
        ...
        const required = { ...botInfo.weaponCache.weapons[choice].Required }; //generate proper slots by sorting?????
    ...
    static async generateStockSlot(parentId, slot, choices, cachedSlots, required, conflicts) {
        ...
        const selection = this.selectWeaponPart(selections);
    ...
    static async generateFireControl(choice, slotId, parentId) {
        const base = Item.get(choice);

        if (base._props.weaponUseType === "primary" && slotId == "Holster") {
            logger.info(`Remove ${base._id} from weightedGear.primaryWeapon`);
        }
        if (base._props.weaponUseType !== "secondary" && slotId === ["FirstPrimaryWeapon", "SecondPrimaryWeapon"]) {
    ...
    static async generateAmmo(base, cachedWeapon, parentId) {
        ...
        const ammo = this.selectWeaponPart(ammos);
    ...
    static async checkForChambers(weapon, firecontrol, ammo) {
        if (weapon._props.Chambers.length > 0) {
            const chambers = await Item.generateChambersList(weapon);
        ...
        if (!cachedWeapon?.Ammo) {
            ...
            if (!read.weapons[weapon._id].hasOwnProperty("Ammo")) {
                ...
                if (cachedWeapon.Ammo) {
                    ...
                    await write("./assets/database/bot/weaponCache.json", stringify(read));
                }
            }
        } else {
    ...
    static async generateModMagazine(ReloadMode, parentId, magazines, ammo) {
        ...
        if (ReloadMode !== "OnlyBarrel") {
            ...
            const selection = this.selectWeaponPart(choices);
    ...
    static async createWeaponPart(itemId, slotId, parentId) {
        ...
        const part = await Item.createAsNewItemWithParent(item._id, parentId);
        part.slotId = slotId;
        const upd = Item.createFreshBaseItemUpd(item);
     ...
     * @returns {string}
     */
    static selectWeaponPart(selection) {
        if (selection.length === 1)
            return selection;

        const entries = Item.createWeightedList(selection);
    ...
    static async generatePatronInWeapon(patronNames, parentId, ammoId) {
        ...
        for (const name of patronNames) {
            const patronTemplate = Item.get(ammoId);

            if (!patronTemplate)
                logger.info("[generatePatronInWeapon] const patronTemplate is undefined")

            const patron = await Item.createAsNewItemWithParent(ammoId, parentId);
            patron.slotId = name;
            patron.upd = {
                StackObjectsCount: 1
            };
    ...
    static async generateAmmoInContainer(mainContainer, secureContainer, ammos, magazines = null) {
        const { randomAmmos } = database.core.gameplay.bots;
        ...
        if (randomAmmos.looseEnabled) {
            ...
            let lastchoice;
            for (let i = 0, maxTypes = randomAmmos.maxTypes; i < maxTypes; i++) {
                const choice = this.selectWeaponPart(ammos);
            ...
            if (magazines) {
                ...
                for (let i = 0, random = getRandomInt(1, 3); i < random; i++) {
    ...
    static async addMagazinesToContainer(mainContainer, securedContainer, magazines) {
        ...
        const secureContainerSize = await Item.getSize(securedContainer);

        for (const [mid, ammos] of Object.entries(magazines)) {
            //create magazine, then add ammo
            const magazine = Item.get(mid);
            const magSize = await Item.getSize(magazine);

            const mainContainerMap = await Item.generateContainerMap(mainContainer, main);
            const secureContainerMap = await Item.generateContainerMap(securedContainer, sc);

            const mainFreeSlot = Item.getFreeSlot(mainContainerMap, magSize);
            ...
            for (let i = 0; i < 2; i++) { // add double the magazines to the sc just incase
                const scFreeSlot = Item.getFreeSlot(secureContainerMap, secureContainerSize);
    ...
    static async addAmmoToContainer(mainContainer, securedContainer, ammos) {
        ...
        for (const [key, value] of Object.entries(ammos)) {
            ...
            const ammoID = Item.get(key);
            const ammoSize = await Item.getSize(ammoID);

            const mainFreeSlot = Item.getFreeSlot(mainContainerMap, ammoSize);
            if (mainFreeSlot)
                main.push(
    ...
    static async generateBotPockets(botInfo, parentId, role) {
        ...
        const authorizedGear = botInfo.bots[role].loadout;
    ...
    static async generateBotBackpack(botInfo, parentId, role, backpackChance) {
        const backpacks = botInfo.bots[role].loadoutWeights.backpack;
        if (backpacks && backpacks.length > 0 && getRandomInt() >= backpackChance) {
            const backpack = Item.get(getRandomFromArray(backpacks));
            return this.createWeaponPart(
                backpack,
                "Backpack",
                parentId
