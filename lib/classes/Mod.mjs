...
import {
    ...
    fileExist, createDirectory, logger, write, stringify, loadModule,
    ...
    */
    ...
    static async updateModInfo(dirPath) {
        if (await fileExist(`${dirPath}/mods.json`, false)) {
            ...
            if (original !== current)
                await write(`${dirPath}/mods.json`, stringify(this.mods), false);
        } else {
            await write(`${dirPath}/mods.json`, stringify(this.mods), false);
        }
    }
    ...
    */
    ...
    static async prepareAkiMod(data, toolkit) {
        if (data.main.endsWith(".ts")) {
            ...
            await write(data.dir + "/scrubbed.txt", "heheheheheh", false);
