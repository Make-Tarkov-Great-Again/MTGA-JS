...
export class Mod {
    ...
    */
    ...
    static async loadMods() {
        ...
        for (const modType of mods) {
            ...
            await Promise.all(Object.keys(modType).map(async key => {
                ...
                switch (modType) {
                    case this.mods.core:
                        return this.loadCoreMod(data, key);
                    case this.mods.community:
                        return this.loadCommunityMod(data, key);
                    case this.mods.akiCommunityMods:
                        if (this["AKI Compatibility Layer"])
                            return this.tryLoadAkiMod(data, key);
                        return;
                }
            }));
        }
    ...
    static async loadCoreMod(data, key) {
        const { mod } = await require("file://" + data.main);
        if (!mod) {
    ...
    static async tryLoadAkiMod(data, key) {
        ...
        if (!data.cleaned) {
            await prepareAkiMod(data, shim.toolkit)
        }
