...
const { database: { core: { gameplay: { 
    ...
    raid: { inRaid: { insuranceEnabled } } } } } } = require('../../app')
...
class RaidController {
    static async raidProfileSave(request, reply) {
        ...
        const listStashItems = await character.generateListItemsInContainer(stash._id); 
        // we don't want to touch items in stash
...
/*             // Not working as intended
            for (const i of character.Inventory.items){ // used for cleaning duplicates?
                if (i._id === item._id) {
                    await logger.info(`Item [${i._id}] in your Inventory has the same Item ID as item found-in-raid, let's fix that!`);
                    character.Inventory.items = await this.removeItemsWithParent(character.Inventory, item._id);
                }
            };
 */
...
class RaidUtilities {
    ...
    static async processOffraidLoot(parent, children, newId) {
        const output = [];
        for (const child of children) {

            if (child.parentId === parent._id) {
                //check if this item has children in the array
                const grandchildren = await this.processOffraidLoot(child, children, child._id);
