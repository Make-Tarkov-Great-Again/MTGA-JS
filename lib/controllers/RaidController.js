...
class RaidController {
    static async raidProfileSave(request, reply) {
        ...
        if (!["survived", "runner"].includes(endRaidData.exit)) {
            const itemsToAdd = await RaidUtilities.skillIssue(endRaidInventory, itemsToAdd);
            await RaidUtilities.addLootToInventory(character.Inventory.items, itemsToAdd);
        } else {
            await RaidUtilities.addLootToInventory(character.Inventory.items, endRaidInventory.items);
        }
...
class RaidUtilities {
    ...
    static async addLootToInventory(inventory, listItems) {
