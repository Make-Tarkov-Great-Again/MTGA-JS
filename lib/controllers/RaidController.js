...
const { database: { core: { gameplay: {
    raid: { inRaid: { insuranceEnabled } },
    development: { devInsuranceTimers,
        devInsuranceTimes: { min, max } } } } } } = require('../../app')
...
class RaidUtilities {
    ...
    static async handleInsurance(playerProfile, preraidItems, location) {
        ...
        for (const trader in traderInsurances) {
            ...
            const time = getCurrentTimestamp() + (maxReturn * 3600, minReturn * 3600);
