...
class RaidController {
    static async raidProfileSave(request, _reply) {
        ...
        const preraidItems = await RaidUtilities.findAllChildrenInInventory(
            character.Inventory,
        ...
        const pockets = await character.getInventoryItemByID("Pockets");
...
class RaidUtilities {
    ...
    static async handleInsurance(playerProfile, preraidItems, location) {
        ...
        for (const trader in traderInsurances) {
            ...
            let min_return_hour = 1;
            let max_return_hour = 1;

            const time = await getCurrentTimestamp() + (max_return_hour * 3600, min_return_hour * 3600);
            ...
            */
            ...
            await playerProfile.addDialogue(
                trader,
            ...
            playerProfile.addNotificationToQueue({
                scheduledTime: time,
                traderId: trader,
