...
class RaidController {
    static async raidProfileSave(request, reply) {
        ...
        const preraidItems = insuranceEnabled
            ?? await RaidUtilities.findAllChildrenInInventory(
                character.Inventory,
                equipment
            );
        ...
        /*         const listStashItems = await character.generateListItemsInContainer(stash);
                ...
                const protecteditems = [
                    ...
                    stash,
                    ...listStashItems,
                    //await character.getInventoryItemBySlotId("Pockets")
                ]; */
        ...
        if (!["survived", "runner"].includes(endRaidData.exit)) {
            ...
            for (const item of endRaidItems) {
                if (item.parentId) {
        ...
        for (const item of itemsToAdd) {
            const newItem = await Item.generateItemModel(item);
