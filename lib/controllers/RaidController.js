...
class RaidController {
    static async raidProfileSave(request, _reply) {
        ...
        if (endRaidData.exit !== "survived" && endRaidData.exit !== "runner") {
            ...
            console.log("My brother in christ I haven't implemented this correctly yet, sorry");
        }
...
class RaidUtilities {
    ...
    static async addItemsToPlayerInventory(character, items) {
        console.log()
    }

    static async findAllChildrenInInventory(iventory, containerId) {
        const items = [];
        let parentItems = await this.getItemsWithParent(iventory, containerId);
        while (parentItems.length > 0) {
            const childrenList = [];
            items.push(...parentItems);
            for (const item of parentItems) {
                childrenList.push(...await this.getItemsWithParent(iventory, item._id));
            }
