...
import {
    logger, float, getPercentOf,
    getRandomInt, getRandomFromArray
...
export class RepairController {
    ...
    static async repairWithKit(moveAction, character, characterChanges) {
        ...
        const itemToRepair = Inventory.getInventoryItemByID(moveAction.target);
        if (!itemToRepair)
            logger.info(`Item ${moveAction.target} can't be repaired because it doesn't exist`);

        const item = Item.get(itemToRepair._tpl);

        DurabilityController.updateItemDurability(character, itemToRepair, item, toRepair, 1, true);
        characterChanges.items.change.push(itemToRepair);

        for (const kit of repairKits) {
            const repairKit = Inventory.getInventoryItemByID(kit._id);
            if (repairKit)
        ...
        if (node.find(x => item._parent === x.id)) {
            Quest.processSkillPoints(characterChanges, playerProfile.character, "WeaponTreatment", globals.config.SkillsSettings.WeaponTreatment.SkillPointsPerRepair);
        }
        return output;
    }

    static async traderRepair(moveAction, character, characterChanges) {
        const itemToRepair = Inventory.getInventoryItemByID(moveAction.repairItems[0]._id);
        if (!itemToRepair)
        ...
        const loyalty = Character.getLoyalty(character, moveAction.tid);
        const item = Item.get(itemToRepair._tpl);

        const coef = 1 + ((loyaltyLevels[loyalty].repair_price_coef) / 100);
        let repairCost = Math.round(item._props.RepairCost * repairAmount * coef);

        const moneyItems = Inventory.getInventoryItemsByTpl(repair.currency);

        for (const moneyStack of moneyItems) {
            if (moneyStack.upd.StackObjectsCount < repairCost) {
                Inventory.removeItem(character, characterChanges, moneyStack._id, repairCost);
        ...
        DurabilityController.updateItemDurability(
            character,
            itemToRepair,
            item,
            repairAmount,
...
export class DurabilityController {
     ...
     */
    static updateItemDurability(character, toRepair, item, repairAmount, quality = 1, useKit = false) {

        const isArmor = !!(item._props?.ArmorMaterial);
        repairAmount /= this.getKitDivisor(character, item._props, isArmor)
        ...
        max = isArmor // kit repair
            ? max -= this.armorRepairDegradation(
                item._props.ArmorMaterial, max, quality, useKit)
            : max -= this.weaponRepairDegradation(item._props, max, quality, useKit);
        ...
        if (this.decideToBuff(item._props, max, character)) {
            this.getBuff(toRepair.upd, isArmor);
        };
    }

    static armorRepairDegradation(material, maxDurability, quality, useKit) {
        const { globals } = database.core;
    ...
    static weaponRepairDegradation(props, maxDurability, quality, useKit) {
        const [min, max] = [
            !useKit //set min
    ...
    static getKitDivisor(character, item, isArmor = null) {
        const { globals } = database.core;

        const { Progress } = Character.getPlayerSkill(character, "Intellect");
        const reduction = globals.config.SkillsSettings.Intellect.RepairPointsCostReduction * Math.trunc(Progress ?? 0 / 100);

        if (isArmor) {
            const armorBonus = (1.0 - (this.getBonusMultiplierValue("RepairArmorBonus", character) - 1.0) - reduction);
            ...
            const weaponBonus = (1.0 - (this.getBonusMultiplierValue("RepairWeaponBonus", character) - 1.0) - reduction);
            return weaponBonus * globals.config.RepairSettings.durabilityPointCostGuns;
        }
    }

    static getBonusMultiplierValue(skillBonusName, character) {
        const bonusesMatched = character?.Bonuses?.filter(b => b.type === skillBonusName);
        let value = 1;
        if (bonusesMatched != null) {
            const sumedPercentage = bonusesMatched.map(async b => b.value).reduce((v1, v2) => v1 + v2, 0);
    ...
    static decideToBuff(itemProperties, repairAmount, character) {
        const { globals } = database.core;

        if (character.Info.Level < globals.config.RepairSettings.MinimumLevelToApplyBuff) {
            return false;
        }
        const skillType = this.getBuffType(itemProperties);
        ...
        const playerSkill = Character.getPlayerSkill(character, skillType);
        const skillLevel = Math.trunc((playerSkill?.Progress ?? 0) / 100);

        const multiplier = this.getDurabilityMultiplier(ReceivedDurabilityMaxPercent, (repairAmount / itemProperties.MaxDurability));

        return Math.random() <= CommonBuffMinChanceValue + CommonBuffChanceLevelBonus * skillLevel * multiplier;
    }

    static getDurabilityMultiplier(ReceivedDurabilityMaxPercent, toRestorePercent) {
    ...
    static getBuff(toRepairUpd, isArmor) {
        const { globals } = database.core;
        ...
        const bonusRarity = getRandomFromArray(Item.generateWeightedList(type.rarityWeight));
        const bonusType = getRandomFromArray(Item.generateWeightedList(type.bonusTypeWeight));
