...
const RPConfig = await ConfigController.registerNewConfigEntry("DiscordRPC", "drpc", 'Use DiscordRPC', 'checkbox', 'on');
...
const STATES = {
    ...
    CONFIGS: getRandomFromArray([
        "The numbers mason! What do they mean!",
        "Nerf Nehax's cock.. Why would i!!??",
        "The fuck does \"Use Lovesense Support\" mean in this context?!",
        "Editing Configs... I guess."
    ]),
...
export class RichPresenseController {
    ...
    static async OnHome(sessionID) {
        if (await ConfigController.getValue("DiscordRPC", "drpc")) {
    ...
    static async OnConfig(sessionID) {
        if (await ConfigController.getValue("DiscordRPC", "drpc")) {
            if (!RichPresense.checkInternet()) return;
            ...
            activity.state = STATES.CONFIGS;
            activity.details = DETAILS.WEBLAUNCHER;
            activity.largeImageText = "In Config Manager"
            activity.startTimestamp = Date.now();
