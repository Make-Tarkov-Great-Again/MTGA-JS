...
class ItemController {
    ...
    static async swapItem(moveAction, reply, playerProfile) {
        for (let i = playerProfile.character.Inventory.items.length - 1; i > 0; i--) {
    ...
    static async moveItem(moveAction = null, _reply = null, playerProfile = null) {
        await logger.debug(`Move request:` + moveAction);

        const movedItems = await playerProfile.character.moveItems(moveAction);
        if (!movedItems) {
            await logger.error("[moveItem] Failed to move items, report to a developer.");
        }
        return {};
    }

    static async examineItem(moveAction = null, _reply = null, playerProfile = null) {
        await logger.debug(`Examine request:` + moveAction);
        ...
        if (typeof moveAction.fromOwner !== "undefined") {
            switch (moveAction.fromOwner.type) {
                case "Trader":
                    ...
                    if (trader) {
                        ...
                        if (inventoryItem) {
                            ...
                            await logger.error(`[examineItem] Examine Request failed: Unable to find item database template of itemId ${moveAction.item}`);
                            return false;
                        }
                    } else {
                        await logger.error("[examineItem] Examine Request failed: Unable to get trader data.");
                        return false;
                    }
                    break;
                ...
                default:
                    await logger.error(`[examineItem] Examine Request failed: Unknown moveAction.fromOwner.Type: ${moveAction.fromOwner.type}`);
                    return false;
            }
        } else {
            const item = await playerProfile.character.getInventoryItemByID(moveAction.item);
        ...
        if (templateItem) {
            if (await playerProfile.character.examineItem(templateItem._id)) {
                ...
                await logger.error(`[examineItem] Examine Request failed: Unable to examine itemId ${templateItem._id}`);
            }
        } else {
            await logger.error(`[examineItem] Examine Request failed: Unable to find item database template`);
        }
