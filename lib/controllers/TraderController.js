...
class TraderController {
    ...
    static async traderRepair(moveAction = null, _reply = null, playerProfile = null) {
        ...
        if (playerProfile) {
            ...
            let repairCost = round(itemTemplate._props.RepairCost * moveAction.repairItems[0].count * coef);
            ...
            // new max durability
            const amountRepaired = round(min(max(itemToRepair.upd.Repairable.Durability + moveAction.repairItems[0].count, 0), itemToRepair.upd.Repairable.MaxDurability));
    ...
    static async tradingConfirm(moveAction = null, _reply = null, playerProfile = null) {
        ...
        logger.logDebug(`[tradingConfirm] ` + moveAction);
        const output = {
            items: {
                new: [],
                change: [],
        ...
        if (moveAction.type === 'buy_from_trader') {
            ...
            for (const actionOffer of moveAction.offers) {
                ...
                if (itemsAdded || itemsMerged) {
                    ...
                    logger.logError(`[clientGameProfileTradingConfirm] Unable to add item(s)`);
                }
            }
