...
class TraderController {
    ...
    static async clientTradingApiGetTradersInfo(_request, reply) {
        ...
        return Response.zlibJsonReply(reply, await Response.applyBody(traders));
    }
    ...
    static async clientTradingCustomizationStorage(request, reply) {
        ...
        return Response.zlibJsonReply(reply, await Response.applyBody(await playerProfile.getStorage()));
    }
    ...
    static async getTraderAssort(request, reply) {
        ...
        return Response.zlibJsonReply(reply, await Response.applyBody(res));
    }
    ...
    static async getTraderOutfitOffers(request, reply) {
        ...
        return Response.zlibJsonReply(reply, await Response.applyBody(trader.suits));
    }

    static async getUserAssortPrice(request, reply) {
        const playerProfile = await Profile.get(await Response.getSessionID(request));
        const trader = await Trader.get(request.params.traderId);
        return Response.zlibJsonReply(reply, await Response.applyBody(await trader.getPurchasesData(playerProfile)));
    }

    static async tradingConfirm(moveAction, _reply, playerProfile) {
        await logger.debug(`[tradingConfirm] ${moveAction}`);
        const output = {
            items: {
                new: [],
                change: [],
