...
class TraderController {
    ...
    static async clientTradingCustomizationStorage(sessionID, reply) {
        ...
        const data = {
            ...
            suites: await playerProfile.getStorageSuites()
        };
    ...
    static async tradingConfirm(moveAction, _reply, playerProfile) {
        if (moveAction.type)
            logger.warn(`[tradingConfirm] ${moveAction.Action} ${moveAction.type}`);
        else
            logger.warn(`[tradingConfirm] ${moveAction.Action}`);
        ...
        switch (moveAction.Action) {
            ...
            case "RagFairBuyOffer":
                ...
                break;
            default:
                logger.warn(`[clientGameProfileTradingConfirm] My brother in christ what are you trying to do ? ${moveAction.type} ? That shit is not done lmao pay me now.`);
                break;
        }
        return output;
    }
    ...
    static async buyFromTrader(output, moveAction, playerProfile) {
        ...
        if (traderItemChildren) {
            ...
            // Handle Ammoboxes //
            if (traderItemTemplate._parent === ITEM_NODES.AMMO_CONTAINER && traderItemTemplate._props.StackSlots) {
                preparedChildren = [];
        ...
        if (itemsAdded || itemsMerged) {
            for (const scheme of moveAction.scheme_items) {
                ...
                if (ragfairOffer && ragfairOffer.buyRestrictionMax)
            ...
            logger.warn(`[clientGameProfileTradingConfirm] Unable to add items`);
            return false;
        }
    }
    ...
    static async sellToTrader(output, moveAction, playerProfile) {
        ...
        if (remainingStack) {
            const containerId = await playerProfile.character.getStashContainer();
            const container = await playerProfile.character.getInventoryItemByID(containerId);
