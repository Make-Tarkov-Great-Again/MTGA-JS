...
const { logger, fileExist, getCurrentTimestamp } = require("../utilities");
const { webinterface, database } = require('../../app');
    ...
    static async launch(request = null, reply = null) {
        ...
        if (!tarkovPath) {
            ...
            if (await fileExist(tarkovPath)) {
                // Try to spawn tarkov.
                await logger.debug("[WEBINTERFACE] Launching EscapeFromTarkov.exe...")
                ...
                await logger.debug("[WEBINTERFACE] EscapeFromTarkov.exe does not exist");
