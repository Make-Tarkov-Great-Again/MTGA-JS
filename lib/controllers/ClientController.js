...
class ClientController {
    ...
    static async clientMailDialogView(request, reply) {
        ...
        output.hasMessageWithRewards = await Dialogue.hasMessageWithRewards(dialogue.messages);
    ...
    static async clientMailDialogInfo(request, reply) {
        const { dialogues } = await Profile.get(await FastifyResponse.getSessionID(request));
        let output = {};
        if (Object.keys(dialogues).length !== 0) output = await ClientControllerUtils.createDialog(dialogues[request.body.dialogId]);
    ...
    static async clientMailDialogGetAllAttachments(request, reply) {
        ...
        const messages = [];
        for (const message of dialogue.messages) {
            if (time < (message.dt + message.maxStorageTime) &&
                !message.rewardCollected && message.hasRewards) messages.push(message);
        }
        ...
        return FastifyResponse.zlibJsonReply(
            ...
            FastifyResponse.applyBody({
                messages: messages,
                profiles: [],
                hasMessageWithRewards: await Dialogue.hasMessageWithRewards(messages)
            })
        )
