...
const { database: { core: { globals, clientSettings, locations } } } = require("../../app");
...
class ClientController {
    ...
    static async clientLanguages(reply) {
        return Response.zlibJsonReply(
            reply,
            await Response.applyBody(await Language.getAllWithoutKeys())
        );
    }
    ...
    static async clientItems(reply) {
        return Response.zlibJsonReply(
            reply,
            await Response.applyBody(await Item.getAll())
        );
    }
    ...
    static async clientCustomization(reply) {
        return Response.zlibJsonReply(
            reply,
            await Response.applyBody(await Customization.getAll())
        );
    }
    ...
    static async clientAccountCustomization(reply = null) {
        const output = [];
        const customizations = await Customization.getAllWithoutKeys();

        for (let c = customizations.length - 1; c > 0; c--) {
            const customization = customizations[c];
