...
import { Language, Profile, Account, Item, Response } from '../classes/_index.mjs';
import { getCurrentTimestamp, readParsed } from "../utilities/_index.mjs";
...
export class GameController {
    ...
    static async clientGameStart(reply, timeStamp) {
        ...
        this.generateWeightedListForBotLoadouts();

        return Response.zlibJsonReply(
            reply,
            Response.applyBody({ utc_time: timeStamp })
        );
    }

    static generateWeightedListForBotLoadouts() {
        const { bots } = database.bot;
        for (const type in bots) {
            const bot = bots[type];
            if (!bot.loadout) continue;
            bot.loadoutWeights = Item.createWeightedList(bot.loadout);
        }
    }
    ...
    static async clientGameConfig(sessionID, reply) {
        const backend = Response.getBackendUrl();
