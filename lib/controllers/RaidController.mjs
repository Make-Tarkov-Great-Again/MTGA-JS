...
import {
    ...
    Item, Inventory, Notification, Response
} from '../classes/_index.mjs';

import {
    getCurrentTimestamp, getRandomFromArray,
...
export class RaidController {
    static async showKilledMessage(_request, reply) {
    ...
    static async personKilled(request, reply) {
        const { killedByAID, diedFaction } = request.body;

        if (killedByAID !== Response.getSessionID(request)) {
        ...
        const character = Character.get(killedByAID);

        const standingChanges = {
            "Savage": killingScavsFenceLevelChange,
            "Scav": killingScavsFenceLevelChange,
            "Usec": killingPMCsFenceLevelChange,
            "Bear": killingPMCsFenceLevelChange
        };

        const standingChange = standingChanges[diedFaction];
        if (standingChange !== undefined) {
            character.TradersInfo["579dc571d53a0658a154fbec"].standing += standingChange;
            await Character.save(killedByAID);
        }
    ...
    static async raidProfileSave(request, reply) {
        ...
        if (gameplay.raid.insuranceEnabled && raid.lastLocation.insurance) {
            preraidItems = RaidUtilities.findAllChildrenInInventory(
                character.Inventory,
                equipment
            );
        };
        ...
        for (const param of [equipment, questRaidItems, sortingTable]) {
            const items = RaidUtilities.findAllChildrenInInventory(character.Inventory, param);

            if (items.length === 0)
                continue;

            for (const item of items)
                Inventory.removeInventoryItemByID(character.Inventory, item._id);
        }
...
class RaidUtilities {
    ...
    static async skillIssue(endRaidInventory) {
        ...
        if (securedContainer) { //need to check if it exists
            ...
            const securedContainerContents = RaidUtilities.findAllChildrenInInventory(endRaidInventory, securedContainer._id);
    ...
    static async handleInsurance(character, preraidItems, location) {
        ...
        for (const trader in traderInsurances) {
            ...
            */
            ...
            const traderInsureStartContent = Dialogues.createMessageContent(
            ...
            const traderInsuranceReturnContent = this.insuranceMessageContent(
                insuranceFound,
                max_storage_time,
                location
            );
    ...
    static insuranceMessageContent(insuranceFound, maxStorageTime, location) {
        return {
            ...
            profileChangeEvents: [],
            systemData: {
                date: getTimeDateMailFormat(),
                time: getTimeMailFormat(),
                location: location
            }
        };
    }
    ...
    static findAllChildrenInInventory(inventory, parentId) {
        const items = [];
        let parentItems = this.getItemsWithParent(inventory, parentId);
        while (parentItems.length > 0) {
            const childrenList = [];
            items.push(...parentItems);
            for (const item of parentItems) {
                childrenList.push(...this.getItemsWithParent(inventory, item._id));
            }
    ...
    static async addLootToInventory(character, listItems) {
        ...
        for (const loot of listItems) {
            ...
            if (loot.parentId) {
                if (Inventory.getInventoryItemByID(character.Inventory, loot._id)) {
                    Inventory.removeInventoryItemByID(character.Inventory, loot._id);
                }
    ...
    static getItemsWithParent(inventory, parentId) {
