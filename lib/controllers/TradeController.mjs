...
export class TradeController {
    ...
    static buyCustomization(moveAction, character, characterChanges) {
        const customizationSuit = Trader.getCustomizationByTraderOfferId(moveAction.offer);

        if (moveAction.items.length > 0) {
            if (!Inventory.removeItem(character.Inventory, characterChanges, moveAction.items[0].id, moveAction.items[0].count)) {
    ...
    static async buyFromTrader(moveAction, character, characterChanges) {
        ...
        const purchase = Trader.getAssortItemByID(assort, moveAction.item_id);
        ...
        const newItemsToAdd = this.processPurchase(character, purchase, itemData, assort, moveAction.count);
    ...
    static async payForTraderTrade(characterInventory, characterChanges, assort, moveAction) {
        ...
        for (const scheme of moveAction.scheme_items) { //moveAction.scheme_items (trader????) 
            if (!Trade.typeOfPurchase(characterInventory, characterChanges, scheme.id, scheme.count)) {
    ...
    static improveTraderTradeRelations(character, characterChanges, traderId, salePrice) {
        // handle lightkeeper offers, probably gonna be removed since lightkeeper isn't supposed to work like that.
        if (!character.TradersInfo[traderId])
            return;
    ...
    static processPurchase(character, item, itemData, assort, amountPurchased) {
        ...
        const children = Item.getAllChildItemsInInventory(item, assort.items);
        const itemChilds = Item.prepareChildrenForAddItem(item._id, children);
    ...
    static async mergeTradedItems(characterInventory, itemData, amountToMerge, itemsMerged) {
        ...
        const existingStacks = Inventory.getInventoryItemsByTpl(characterInventory, itemData._id);
        if (!existingStacks)
            return amountToMerge;
    ...
    static async sellToTrader(moveAction, character, characterChanges) {
        for (const itemSelling of moveAction.items) {
            const item = Inventory.getInventoryItemByID(character.Inventory, itemSelling.id);
            Inventory.removeItem(character.Inventory, characterChanges, item._id, itemSelling.count);
        }
        ...
        const newItemsToAdd = [];
        const stashContainer = Inventory.getInventoryItemByID(character.Inventory, Inventory.getStashContainer(character.Inventory));
        newItemsToAdd.push(...Inventory.addItemToInventory(character, stashContainer, currency, amountToAdd));
    ...
    static async buyFromFlea(moveAction, character, characterChanges) {
        for (const offer of moveAction.offers) {
            const listedOffer = Ragfair.getOfferById(offer.id);
            ...
            const newItemsToAdd = [];
            newItemsToAdd.push(... this.processPurchase(character, listedOffer.items[0], itemData, listedOffer, amountToAdd));
            if (newItemsToAdd.length !== 0) {
    ...
    static async payForFleaTrade(character, characterChanges, offer, listedOffer) {
        ...
        for (const scheme of offer.items) { //payTrade function needs to be modified for this
            if (!Trade.typeOfPurchase(character.Inventory, characterChanges, scheme.id, scheme.count)) {
