...
class HideoutController {
    ...
    static async startUpgradeArea(moveAction = null, playerProfile = null) {
        logger.warn(`[HideoutController.startUpgradeArea]` + moveAction);
        ...
        if (!templateHideoutArea) {
            logger.error(`[HideoutController.startUpgradeArea] Upgrading HideoutArea failed. Unknown hideout area ${moveAction.areaType} in hideoutArea database.`);
            return false;
        }

        if (!characterHideoutArea) {
            logger.error(`[HideoutController.startUpgradeArea] Upgrading HideoutArea failed. Unknown hideout area ${moveAction.areaType} in character profile.`);
            return false;
        }

        const nextLevel = characterHideoutArea.level + 1;
        if (!templateHideoutArea.stages[nextLevel]) {
            logger.error(`[HideoutController.startUpgradeArea] Upgrading HideoutArea ${templateHideoutArea._id} failed. The level ${nextLevel} doesn't exist.`);
            return false;
        }
        ...
        for (const itemToTake of moveAction.items) {
            ...
            if (!itemTaken) {
                logger.error(`[HideoutController.startUpgradeArea] Upgrading HideoutArea ${templateHideoutArea._id} failed. Unable to take required items.`);
                return false;
            }
    ...
    static async completeUpgradeArea(moveAction = null, playerProfile = null) {
        ...
        if (typeof templateHideoutAreaStage === "undefined") {
            logger.error(`[completeUpgradeArea] Upgrading HideoutArea ${templateHideoutArea._id}. The level ${nextLevel} doesn't exist.`);
            return false;
        }
    ...
    static async singleProductionStart(moveAction = null, playerProfile = null) {
        logger.warn(`[singleProductionStart]` + moveAction);
