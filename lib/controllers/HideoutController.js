...
class HideoutController {
    ...
    static async clientHideoutQTEList(){
        return [];
    }
    ...
    static async startUpgradeArea(moveAction = null, _reply = null, playerProfile = null) {
        await logger.debug(`[startUpgradeArea]` + moveAction);
        if (playerProfile) {
            ...
            if (!templateHideoutArea) {
                await logger.error(`[startUpgradeArea] Upgrading HideoutArea failed. Unknown hideout area ${moveAction.areaType} in hideoutArea database.`);
                return;
            }

            if (!characterHideoutArea) {
                await logger.error(`[startUpgradeArea] Upgrading HideoutArea failed. Unknown hideout area ${moveAction.areaType} in character profile.`);
                return;
            }

            const nextLevel = characterHideoutArea.level + 1;
            if (typeof templateHideoutArea.stages[nextLevel] === "undefined") {
                await logger.error(`[startUpgradeArea] Upgrading HideoutArea ${templateHideoutArea._id} for character ${playerProfile.character._id} failed. The level ${nextLevel} doesn't exist.`);
                return;
            }
            ...
            if (allItemsTaken) {
                ...
                await logger.error(`[startUpgradeArea] Upgrading HideoutArea ${templateHideoutArea._id} for character ${playerProfile.character._id} failed. Unable to take required items.`);
                return;
            }
        }
    }
    ...
    static async completeUpgradeArea(moveAction = null, _reply = null, playerProfile = null) {
        if (playerProfile) {
            ...
            if (typeof templateHideoutAreaStage === "undefined") {
                await logger.error(`[completeUpgradeArea] Upgrading HideoutArea ${templateHideoutArea._id} for character ${playerProfile.character._id} failed. The level ${nextLevel} doesn't exist.`);
                return;
            }
    ...
    static async toggleArea(moveAction = null, _reply = null, playerProfile = null) {
        if (playerProfile) {
            const hideoutArea = await playerProfile.character.getHideoutAreaByType(moveAction.areaType);
            if (!hideoutArea) {
                await logger.error(`[toggleArea] Unable to find hideout area type ${moveAction.areaType} for playerProfile ${playerProfile.character._id}.`);
                return;
            }
    ...
    static async singleProductionStart(moveAction = null, _reply = null, playerProfile = null) {
        ...
        if (playerProfile) {
            ...
            if (!hideoutProductionTemplate) {
                await logger.error(`[singleProductionStart] Starting hideout production failed. Unknown hideout production with Id ${moveAction.recipeId} in hideoutProduction database.`);
                return;
            }
