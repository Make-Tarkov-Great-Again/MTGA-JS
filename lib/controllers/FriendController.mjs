import { zlibJsonReply, applyBody, getSessionID, applyEmpty, generateMongoID, getCurrentTimestamp, logger } from "../utilities/_index.mjs";
...
export class FriendController {
    ...
    static async clientMailMessageSend(request, reply) {
        const yourProfile = Character.get(await getSessionID(request));
    ...
    static async clientFriendRequestSend(request, reply) {
        ...
        const yourSessionID = await getSessionID(request)
    ...
    static async clientFriendDelete(request, reply) {
        const yourSessionID = getSessionID(request);
        const yourAccount = Account.getWithSessionId(yourSessionID);
