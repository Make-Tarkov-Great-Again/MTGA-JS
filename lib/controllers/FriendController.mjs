...
export class FriendController {
    ...
    static async clientMailMessageSend(request, reply) {
        const yourProfile = Character.get(Response.getSessionID(request));
        const theirProfile = Character.get(request.body.dialogId);

        const yourDialog = Dialogues.createPlayerDialogue(request.body.dialogId, yourProfile.aid);
        const theirDialog = Dialogues.createPlayerDialogue(yourProfile.aid, request.body.dialogId);

        const yourMessage = Dialogues.playerMessageForMessageSend(request.body.text, yourProfile.aid);
