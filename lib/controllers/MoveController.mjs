...
import { Profile, Character, Response } from "../classes/_index.mjs";
...
export class MoveController {
    ...
    static async clientGameProfileItemsMoving(request, reply) {
        ...
        const outputData = Character.getChangesTemplate(playerProfile.character);
        const characterChanges = outputData.profileChanges[playerProfile.character._id];

        for (let a = 0, length = request.body.data.length; a < length; a++) {
            ...
            switch (moveAction.Action) {
                ...
                case "OpenRandomLootContainer":
                    ...
                    ItemController.itemActions(moveAction, playerProfile.character, characterChanges);
                    break;
                ...
                case "HideoutImproveArea":
                    ...
                    HideoutController.hideoutActions(moveAction, playerProfile.character, characterChanges);
                    break;

                case "AddNote":
                case "EditNote":
                case "DeleteNote":
                    NoteController.noteActions(moveAction, playerProfile.character, characterChanges);
                    break;
