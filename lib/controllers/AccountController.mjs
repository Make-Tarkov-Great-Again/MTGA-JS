 ...
 //               DiscordRPC.OnServerLogin(userAccount.id) //Not hititng on login
    ...
    static async tauriLogin(request = null, reply = null) {
        ...
        if (request.body.email !== null && request.body.password !== null) {
            logger.warn("[ACCOUNT CONTROLLER: LOGIN] " + Account.getAccountWithEmail(request.body.email));
            ...
            logger.warn(userAccount);
            console.log(`sessionID: ${userAccount.id} \n Username: ${displayname}correct: true`)
            if (userAccount && request.body.password === userAccount.password)

                return reply.send({
                    sessionID: userAccount.id,
    ...
    static async update(request = null, reply = null) {
        ...
        if (request.body.wipe) {
            ...
            const accountDirectory = getFilesFrom(accountPath);
