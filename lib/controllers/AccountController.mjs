 ...
 */
    static async tauriLogin(request = null, reply = null) {
        ...
        if (request.body.email !== null && request.body.password !== null) {
            ...
            const { LowerNickname, Side, Level, Experience } = database.profiles?.[sessionID]?.character?.Info;

            //const displayname = database.profiles[sessionID]?.character?.Info?.LowerNickname
            //const side = database.profiles[sessionID]?.character?.Info?.Side
