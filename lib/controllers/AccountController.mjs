...
export class AccountController {
 ...
 //               DiscordRPC.OnServerLogin(userAccount.id) //Not hititng on login
    ...
    static async update(request = null, reply = null) {
        ...
        // clienttype option
        if (request.body.clienttype !== userAccount.clienttype) {
            userAccount.clienttype = request.body.clienttype;
        }       
        if (userAccount.clienttype !== 'default') {
            logger.trace(`Profile for Account ${sessionID} requires scripts`);
        } else {
            logger.trace(`Profile for Account doesn't requires scripts`);
        }
