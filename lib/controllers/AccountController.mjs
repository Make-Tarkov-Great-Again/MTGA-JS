...
export class AccountController {
    ...
    static async login(request = null, reply = null) {
        ...
        if (webinterface.checkForSessionID(request))
            return webinterface.renderMessage("Error", RENDER_MESSAGES.INCORRECT_CALL);
    ...
    * @param {*} request
    * @param {*} reply
    * @returnstotauri sessionID, username, experience, side, level, correct
    */
    static async tauriLogin(request = null, reply = null) {
        reply.type("application/json");
        logger.warn(request);
        if (request.body.email !== null && request.body.password !== null) {
            const userAccount = Account.getAccountWithEmail(request.body.email);
            if (!userAccount) {
                logger.error("[ACCOUNT CONTROLLER: LAUNCHER LOGIN] User Account not found");
                reply.type("text/html");
                return "INVALID_CREDENTIALS"
            }

            const sessionID = userAccount.id;
            const character = database.profiles[sessionID]?.character;
            const { LowerNickname = "", Side = "", Level = 0, Experience = 0 } = character?.Info ?? {};

            if (request.body.password === userAccount.password)
                return reply.send({
                    sessionID: sessionID,
                    username: LowerNickname,
                    experience: Experience,
                    side: Side,
                    level: Level,
                    correct: true
                });
            //TODO: Maybe health, production, or fuel levels if MTGA keeps track of shit while user is offline.
            else {
                reply.type("text/html");
                return "INVALID_CREDENTIALS"
            }
    ...
    static async launcherLogin(request = null, reply = null) {
        ...
        logger.warn(request);
        if (request.body.email !== null && request.body.password !== null) {
            logger.warn("[ACCOUNT CONTROLLER: WEBLAUNCHER LOGIN] " + Account.getAccountWithEmail(request.body.email));
            const userAccount = Account.getAccountWithEmail(request.body.email);
            logger.warn(userAccount);
            if (userAccount && request.body.password === userAccount.password)
    ...
    static async create(request = null, reply = null) {
        ...
        if (Account.getWithSessionId(webinterface.checkForSessionID(request)))
            return webinterface.renderMessage("Error", RENDER_MESSAGES.INCORRECT_CALL);

        let editionsHTML = "";
        for (const value of Edition.getAllKeys()) {
    ...
    static async tauriRegister(request = null, reply = null) {
        logger.warn("[TAURI LAUNCHER] Registering new account...");
        ...
        if (Account.getAccountWithEmail(request.body.email)) {
        ...
        if (newAccountID !== (undefined || null || false)) {
            logger.warn('[TAURI LAUNCHER] Registration successful for account ID: ' + newAccountID);
            return reply.send({
                sessionID: newAccountID,
    ...
    static async edit(sessionID, reply) {
        ...
        let editionsHTML = "";
        const editions = Edition.getAllKeys();
        ...
        clienttypechoices['default'] = 'Default';
        clienttypechoices['script'] = 'Download scripts';

        let clienttypechoicesHTML = '';

        for (let key in clienttypechoices) {

            if (userAccount.clienttype === key) {
                clienttypechoicesHTML += '<option value="' + key + '" selected>' + clienttypechoices[key] + '</option>';
            }
            else {
                clienttypechoicesHTML += '<option value="' + key + '">' + clienttypechoices[key] + '</option>';
            }
        ...
        const pageVariables = {
            ...
            "clienttypechoices": clienttypechoicesHTML
        };
    ...
    static async update(request = null, reply = null) {
        ...
        if (!sessionID) return reply.redirect(ROUTES_REDIRECT.LOGIN);

        const userAccount = Account.getWithSessionId(sessionID);
        if (!userAccount) return reply.redirect(ROUTES_REDIRECT.LOGIN);
        ...
        if (request.body.wipe) {
            const accountPath = Account.getAccountDirectory(sessionID);
            ...
            const filesToDelete = ["character", "storage", "special", "dialogues"];
            for (const index of accountDirectory) {
                const name = index.replace(".json", "");
                if (!filesToDelete.includes(name)) continue;
                const filePath = `${accountPath}/${index}`;
                if (await fileExist(filePath, true)) await deleteFile(filePath, true);
            }
