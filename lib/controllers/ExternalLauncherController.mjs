...
export class LauncherControllerinator {
    static async LaunchGame(request, reply) {
        ...
        if (database.profiles[sessionID]) {
            if (fileExist(tarkovPath)) {
                ...
                console.log(`${tarkovPath}} -token=${sessionID} -config={"BackendUrl":"https://${serverConfig.ip}:${serverConfig.port}","Version":"live"}`)
                try {
    ...
    static async UpdateProfileDataInLauncher(request, reply){
        const sessionID = request.body.sessionID
