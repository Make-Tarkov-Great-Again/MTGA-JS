...
class GameController {
    ...
    static async modeOfflinePatches(_request = null, reply = null) {
        const { database: { core: { serverConfig: { Patches } } } } = require('../../app');

        return FastifyResponse.zlibJsonReply(reply, Patches);
    }

    static async modeOfflinePatchNodes(_request = null, reply = null) {
        const { database: { core: { serverConfig: { PatchNodes } } } } = require('../../app');
        return FastifyResponse.zlibJsonReply(reply, PatchNodes);
    }
    ...
    static async clientGameProfileAcceptQuest(moveAction = null, _reply = null, playerProfile = null, sessionID = null) {
        ...
        const messageContent = {
            templateId: questLocale.description, //is description for whatever reason
            type: 10, //start
            maxStorageTime: RedeemTime * 3600
        };
        await playerProfile.addDialogue(quest.traderId, messageContent, questReward, sessionID);
        return {};
    }
