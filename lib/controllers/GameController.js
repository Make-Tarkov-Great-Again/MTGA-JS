...
class GameController {
    ...
    static async clientGameProfileCreate(request = null, reply = null) {
        ...
        character.Info.RegistrationDate = getCurrentTimestamp();
        ...
        switch (validate) {
            ...
            default:
                ...
                return FastifyResponse.zlibJsonReply(
                    ...
                    FastifyResponse.applyBody({
                        status: 0,
                        nicknamechangedate: getCurrentTimestamp()
                    })
                );
        }
    };
        ...
        if (moveAction.type === 'buy_from_trader') {
            ...
            if (itemsAdded || itemsMerged) {
                ...
                for (const scheme of moveAction.scheme_items) {
                    ...
                    const ragfairOffer = await RagfairOffer.getOfferByItemId(moveAction.item_id);
                    if (ragfairOffer) {
            ...
            const offer = await RagfairOffer.getById(moveAction.offers[0].id);
            console.log(offer.items[0]._tpl);
