...
const { generateMongoID, getCurrentTimestamp, logger, FastifyResponse, readParsed, generateMongoID } = require("../../utilities");
...
class GameController {
    ...
    static async clientProfileList(request = null, reply = null) {
        ...
        logger.logDebug("SC4V-DVMMY deployed...");
    ...
    static async clientGameProfileTradingConfirm(moveAction = null, _reply = null, playerProfile = null) {
        ...
        if (moveAction.type === 'buy_from_trader') {
            ...
            logger.logConsole(item.items[0]._tpl)
            ...
            if (maxStack) {
                ...
                logger.logConsole(itemsMerged);
                logger.logConsole(remainingStack);
            }
            if (remainingStack) {
