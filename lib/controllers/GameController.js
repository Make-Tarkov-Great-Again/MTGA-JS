...
let { database: { core: {
    ...
    gameplay: { seasonalEvents, hideout } } } } = require('../../app');

const { getCurrentTimestamp, logger, Response, readParsed } = require("../utilities");
const Profile = require('../routes/client/Profile');
...
class GameController {
    ...
    static async clientGameStart(reply, timeStamp, sessionID) {
        ...
        await this.basedMopEvent(sessionID, timeStamp);
    ...
    static async basedMopEvent(sessionID, timeStamp) {
        const { character } = await Profile.get(sessionID);
        if (!character?.Info?.RegistrationDate)
            return;

        const seconds = hideout.wallUnlockInSeconds;
        const wallUnlockTimestamp = (seconds + character.Info.RegistrationDate);
        if (wallUnlockTimestamp < timeStamp) {
            const type = await character.getHideoutAreaTypeValueByName("EMERGENCY_WALL");
            const wall = await character.getHideoutAreaByType(type);

            if (wall && wall.level === 1) {
                wall.level++;
                await logger.debug("Enough time has passed, upgrading hideout wall");
            }
        }
    }
