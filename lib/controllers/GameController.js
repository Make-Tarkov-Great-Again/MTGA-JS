...
class GameController {
    ...
    static async clientGameProfileAcceptQuest(moveAction, playerProfile, sessionID) {
        ...
        const { lang } = await Account.get(sessionID);
        const locales = await Locale.get(lang);


        const questLocale = await locales.getQuestLocales(quest._id);

        let dialogue = await Dialogue.get(sessionID);
        if (!dialogue) dialogue = new Dialogue(sessionID);

        const messageContent = await dialogue.createMessageContent(
            questLocale.description,
            10,
            (RedeemTime * 3600))
        const generatedDialogue = await dialogue.generateDialogue(quest.traderId, messageContent, sessionID, questReward);
        await playerProfile.sendNotificationMessage(generatedDialogue, sessionID)
        return {};
    }
