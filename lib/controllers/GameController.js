...
class GameController {
    ...
    static async clientGameVersionValidate(version, reply) {
        await logger.info(`Client connected with version: ${version}`);
        return Response.zlibJsonReply
            (
                reply,
    ...
    static async clientGameConfig(sessionID, reply) {
        ...
        const languages = await Language.getAllWithoutKeys();
        const output = {};
        for (const language of languages){
            output[language.ShortName] = language.Name;
        }
