...
class ProfileController {
    ...
    static async profileList(sessionID, reply) {
        ...
        if (!playerAccount.wipe) {
            ...
            if (character && Object.keys(character).length !== 0) {
                ...
                pmc.savage = playerScav._id;
                output.push(playerScav, pmc);
            } else {
    ...
    static async profileSelect(sessionID, reply) {
        return Response.zlibJsonReply(
            ...
            await Response.applyBody({
                status: "ok",
                notifier: Response.getNotifier(sessionID),
                notifierServer: ""
            })
        );
    }
    ...
    static async profileCreate(request, reply) {
        ...
        //character.Inventory.items = await this.processInventoryItems(character.Inventory);
