...
const { getCurrentTimestamp, logger, Response, readParsed, generateMongoID, round } = require("../../utilities");
...
class ProfileController {
    ...
    static async profileNicknameValidate(request = null, reply = null) {
        ...
        switch (validate) {
            case "tooshort":
                return Response.zlibJsonReply(
                    reply,
                    Response.applyBody(null, 256, "256 -")
                );
            case "taken":
                return Response.zlibJsonReply(
                    reply,
                    Response.applyBody(null, 255, "255 - ")
                );
            default:
                return Response.zlibJsonReply(
                    reply,
    ...
    static async addToWishList(moveAction, playerProfile) {
        if (playerProfile.character) {
            logger.info(`[AddToWishList] ${moveAction}`)
    ...
    static async removeFromWishList(moveAction, playerProfile) {
        if (playerProfile.character) {
            logger.info(`[RemoveFromWishList] ${moveAction}`)
