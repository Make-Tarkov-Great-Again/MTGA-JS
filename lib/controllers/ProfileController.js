...
class ProfileController {
    ...
    static async applyInventoryChanges(moveAction, playerProfile) {
        ...
        const items = playerProfile.character.Inventory.items;
        // Changed Items //
        for (const inventoryChange of moveAction.changedItems) {
            ...
            if (item) {
                ...
                await playerProfile.character.removeInventoryItemByID(inventoryChange._id);

                if (typeof item._tpl !== "string")
                    item = inventoryChange;
                else if (typeof inventoryChange.location !== "undefined") {
                ...
                if (await Item.checkIfTplIsMoney(item._tpl)) {
        ...
        playerProfile.character.Inventory.items = items;
    }
