...
import { cloneDeep, readParsed } from "../utilities/_index.mjs";
...
export class BundlesController {
    ...
    async addBundles(modPath) {
        ...
        for (const bundle of manifest) {
            const path = modPath.substring(modPath.indexOf('/user') + 1);
            const bundlePath = `${Response.getBackendUrl()}files/bundle/${bundle.key}`;
            const bundleFilepath = bundle.path || `${path}/bundles/${bundle.key}`.replace(/\\/g, "/");
            this.bundles[bundle.key] = this.BUNDLE(path, bundle, bundlePath, bundleFilepath);
        }
    }
}