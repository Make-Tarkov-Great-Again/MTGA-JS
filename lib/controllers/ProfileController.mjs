...
import { getCurrentTimestamp, logger, zlibJsonReply, 
    applyBody, getSessionID, round } from "../utilities/_index.mjs";
...
export class ProfileController {
    ...
    static async profileCreate(request, reply) {
        ...
        return zlibJsonReply(
            reply,
            await applyBody({ uid: character.aid })
        );
    }

    static async profileVoiceChange(request, reply) {
        const sessionID = await getSessionID(request);
        Character.setCharacterVoice(await getSessionID(request), request.body.voice);
    ...
    static async profileNicknameValidate(request = null, reply = null) {
        switch (await Profile.isAvailableNickname(request.body.nickname)) {
            case "tooshort":
                return zlibJsonReply(
                    reply,
                    await applyBody(null, 256, "The nickname is too short")
                );
            case "taken":
                return zlibJsonReply(
                    reply,
                    await applyBody(null, 255, "The nickname is already in use")
                );
            case "ok":
                return zlibJsonReply(
                    reply,
    ...
    static async profileNicknameChange(request, reply) {
        switch (await Profile.isAvailableNickname(request.body.nickname)) {
            case "tooshort":
                return zlibJsonReply(reply, await applyBody(null, 256, "256 -"));
            case "taken":
                return zlibJsonReply(reply, await applyBody(null, 255, "255 - "));
            case "ok":
                const sessionID = await getSessionID(request);
                ...
                return zlibJsonReply(
                    reply,
                    await applyBody({ status: 0, nicknamechangedate: getCurrentTimestamp() })
                );
        };
    }
