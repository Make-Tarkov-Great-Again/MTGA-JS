...
export class ProfileController {
    ...
    static async profileActions(moveAction, profile, characterChanges) {
        switch (moveAction.Action) {
            ...
            case "ResetWishList":
                return this.resetWishList(profile.character);
            case "CustomizationBuy":
                return this.customizationBuy(moveAction, profile, characterChanges);
    ...
    static async profileStatus(request, reply) {
        ...
        const { savage, _id } = await Character.get(sessionID);
    ...
    static async profileList(sessionID, reply) {
        ...
        const character = await Character.get(sessionID);
    ...
    static async profileCreate(request, reply) {
        const sessionID = await Response.getSessionID(request);
        const playerAccount = await Account.getWithSessionId(sessionID);
        if (!playerAccount) {
    ...
    static async customizationBuy(moveAction, profile, characterChanges) {
        if (moveAction.items.length > 0) {
            //const itemTaken = await Inventory.removeItem(profile.character.Inventory, characterChanges, moveAction.items[0].id, moveAction.items[0].count);
            if (!await Inventory.removeItem(profile.character.Inventory, characterChanges, moveAction.items[0].id, moveAction.items[0].count))
        ...
        await Storage.addCustomization(profile, customizationSuit._id);
