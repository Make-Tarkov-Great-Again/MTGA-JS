...
class Ragfair extends BaseModel {
    ...
    static async generateAmountOffersPerCategories(offers, filters = false) {
        ...
        if (filters) {
            ...
            for (let i = offers.length - 1; i > -1; i--) {
                const item = offers[i].items?.[0] ? offers[i].items[0] : offers[i].items;
    ...
    static async getOffersWithCategories(categories, offers) {
        return offers.filter(function (ragfairOffer) {
            const offer = ragfairOffer.items?.[0]?._tpl
                ? ragfairOffer.items[0]._tpl
                : ragfairOffer.items._tpl

            return offer in categories;
        });
    }
    ...
    static async reduceOffersBasedOnFilterRequest(offers, request) {
        ...
        for (let i = offers.length - 1; i > -1; i--) {
            ...
            if ((request.currency != 0
                ...
                offers.splice(i, 1); // Remove this offer
            } else continue;
        }
    ...
    static async getTplFromHandbook(handbookId) {
        ...
        if (bastards.includes(handbookId)) {
            ...
            for (let a = categories.length - 1; a > -1; a--) {
                categories = await childrenCategories(categories[a]);
                for (let b = categories.length - 1; b > -1; b--) {
                    result = result.concat(await templatesWithParent(categories[b]));
                }
            }
        } else {
        ...
        switch (request.body.sortType) {
            ...
            case 4: // Offer (title)
                ...
                offers.sort((a, b) => {
                    ...
                    const aa = locale[`${ia} Name`].Name || ia;
