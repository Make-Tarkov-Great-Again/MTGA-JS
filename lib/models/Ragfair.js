...
class Ragfair extends BaseModel {
    ...
    async generateOffersBasedOnRequest(request) {
        ...
        if (request.neededSearchId) {
            const foundOffers = await Ragfair.getNeededFor(request.neededSearchId, this.offers);
            output.categories = await this.generateAmountOffersPerCategories(await Ragfair.getCategoriesForOffers(foundOffers));
            output.offers = await this.getOffersWithCategories(output.categories);
            ...
            output.categories = await this.generateAmountOffersPerCategories(await Ragfair.getLinkedTo(request.linkedSearchId));
            output.offers = await this.getOffersWithCategories(output.categories);
            // when the user click a categorie, we want to show only the items related to the categorie
            if (request.handbookId) {
                const categoriesFilter = await Ragfair.getTplFromHandbook(request.handbookId);
            ...
            const filteredCategories = await this.generateAmountOffersPerCategories(categoriesFilter); // we get the amount of offers per sub category
            output.categories = this.categories; // we want all categories to show
            output.offers = await this.getOffersWithCategories(filteredCategories); // we get the offers with the current categories (handbook & subcategories)
        }
        ...
        output.offers = output.offers.slice(request.page * request.limit, output.offers.length);
