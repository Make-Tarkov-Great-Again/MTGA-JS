...
class Ragfair extends BaseModel {
    ...
    static async reduceOffersBasedOnFilterRequest(offers, request) {
        ...
        for (const thisOfferObj of offers) {
    ...
    static async getTplFromHandbook(handbookId) {
        ...
        if (bastards.includes(handbookId)) {
            ...
            for (const one of categories) {
                categories = await childrenCategories(one);
                for (const two of categories) {
                    result = result.concat(await templatesWithParent(two));
                }
            }
        } else {
            if (await isCategory(handbookId)) {
                ...
                for (const category of categories) {
                    result = result.concat(await templatesWithParent(category));
                }
            } else {
