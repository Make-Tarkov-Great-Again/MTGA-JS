...
const {
    ...
    childrenCategories, isCategory, Response
} = require("../utilities");
...
class Ragfair extends BaseModel {
    ...
    static async generateAmountOffersPerCategories(offers, filters = false) {
        ...
        if (filters) {
            ...
            for (let o = offers.length -1; o >0; o--) {
                const item = offers[o].items[0];
    ...
    static async getLinkedTo(searchId) {
        ...
        for (let i = items.length - 1; i > 0; i--) {
            if (!await items[i].isBlacklisted()) {
                output.push(items[i]);
            }
        }
        const needed = [];
        ...
        for (let i = output.length - 1; i > 0; i--) {
            if (
                await this.checkFilters(output[i], "Slots", searchId) ||
                await this.checkFilters(output[i], "Chambers", searchId) ||
                await this.checkFilters(output[i], "Cartridges", searchId) ||
                await this.checkFilters(output[i], "Cartridges", searchId)
            )
                needed.push(output[i]._id);
        }
    ...
    static async getTplFromHandbook(handbookId) {
        ...
        if (bastards.includes(handbookId)) {
            let categories = await childrenCategories(handbookId);
            for (let c = categories.length - 1; c > 0; c--) {
                categories = await childrenCategories(categories[c]);
                for (let c1 = categories.length - 1; c1 > 0; c1--) {
                    result = result.concat(await templatesWithParent(categories[c1]));
                }
            }
        } else {
            if (await isCategory(handbookId)) {
                ...
                const categories = await childrenCategories(handbookId);
                for (let c = categories.length - 1; c > 0; c--) {
                    result = result.concat(await templatesWithParent(categories[c]));
                }
            } else {
    ...
    async generatePlayersOffers() {
        ...
        for (let i = items.length - 1; i > 0; i--) {
            if (await items[i].isBlacklisted() || await Preset.itemHasPreset(items[i]._id))
                continue;
            const offerItems = [];
            offerItems.push({
            ...
            const itemPrice = await Item.getItemPrice(items[i]._id);
