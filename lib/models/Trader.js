...
const { findAndReturnChildrenByItems, readParsed, getAbsolutePathFrom, findChildren, generateItemId, getCurrentTimestamp } = require("../../utilities");
        ...
        for (const item of this.assort.items) {
            if (item.slotId === "hideout") {
                ...
                for (const barterID in this.assort.barter_scheme) {
                    if (item._id === barterID) {
                        fullItem.barter = this.assort.barter_scheme[barterID][0];
                        break;
                    }
                }
                for (const loyalID in this.assort.loyal_level_items) {
                    if (item._id === loyalID) {
                        fullItem.loyality = this.assort.loyal_level_items[item._id];
                        break;
                    }
                }
                const offerTimestamp = await getCurrentTimestamp();
                const newRagfairOffer = {
                    ...
                    root: fullItem.item[0]._id,
                    items: fullItem.item,
                    itemsCost: parseInt(fullItem.barter[0].count),
                    requirements: fullItem.barter,
                    requirementsCost: parseInt(fullItem.barter[0].count),
                    summaryCost: parseInt(fullItem.barter[0].count),
                    sellInOnePiece: false,
                    startTime: offerTimestamp - 3600,
                    endTime: offerTimestamp + 3600,
                    locked: false,
                    loyaltyLevel: fullItem.loyality,
                    unlimitedCount: false
                };
                if (typeof fullItem.item === "object" && fullItem.item.hasOwnProperty("upd") && fullItem.item.hasOwnProperty("UnlimitedCount")) {
                        newRagfairOffer.unlimitedCount = fullItem.item.upd.UnlimitedCount;
                } else {
                    if (fullItem.item[0].hasOwnProperty("upd") && fullItem.item[0].hasOwnProperty("UnlimitedCount")) {
                        newRagfairOffer.unlimitedCount = fullItem.item[0].upd.UnlimitedCount;
                    }
                }
                convertedOffers.push(newRagfairOffer);
                offerIntID+=1;
            }
        }
