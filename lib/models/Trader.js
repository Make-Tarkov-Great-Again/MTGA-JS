...
const { round, findAndReturnChildrenByItems, readParsed, getAbsolutePathFrom, findChildren, generateMongoID, getCurrentTimestamp, logger } = require("../../utilities");
    ...
    async generateAssort(currentTime) {
        ...
        if (!this.assort) {
            ...
            if (!this.assort.nextResupply || this.assort.nextResupply <= currentTime) {
                const assort = cloneDeep(this.baseAssort);
                assort.nextResupply = currentTime + refreshTimeInMinutes * 60;
