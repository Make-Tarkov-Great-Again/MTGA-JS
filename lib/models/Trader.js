...
const { findAndReturnChildrenByItems, readParsed, getAbsolutePathFrom, findChildren, generateMongoID, getCurrentTimestamp, logger } = require("../../utilities");
...
class Trader extends BaseModel {
    ...
    async solve() {
        ...
        for (const [index, item] of Object.entries(this.baseAssort.items)) {
            this.baseAssort.items[index] = await UtilityModel.createModelFromParse("Item", item);
        }
    }
    ...
    async resetOffers(profile) {
        const filteredAssort = await this.filterAssort(profile);
        this.assort = filteredAssort;
    }

    async filterAssort(profile) {
        const filteredAssort = {
            nextResupply: 0,
            items: [],
            barter_scheme: {},
            loyal_level_items: {}
        ...
        if (!this.isRagfair()) {
            for (const item of this.baseAssort.items) {
                if (!this.baseAssort.loyal_level_items[item._id] || this.baseAssort.loyal_level_items[item._id] <= loyalty) {
                    filteredAssort.items.push(item);
                    if (this.baseAssort.barter_scheme[item._id]) {
                        filteredAssort.barter_scheme[item._id] = this.baseAssort.barter_scheme[item._id];
                    }
                    if (this.baseAssort.loyal_level_items[item._id]) {
                        filteredAssort.loyal_level_items[item._id] = this.baseAssort.loyal_level_items[item._id];
                    }
                }
            }
        }
        return filteredAssort;
    }

    async getAssort(profile) {
        const currentTime = await getCurrentTimestamp();
        if (!this.assort || !this.assort.nextResupply || this.assort.nextResupply <= currentTime) {
            await this.resetOffers(profile);
            this.assort.nextResupply = currentTime + database.core.gameplay.trading.refreshTimeInMinutes * 60;
        }
        const output = {
            nextResupply: this.assort.nextResupply,
            items: this.assort.items,
            barter_scheme: this.assort.barter_scheme,
            loyal_level_items: this.assort.loyal_level_items
        };
        return output;
    }
        ...
        for (const item of this.baseAssort.items) {
            if (item.slotId === "hideout") {
                ...
                if (childlessList.includes(item._id)) {
                    ...
                    fullItem.item = await findChildren(item._id, this.baseAssort.items);
                }
    ...
    async removeItemFromAssort(moveData) {
