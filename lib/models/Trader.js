...
const { round, readParsed, generateMongoID, getCurrentTimestamp, logger, cloneDeep, readParsedSync } = require("../utilities");
const { Item } = require("./Item");
const childlessList = readParsedSync(`./assets/database/childlessList.json`);
...
class Trader extends BaseModel {
    ...
    async getRagfairOffers() {
        ...
        for (let i = this.assort.items.length - 1; i > -1; i--) {
            ...
            if (item.slotId === "hideout") {
                ...
                const fullItem = {};
                fullItem.item = childlessList.includes(item._tpl)
                    ? item
