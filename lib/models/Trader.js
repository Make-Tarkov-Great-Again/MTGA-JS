...
const { round, readParsed, generateMongoID, getCurrentTimestamp, logger } = require("../utilities");
const { Item } = require("./Item");

const cloneDeep = require(`fast-copy`).default
const { database: { core: { gameplay: { trading: { refreshTimeInMinutes } } } } } = require("../../app");
...
class Trader extends BaseModel {
    ...
    async refreshTrader() {
        this.generateAssort(await getCurrentTimestamp());
    }
    ...
    async generateAssort(currentTime) {
        ...
        if (!this.assort) {
            ...
            if (!this.assort.nextResupply || this.assort.nextResupply <= currentTime) {
                assort = cloneDeep(this.baseAssort);
                assort.nextResupply = currentTime + refreshTimeInMinutes * 60;
    ...
    async generateFilteredAssort(profile) {
        ...
        if (this.isRagfair()) {
            ...
            for (let i = this.assort.items.length - 1; i > 0; i--) {
                const item = this.assort.items[i];
    ...
    async removeItemFromAssortAfterBuy(moveData) {
        ...
        if (foundItem) {
            ...
            await logger.error(`Could not find item to remove ${moveData.item_id} in assort`);
        }
    }
            ...
            if (!["5449016a4bdc2d6f028b456f", "569668774bdc2da2298b4568", "5696686a4bdc2da3298b456a",
                ...
                if (await this.itemInPurchaseCategories(item)) {
                    ...
                    if (currency !== "5449016a4bdc2d6f028b456f") {
                        price = await round(price / priceTable[currency]);
                    }
                    if (price) {
    ...
    async itemInPurchaseCategories(item) {
        const { database: { templates: { Categories, Items } } } = require('../../app');
    ...
    async getRagfairOffers() {
        ...
        const childlessList = await readParsed(`./assets/database/childlessList.json`);
        let offerIntID = 1;
        ...
        for (let i = this.assort.items.length - 1; i > 0; i--) {
            ...
            if (item.slotId === "hideout") {
                ...
                const fullItem = {};
                fullItem.item = childlessList.includes(item._id)
                    ? item
                    : await Item.findAndReturnChildrenAsItems(item._id, this.assort.items);
    ...
    static async convertAssortMongoID(traderAssort) {
        ...
        const cached = traderAssort.items.length;
        for (let c = cached - 1; c > 0; c--) {
            const item = traderAssort.items[c];
