...
const { round, readParsed, getAbsolutePathFrom, generateMongoID, getCurrentTimestamp, logger } = require("../utilities");
...
class Trader extends BaseModel {
    ...
    async generateAssort(currentTime) {
        ...
        let assort;
        if (!this.assort) {
            assort = readParsed(getAbsolutePathFrom(`./assets/database/traders/${this.base._id}/assort.json`));
            if (assort.data) {
                assort = assort.data;
            }
