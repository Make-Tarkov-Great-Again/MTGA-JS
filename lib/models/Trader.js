...
class Trader extends BaseModel {
    ...
    async getFilteredAssort(profile) {
        ...
        if (!this.nextResupply || currentTime >= this.nextResupply) {
            const nextResupplly = currentTime + database.core.gameplay.trading.refreshTimeInMinutes * 60;
            output.nextResupply = nextResupplly;
            this.nextResupply = nextResupplly;
