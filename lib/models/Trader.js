...
const { round, readParsedAsync, generateMongoID, getCurrentTimestamp, logger } = require("../utilities");
...
class Trader extends BaseModel {
    ...
    async generateAssort(currentTime) {
        ...
        let assort;
        if (!this.assort) {
            assort = await readParsedAsync(`./assets/database/traders/${this.base._id}/assort.json`);
            if (assort.data) {
                assort = assort.data;
            }
    ...
    async getRagfairOffers() {
        ...
        const childlessList = await readParsedAsync(`./childlessList.json`);
        let offerIntID = 1;
        for (const item of this.assort.items) {
