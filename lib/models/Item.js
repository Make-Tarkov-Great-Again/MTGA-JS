...
const { logger, generateMongoID, readParsed, round } = require("../utilities");
    ...
    */
    static async findAndReturnChildrenAsItems(itemId, items) {
        ...
        for (const item of items) {
            ...
            if (item.parentId === itemId && !output.find(item => item._id === item._id)) {
                output.push(...await this.findAndReturnChildrenAsItems(item._id, items));
            }
        }
    ...
    static async findAndReturnChildrenAsIds(itemId, items) {
        ...
        for (const item of items) {
            if (item.parentId === itemId) {
                output.push(...await this.findAndReturnChildrenAsIds(item._id, items));
            }
        }
