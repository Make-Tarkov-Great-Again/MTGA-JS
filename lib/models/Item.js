...
class Item extends BaseModel {
    ...
    async isBlacklisted() {
        ...
        if ( await this.isNode() || await this.isBuyable() || await this.isQuestItem()) {
            return true;
        } else if (!this._props.CanSellOnRagfair && !database.core.gameplay.flea.removeBlacklist) {
            return true;
        } else {
            return false;
        }
    ...
    static async generateFormatedNodes(itemsNodes) {
        let itemsNodesCopy = JSON.parse(JSON.stringify(itemsNodes));
        itemsNodesCopy = itemsNodesCopy.splice(1);
        let formatedNodes = {};
        formatedNodes = await Item.getChildrenNodes(itemsNodes[0]._id, itemsNodesCopy, formatedNodes);
        const items = await Item.getAllWithoutKeys();
        const filteredItems = [];
        for (const item of items) {
            if (!await item.isNode()) {
        ...
        formatedNodes = await Item.populateItemsNodes(formatedNodes, filteredItems);
        return formatedNodes;
    }

    static async getChildrenNodes(parentNode, itemsNodes) {
        // retrieve all children nodes of the parent node
        const childrenNodes = [];
        for (const item of itemsNodes) {
            if (item._parent === parentNode) {
                childrenNodes.push({"name": item._name, "id": item._id, "children": []});
            }
        }
        if (childrenNodes.length > 0) {
            for (const childNode of childrenNodes) {
                childNode.children = await Item.getChildrenNodes(childNode.id, itemsNodes);
            }
        }
        return childrenNodes;
    }

    static async populateItemsNodes(formatedNodes, items) {
        if (Array.isArray(formatedNodes)) {
            // retrieve all items of the nodes
            for (const node of formatedNodes) {
                if (node.children.length > 0) {
                    // retrieve all items of the children nodes
                    for (const childNode of node.children) {
                        await Item.populateItemsNodes(childNode, items);
                    }
                } else {
                    for (const item of items) {
                        if (item._parent === node.id) {
            ...
            if (formatedNodes.children.length > 0) {
                // retrieve all items of the children nodes
                for (const childNode of formatedNodes.children) {
                    await Item.populateItemsNodes(childNode, items);
                }
            } else {
                for (const item of items) {
                    if (item._parent === formatedNodes.id) {
                        formatedNodes.children.push(item);
                    }
                }
            }
        }
