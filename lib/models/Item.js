...
class Item extends BaseModel {
     ...
     */
    async isBlacklisted() {
        const { database: { core: { gameplay: { flea: { removeBlacklist } } } } } = require("../../app");
    ...
    static generatePriceTable(templatesItems) {
        const priceTable = {};
    ...
    static async createFreshBaseItemUpd(item) {
        ...
        switch (item._parent) {
            ...
            default:
                logger.error(`Unable to create fresh UPD from parent [${item._parent}] for item [${item._tpl}]`);
                return "error";
        }
    }
     ...
     */
    async generateCompatibleMagazineList() {
        if (this._props?.ReloadMode && this._props.ReloadMode === "ExternalMagazine") {
            const magazineSlot = this._props.Slots.find(slot => slot._name === "mod_magazine");
            return magazineSlot._props.filters[0].Filter;
        }
        return false;
    }
