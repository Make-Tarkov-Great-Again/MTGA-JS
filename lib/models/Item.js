...
const { logger, generateMongoID, readParsed } = require("../utilities");
...
class Item extends BaseModel {
    ...
    */
    static async findAndReturnChildrenAsItems(itemId, items) {
        ...
        for (let i = items.length - 1; i > 0; i--) {
            if (items[i]._id === itemId) {
                output.unshift(items[i]);
                continue;
            }

            if (items[i].parentId === itemId && !output.find(item => items[i]._id === item._id)) {
                output.push(...await this.findAndReturnChildrenAsItems(items[i]._id, items));
            }
        }
    ...
    static async findAndReturnChildrenAsIds(itemId, items) {
        ...
        for (let i = items.length - 1; i > 0; i--) {
            if (items[i].parentId === itemId) {
                output.push(...await this.findAndReturnChildrenAsIds(items[i]._id, items));
            }
        }
    ...
    async getAllChildItemsInInventory(items) {
        if (!items) {
            return false;
        }
        ...
        for (let i = items.length - 1; i > 0; i--) { // Loop through inventory items to create the parentReference object
        ...
        if (typeof parentReference[this._id] == 'undefined') { // The input parentId has no children in the items
            ...
            for (let c = returnArray.length - 1; c > 0; c--) {
                if (typeof parentReference[returnArray[c]._id] != 'undefined') { // If this child ID has children defined
                    returnArray.push(...parentReference[returnArray[c]._id]); // Add this ID's children to the returnArray, which will also be checked by further iterations of this loop
                }
            }
        ...
        for (const grid in containerMap) {
            if (containerMap[grid].width == itemWidth && containerMap[grid].height == itemHeight) { // Check slots that are the exact size we need, horizontally
                if (containerMap[grid][0][0] != null) {
                    //await logger.debug(`Exact horizontal: Occupied G${grid} R${0} C${0}`);
                    continue;
                }
                // If this loop hasn't been continued then this slot is empty. Place the item here
                //await logger.debug(`Exact horizontal: Free slot found at (x${itemWidth} y${itemHeight}) at G${grid} R${0} C${0}`);
            ...
            if (!freeSlot) {
                if (containerMap[grid].height == itemWidth && containerMap[grid].width == itemHeight) { // Check slots that are the exact size we need, vertical
                    if (containerMap[grid][0][0] != null) {
                        //await logger.debug(`Exact vertical: Occupied G${grid} R${0} C${0}`);
                        continue;
                    }
                    // If this loop hasn't been continued then this slot is empty. Place the item here
                    //await logger.debug(`Exact vertical: Free slot found at (x${itemWidth} y${itemHeight}) at G${grid} R${0} C${0}`);
                            ...
                            for (let searchRow = row; searchRow < (row + itemHeight); searchRow++) { // Search the surrounding squares in the shape of this item
                                for (let searchColumn = column; searchColumn < (column + itemWidth); searchColumn++) {
                                    if (containerMap[grid][searchRow][searchColumn] != null) {
                                        //await logger.debug(`Larger horizontal: Occupied G${grid} R${searchRow} C${searchColumn}`);
                                        continue columnSearch; // Search the next column to the right
                                    }
                                }
                            }
                            //await logger.debug(`Larger horizontal: Free slot found at (${itemHeight}x${itemWidth}) at G${grid} R${row} C${column}`);
                        ...
                        columnSearch: for (let column = 0; column <= (containerMap[grid].width - itemHeight); column++) {
                            for (let searchRow = row; searchRow < (row + itemWidth); searchRow++) { // Search the surrounding squares in the shape of this item
                                for (let searchColumn = column; searchColumn < (column + itemHeight); searchColumn++) {
                                    if (containerMap[grid][searchRow][searchColumn] != null) {
                                        //await logger.debug(`Larger vertical: Occupied G${grid} R${searchRow} C${searchColumn}`);
                                        continue columnSearch; // Search the next column to the right
                                    }
                                }
                            }
                            //await logger.debug(`Larger vetical: Free slot found at (${itemHeight}x${itemWidth}) at G${grid} R${row} C${column}`);
            ...
            for (let row = item.location.y; row < (item.location.y + itemSize.height); row++) { // Iterate item height in relation to inventory squares
                for (let column = item.location.x; column < (item.location.x + itemSize.width); column++) { // Iterate item width in relation to inventory squares
                    if (typeof containerMap[item.slotId][row][column] !== "undefined") { // If the grid square this item occupies exists in the container
                        //await logger.debug(`Read inventory: Item ${item._id} (${itemSize.height}x${itemSize.width}) at ${item.parentId} slotId: ${item.slotId} R${row} C${column}`);
                        containerMap[item.slotId][row][column] = item._id; // Mark this grid square as occupied by this item
                    } else {
                        await logger.debug(item);
                        await logger.error(`Inventory item occupies invalid slot: _id: ${item._id} _tpl: ${item._tpl} parentId: ${item.parentId} slotId: ${item.slotId} y: ${row} x: ${column} - width: ${itemSize.width} height: ${itemSize.height}`);
                        return [];
                    }
                }
            }
    ...
    static async createFreshBaseItemUpd(item) {
        switch (item._parent) {
            ...
            default:
                await logger.error(`Unable to create fresh UPD from parent [${item._parent}] for item [${item._id}]`);
                return "error";
        }
    }
