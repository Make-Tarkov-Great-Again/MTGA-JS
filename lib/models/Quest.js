...
const { logger, getCurrentTimestamp, round, generateMongoID, cloneDeep } = require("../utilities");
...
class Quest extends BaseModel {
    ...
    static async generateQuestModel(quest) {
        const { UtilityModel: { createModelFromParse } } = require("./UtilityModel");
        return createModelFromParse("Quest", quest);
    }
    ...
    static async getQuestsForPlayer(playerProfile) {
        ...
        for (const quest of quests) {
            ...
            if (await this.questForOtherFaction(playerProfile.character.Side, quest._id)) continue;

            if (playerProfile.character.Quests.find(q => q.qid === quest._id)) {
                output.push(quest)
                continue;
            }
            ...
            for (const condition of questRequirements) {
                ...
                if (previousQuest.status === questStatus[condition._props.status[0]]) {
    ...
    static async getQuestStatusIntByName(statusInt) {
        return questStatus.indexOf(statusInt);
    }
    ...
    static async evaluateLevel(character, condition) {
        ...
        if (condition._parent === "Level") {
            if (condition._props.compareMethod == ">=") return level >= condition._props.value;
                ...
                await logger.debug(`Unrecognised Comparison Method: ${condition._props.compareMethod}`);
                return false;
            }
        }
    }
