...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            if (requiredSlots["mod_sight_rear"]) { //can make roll
                ...
                const mod_sight_rear = await this.generateRearSight(rearSight, firecontrol._id, conflictIds);
            ...
            const mod_receiver = await this.generateReciever(reciever, firecontrol._id, conflictIds);
            ...
            if (requiredSlots["mod_gas_block"]) {
                ...
                const mod_gas_block = await this.generateGasBlock(gasblock, firecontrol._id, conflictIds);
            ...
            if (requiredSlots["mod_handguard"]) {
                // remove conflictIds from requiredSlots list
                requiredSlots["mod_handguard"].filter(mod => !conflictIds.includes(mod));
                ...
                const mod_handguard = await this.generateHandguard(handguard, firecontrol._id, conflictIds);
                assembledWeapon.push(mod_handguard);
            }

            if (requiredSlots.includes("mod_pistol_grip")){
     ...
     * @returns {<Promise>{}}
     */
    static async generateReciever(reciever, parentId, conflictIds) {
...
class BotUtilities {
    ...
    static async generateListOfRequiredParts(weaponProps) {
        ...
        slots.find(slot => {
            ...
            if (["mod_reciever", "mod_sight_rear", "mod_stock", "mod_muzzle"].includes(slot._name)) {
                output[slot._name] = slot._props.filters[0].Filter;
            }
        })
        return output;
    }
