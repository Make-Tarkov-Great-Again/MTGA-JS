...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            const requiredSlots = await BotUtilities.generateListOfRequiredParts(weaponProps);
            ...
            // mod_reciever
            requiredSlots["mod_reciever"].filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
            ...
            if (requiredSlots["mod_gas_block"]) {
                requiredSlots["mod_gas_block"].filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
            ...
            if (requiredSlots["mod_handguard"]) {
                requiredSlots["mod_handguard"].filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
            ...
            // mod_pistol_grip
            requiredSlots["mod_pistol_grip"].filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
    ...
    * @returns {<Promise>{}}
    */
    static async generateHandguard(pistolGrip, slotId, parentId, conflictIds) {
...
class BotUtilities {
    ...
    static async generateListOfRequiredParts(weaponProps) {
        const slots = weaponProps.Slots;
        const output = {};
