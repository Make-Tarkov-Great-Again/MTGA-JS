...
class BotItemGeneration {
    ...
    static async generateBotEarpiece(parentId, role, conflictIds, earpieceChance) {
        ...
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() <= earpieceChance) {
            ...
            const earpiece = await this.generateBotInventoryItem(
                earpieceTemplate,
                "Earpiece",
                parentId
            );
    ...
    static async generateBotFacecover(parentId, role, conflictIds, facecoverChance) {
        const facecovers = database.bot.bots[role].gearEntries.facecover.filter(item => !conflictIds.includes(item));
        if (facecovers && facecovers.length > 0 && await getRandomInt() <= facecoverChance) {
            ...
            const facecover = await this.generateBotInventoryItem(
                facecoverTemplate,
                "FaceCover",
                parentId
            );
    ...
    static async generateBotHeadwear(parentId, role, headwearChance) {
        ...
        if (headwears && headwears.length > 0 && await getRandomInt() <= headwearChance) {
            ...
            const headwear = await this.generateBotInventoryItem(
                headwearTemplate,
                "Headwear",
                parentId
            );
    ...
    static async generateBotBodyArmor(parentId, role) {
        ...
        if (gearEntries && gearEntries.length > 0) {
            ...
            const armor = await this.generateBotInventoryItem(
                armorTemplate,
                "ArmorVest",
                parentId
            );
    ...
    static async generateBotVest(parentId, role) {
        ...
        if (gearEntries && gearEntries.length > 0) {
            ...
            const botVest = await this.generateBotInventoryItem(
                vestTemplate,
                "TacticalVest",
                parentId
            );
    ...
    static async generateBotMelee(parentId, role) {
        ...
        const scabbardItem = await this.generateBotInventoryItem(
            scabbard,
            "Scabbard",
            parentId
        );
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        const weapon = await Item.get(await getRandomFromArray(weaponPool));

        const assembledWeapon = [];
        if (weapon._props) {

            const requiredSlots = await BotUtilities.generateListOfRequiredForWeaponPart(weapon);
            ...
            const firecontrol = await this.generateBotInventoryItem(weapon, slotId, parentId);
            ...
            if (weapon._props.Chambers.length > 0) {
                ...
                const ammo = await this.generateBotInventoryItemChoices(ammos);
                ...
                if (weapon._props.ReloadMode === "OnlyBarrel") {
                    ...
                    const magazine = magazines[await this.generateBotInventoryItemChoices(Object.keys(magazines))];
                ...
                switch (weapon._props.ReloadMode) {
                    case "OnlyBarrel":
            ...
            if (requiredSlots?.mod_barrel) {
                requiredSlots.mod_barrel.filter = requiredSlots.mod_barrel.filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list

                const barrel = await this.generateBotInventoryItemChoices(requiredSlots.mod_barrel.filter);
                const mod_barrel = await this.generateBotInventoryItem(
                    barrel,
                    requiredSlots.mod_barrel.slotId,
                    firecontrol._id,
                    conflictIds
                );

                assembledWeapon.push(mod_barrel);
            }

            if (requiredSlots?.mod_charge) {
                const charge = await this.generateBotInventoryItemChoices(requiredSlots.mod_charge.filter);
                const mod_charge = await this.generateBotInventoryItem(
                    charge,
                    requiredSlots.mod_charge.slotId,
                    firecontrol._id,
                    conflictIds
                );

                assembledWeapon.push(mod_charge);
            }
            ...
            if (requiredSlots.mod_gas_block) {
                ...
                if (requiredSlots.mod_gas_block.filter.length > 0) {
                    const gasblock = await this.generateBotInventoryItemChoices(requiredSlots.mod_gas_block.filter);
                    const mod_gas_block = await this.generateGasBlockAndHandguard(
                        gasblock,
                        requiredSlots.mod_gas_block.slotId,
                        firecontrol._id,
                        conflictIds
                    );
            ...
            if (requiredSlots.mod_handguard) {
                requiredSlots.mod_handguard.filter = requiredSlots.mod_handguard.filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list

                const handguard = await this.generateBotInventoryItemChoices(requiredSlots.mod_handguard.filter);
                const mod_handguard = await this.generateBotInventoryItem(
                    handguard,
                    requiredSlots.mod_handguard.slotId,
                    firecontrol._id,
                    conflictIds
                );

                assembledWeapon.push(mod_handguard);
            }


            // mod_sight_rear
            if (requiredSlots.mod_sight_rear) { //can make roll
                const rearSight = await this.generateBotInventoryItemChoices(requiredSlots.mod_sight_rear.filter);
                const mod_sight_rear = await this.generateAdapterAndOrRearSight(
                    rearSight,
                    requiredSlots.mod_sight_rear.slotId,
                    firecontrol._id,
                    conflictIds
                );
            ...
            const mountCheck = requiredSlots?.mod_mount ? await getPercentRandomBool(25) : false;
            ...
            if (requiredSlots.mod_reciever.filter.length > 0) {
                const reciever = await this.generateBotInventoryItemChoices(requiredSlots.mod_reciever.filter);

                // check and roll for mod_sight_rear on reciever
                const mod_receiver = await this.generateReciever(
                    reciever,
                    requiredSlots.mod_reciever.slotId,
                    firecontrol._id,
                    conflictIds,
                    mountCheck ? false : true
                );
            ...
            if (mountCheck) {
                requiredSlots.mod_mount.filter = requiredSlots.mod_mount.filter.filter(mod => !conflictIds.includes(mod));

                const sideRailMount = await this.generateBotInventoryItemChoices(requiredSlots.mod_mount.filter);
                const mod_mount_000 = await this.generateMountAndOrScope(
                    sideRailMount,
                    requiredSlots.mod_mount.slotId,
                    firecontrol._id,
                    conflictIds
                )

                if (mod_mount_000.length !== 0) assembledWeapon.push(...mod_mount_000);
            }
            ...
            // mod_pistol_grip
            const pistolGrip = await this.generateBotInventoryItemChoices(requiredSlots.mod_pistol_grip.filter);
            const mod_pistol_grip = await this.generateBotInventoryItem(
                pistolGrip,
                requiredSlots.mod_pistol_grip.slotId,
                firecontrol._id,
                conflictIds
            );
            ...
            /* const pistolGripSlots = await BotUtilities.generateListOfSlots(await Item.get(reciever));
            if (pistolGripSlots) {
                // do rolls for slots
            ...
            if (requiredSlots.mod_stock) {
                ...
                if (requiredSlots.mod_stock.filter.length > 0) {
                    const stock = await this.generateBotInventoryItemChoices(requiredSlots.mod_stock.filter);
                    const mod_stock = await this.generateAdapterAndOrStock(
                        stock,
                        requiredSlots.mod_stock.slotId,
                        firecontrol._id
                    );
     ...
     * @returns {<Promise> {}}
     */
    static async generateBotInventoryItem(item, slotId, parentId, conflictIds = null) {
     ...
     */
    static async generateBotInventoryItemChoices(filter) {
    ...
    static async generateMagazineInWeapon(magazine, parentId, ammo) {
        ...
        const mod_magazine = await this.generateBotInventoryItem(
            magazine.id,
            "mod_magazine",
            parentId
        );

        const cartridges = await this.generateCartridgesInMagazine(ammo, mod_magazine._id);
        cartridges.upd.StackObjectsCount = magazine.count;

        return [mod_magazine, cartridges];
    }
    ...
    static async generateCartridgesInMagazine(ammo, parentId) {
        const cartridges = await this.generateBotInventoryItem(
            ammo,
            "cartridges",
            parentId
        );
    ...
    /**
     * Generate adapter and/or rear sight for Weapon, 
     * return array of objects and updated conflictIds
     * @param {string} anamoly 
     * @param {string} slotId 
     * @param {string} parentId 
     * @param {[string]} conflictIds 
     * @returns {<Promise>{}}
     */
    static async generateAdapterAndOrRearSight(anamoly, slotId, parentId, conflictIds) {
        const output = [];

        let rearSightTemplate = await Item.get(anamoly);

        const anamolySlots = await BotUtilities.generateListOfSlots(rearSightTemplate);
        if (anamolySlots?.mod_sight_rear) {
            const rearSightAdapterTemplate = rearSightTemplate;
            const mod_adapter = await this.generateBotInventoryItem(
                rearSightAdapterTemplate,
                slotId,
                parentId
            );
            output.push(mod_adapter);

            const anamoly = await this.generateBotInventoryItemChoices(anamolySlots.mod_sight_rear.filter)
            const mod_sight_rear = await this.generateAdapterAndOrRearSight(
                anamoly,
                anamolySlots.mod_sight_rear.slotId,
                mod_adapter._id,
                conflictIds
            );

            output.push(...mod_sight_rear);
            return output;
        }
        else {
            const mod_sight_rear = await this.generateBotInventoryItem(
                rearSightTemplate,
                slotId,
                parentId,
                conflictIds
    ...
    /**
     * Generate mounted optic, or mount and optic for Weapon, 
     * return array of objects and updated conflictIds
     * @param {string} anamoly 
     * @param {string} slotId 
     * @param {string} parentId 
     ...
     * @returns {<Promise>[{}]}
     */
    static async generateMountAndOrScope(anamoly, slotId, parentId, conflictIds) {
        const output = [];
        let mountedOpticTemplate = await Item.get(anamoly);

        if (mountedOpticTemplate._parent === "55818b224bdc2dde698b456f") {
            const opticMountTemplate = mountedOpticTemplate;
            const opticMountSlots = await BotUtilities.generateListOfSlots(opticMountTemplate);

            const mod_mount = await this.generateBotInventoryItem(
                opticMountTemplate,
                slotId,
                parentId,
                conflictIds
            );
            output.push(mod_mount);

            const scope = await this.generateBotInventoryItemChoices(opticMountSlots.mod_scope.filter)
            const mod_scope = await this.generateAdapterAndOrScope(
                scope,
                opticMountSlots.mod_scope.slotId,
                mod_mount._id,
                conflictIds
            );

            if (mod_scope.length < 2) return [];
            output.push(...mod_scope);
            return output;
        } else {
            const mod_mount_000 = await this.generateBotInventoryItem(
                mountedOpticTemplate,
                slotId,
                parentId,
                conflictIds
            );

            output.push(mod_mount_000);
            return output;
        }
    }

    /**
     * Generate adapter and/or scope, return array of objects and updated conflictIds
     * @param {string} anamoly 
     * @param {string} slotId 
     * @param {string} parentId 
     * @param {[]} conflictIds 
     * @returns {<Promise>[{}]}
     */
    static async generateAdapterAndOrScope(anamoly, slotId, parentId, conflictIds) {
        const output = [];
        let scopeTemplate = await Item.get(anamoly);

        if (scopeTemplate._parent === "55818b224bdc2dde698b456f") {
            const adapterTemplate = scopeTemplate;
            let adapterSlots = await BotUtilities.generateListOfSlots(adapterTemplate);
             ...
             * "mod_scope_000 - _002" 
             * Should shuffle through keys, maybe... we'll see
             */

            const adapterSlot = await getRandomFromObject(adapterSlots);
            const weightedList = adapterSlot
                ? await BotUtilities.generateWeightedList(adapterSlot.filter)
                : logger.info("fix it fool");
            const choices = await BotUtilities.generateGearEntries(weightedList);

            const scope = await getRandomFromArray(choices);
            const mod_scope = await this.generateBotInventoryItem(
                scope,
                adapterSlot.slotId,
                mod_adapter._id,
                conflictIds
    ...
    /**
     * Generate upper reciever/dustcover for weapon,
     * include required parts, as well as rolls for optional parts
     * return object and updated conflictIds
     * @param {string} reciever
     * @param {string} slotId
     * @param {string} parentId 
     * @param {[string]} conflictIds 
     * @param {bool} mountedSightChance
     * @returns {<Promise>{}}
     */
    static async generateReciever(reciever, slotId, parentId, conflictIds, mountedSightChance) {
        const output = [];

        const recieverTemplate = await Item.get(reciever);
        const recieverSlots = await BotUtilities.generateListOfSlots(recieverTemplate);

        const mod_receiver = await this.generateBotInventoryItem(
            recieverTemplate,
            slotId,
            parentId,
            conflictIds
        )
        output.push(mod_receiver);

        if (recieverSlots?.mod_sight_rear) {
            const anamoly = await this.generateBotInventoryItemChoices(recieverSlots.mod_sight_rear.filter)
            const mod_sight_rear = await this.generateAdapterAndOrRearSight(
                anamoly,
                recieverSlots.mod_sight_rear.slotId,
                mod_receiver._id,
                conflictIds
            )
        ...
        if (recieverSlots?.mod_barrel) {
            const barrel = await this.generateBotInventoryItemChoices(recieverSlots.mod_barrel.filter);
            const mod_barrel = await this.generateBotInventoryItem(
                barrel,
                recieverSlots.mod_barrel.slotId,
                mod_receiver._id,
                conflictIds
            );
            output.push(mod_barrel);

            const barrelSlots = await BotUtilities.generateListOfRequiredForWeaponPart(barrel);
            if (barrelSlots?.mod_gas_block) {
                const gasblock = await this.generateBotInventoryItemChoices(barrelSlots.mod_gas_block.filter)
                const gasblockTemplate = await Item.get(gasblock);
                const mod_gas_block = await this.generateBotInventoryItem(
                    gasblockTemplate,
                    barrelSlots.mod_gas_block.slotId,
                    mod_barrel._id,
                    conflictIds
                );
        ...
        if (recieverSlots?.mod_handguard) {
            recieverSlots.mod_handguard.filter = recieverSlots.mod_handguard.filter.filter(item =>
                !conflictIds.includes(item));
            const handguard = await this.generateBotInventoryItemChoices(recieverSlots.mod_handguard.filter);
            const mod_handguard = await this.generateBotInventoryItem(
                handguard,
                recieverSlots.mod_handguard.slotId,
                mod_receiver._id,
                conflictIds
            );

            const handguardSlots = await BotUtilities.generateListOfSlots(handguard);
            if (handguardSlots?.mod_sight_front) {
                const frontSight = await this.generateBotInventoryItemChoices(handguardSlots.mod_sight_front.filter);
                const mod_sight_front = await this.generateBotInventoryItem(
                    frontSight,
                    handguardSlots.mod_sight_front.slotId,
                    mod_handguard._id,
                );

                output.push(mod_sight_front)
            }

            output.push(mod_handguard);
        }

        if (mountedSightChance && await getPercentRandomBool(await getRandomInt()) && recieverSlots?.mod_scope) {
            recieverSlots.mod_scope.filter = recieverSlots.mod_scope.filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
            if (recieverSlots.mod_scope.filter.length > 0) {

                const anomaly = await this.generateBotInventoryItemChoices(recieverSlots.mod_scope.filter)
                const mod_scope = await this.generateAdapterAndOrScope(
                    anomaly,
                    recieverSlots.mod_scope.slotId,
                    mod_receiver._id,
                    conflictIds
                )
                output.push(...mod_scope);
    ...
    static async generateGasBlockAndHandguard(gasblock, slotId, parentId, conflictIds) {
        ...
        output.push(mod_gas_block);
        const gasblockSlots = await BotUtilities.generateListOfSlots(gasblockTemplate);

        if (gasblockSlots.mod_handguard) {
            const handguard = await this.generateBotInventoryItemChoices(gasblockSlots.mod_handguard.filter);
            const mod_handguard = await this.generateBotInventoryItem(
                handguard,
                gasblockSlots.mod_handguard.slotId,
                mod_gas_block._id,
                conflictIds
            );
    ...
    static async generateAdapterAndOrStock(anomaly, slotId, parentId) {
        ...
        if (await BotUtilities.isBufferTubeOrAdapter(stockTemplate._id)) {
            ...
            const adapterSlots = await BotUtilities.generateListOfRequiredForWeaponPart(adapterTemplate);
            const stock = await this.generateBotInventoryItemChoices(adapterSlots.mod_stock.filter);

            const mod_stock = await this.generateBotInventoryItem(
                stock,
                adapterSlots.mod_stock.slotId,
                mod_adapter._id,
            );
    ...
    static async generateBotPockets(parentId, role) {
        ...
        const pockets = await this.generateBotInventoryItem(
            pocket,
            "Pockets",
            parentId
        );
    ...
    static async generateBotBackpack(parentId, role, backpackChance) {
        ...
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() >= backpackChance) {
            ...
            const backpack = await this.generateBotInventoryItem(
                backpackTemplate,
                "Backpack",
                parentId
            );
    ...
    static async generateSecuredContainer(parentId) {
        ...
        const securedContainer = await this.generateBotInventoryItem(
            securedContainerTemplate,
            "SecuredContainer",
            parentId
        );
...
class BotUtilities {
    ...
    static async generateListOfRequiredForWeaponPart(anamoly) {
        ...
        let name;
        item._props.Slots.find(slot => {
            if (slot._required === true) {
                if (slot._name.includes("mod_pistol_grip")) name = "mod_pistol_grip"
                else if (slot._name.includes("mod_stock")) name = "mod_stock"
            ...
            // these are required for proper bot generation
                ...
                const check = ["mod_reciever", "mod_sight_rear", "mod_stock", "mod_muzzle", "mod_mount"].some(
                    condition => slot._name.includes(condition));
                if (check) {
                    if (slot._name.includes("mod_stock")) name = "mod_stock"
                    else if (slot._name.includes("mod_mount")) name = "mod_mount"
                    else name = slot._name;
    ...
    static async generateListOfSlots(anamoly) {
        const item = anamoly?._props ? anamoly : await Item.get(anamoly);
        if (!item?._props?.Slots) return false;
