...
class Bot extends BaseModel {
    ...
    static async generateBots(request = null, reply = null, preload = null) {
        ...
        if (database.core.gameplay.development.debugBots)
            await writeFile("./botweaponcache.json", stringify(database.bot.weaponCache));
...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId, ammos = [], magazines = {}) {
        if (!database.bot.weaponCache)
            database.bot.weaponCache = {};

        let gearEntries = []
        gearEntries = database.bot.bots[role].gearEntries;
        ...
        if (!database.bot.weaponCache[choice]) {
            database.bot.weaponCache[choice] = {};
        }
        ...
        if (!database.bot.weaponCache[choice]["parts"]) {
            const { required, optional } = await BotUtilities.generatePartList(weapon);

            database.bot.weaponCache[choice]["parts"] = {};
            database.bot.weaponCache[choice].parts["required"] = required;
            database.bot.weaponCache[choice].parts["optional"] = optional;
        }

        const [required, optional] = [
            database.bot.weaponCache[choice].parts.required,
            database.bot.weaponCache[choice].parts.optional
        ];
        ...
        if (!database.bot.weaponCache[choice]["ammos"]) {
            database.bot.weaponCache[choice]["ammos"] = await weapon.generateCompatibleAmmoList();
        }

        ammos = database.bot.weaponCache[choice].ammos;
        ...
        if (weapon._props.ReloadMode !== "OnlyBarrel") {
            if (!database.bot.weaponCache[choice]["magazines"]) {
                database.bot.weaponCache[choice]["magazines"] = await weapon.generateCompatibleMagazineList(true);
            }
            magazines = database.bot.weaponCache[choice].magazines;
