...
class Bot extends BaseModel {
    ...
    async generatePocketLoot(parentId, salewa, core_medical_surgical_kit, looseAmmo){
        const pocketLoot = {
             "salewa":{
             ...
             "core_medical_surgical_kit":{
                 "_id": await generateMongoID(),
                 "_tpl": core_medical_surgical_kit._id,
                 "_parentId": parentId,
                 "slotId": "Medical Supplies"
             },
             "conflicts": [],
        ...
        foreach(pocketLoot)
        {
            const random = await getRandomFromArray(pocketLoot);
            if(random.pocketLoot.salewa){
                output.push(salewa);
            }
            if(random.pocketLoot.core_medical_surgical_kit){
                output.push(core_medical_surgical_kit);
            }
            if(random.pocketLoot.looseAmmo){
