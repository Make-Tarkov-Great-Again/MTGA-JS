        ...
        switch (true) {
            case Object.keys(required).length > 0:
                ...
                for (const id in required) {
                    ...
                    let requiredPart;
                    if (["mod_sight_rear", "mod_sight_front"].some(condition => require.slotId.includes(condition))) {
                        const { _props: { Slots } } = await Item.get(part);
                        if (Slots.length === 0 && await getPercentRandomBool(50)) {
                            requiredPart = await this.generateWeaponPart(
                                part,
                                require.slotId,
                                parent._id,
                                conflictIds,
                    ...
                    if (requiredPart) requires.push(...requiredPart);
                }
