...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        let chambers;
        let patron_in_weapon;
        let magazines; // useful for shuffling magazines later that are stored
        ...
        switch (weapon._props.ReloadMode) {
            case "OnlyBarrel":
                if (weapon._props.Chambers.length > 0) {
                    chambers = await weapon.generateChambersList();
                    patron_in_weapon = await this.generatePatronInWeapon(chambers, firecontrol._id, ammo);
                    ...
                    const mod_magazine = await this.createWeaponPart(magazine, "mod_magazine", firecontrol._id);
                ...
                else if (weapon_name.includes("rsp30")) {
                    logger.info(`This is a flare, remove from gearEntries.${slotId}`);
                    break;
                }
                break;
    ...
    static async generateWeaponPart(partId, slotId, parentId, conflictIds, filledAttachments) {
        const output = [];

        const partTemplate = await Item.get(partId);
        const { required, optional } = await BotUtilities.generatePartList(partTemplate);
        ...
        switch (true) {
            case Object.keys(required).length > 0:
                ...
                for (const id in required) {
                    ...
                    const part = filtered.length === 1 ? filtered[0] : await this.selectWeaponPart(filtered);
                    const requiredPart = await this.generateWeaponPart(
                        part,
                        require.slotId,
                        parent._id,
                        conflictIds,
            ...
            case Object.keys(optional).length > 0:
                ...
                for (const id in optional) {
                    ...
                    /**
                     * Make check for all the below mentioned to reduce stacking of optional components
                     * If reciever has `mod_scope` slot, ignore `mod_mount` if roll success, etc
                     */
                    if (["mod_mount", "mod_foregrip", "mod_scope", "mod_tactical", "mod_muzzle"].some(condition => id.includes(condition))) {
                        if (filledAttachments.includes(id)) break;
                        if (!await getPercentRandomBool(50)) break;
                    };
                    ...
                    const part = filtered.length === 1 ? filtered[0] : await this.selectWeaponPart(filtered);
                    const optionalPart = await this.generateWeaponPart(
                        part,
                        option.slotId,
                        parent._id,
                        conflictIds,
     ...
     * @returns {<Promise>{}}
     */
    static async generateMagazineInWeapon(magazine, parentId) {
