...
class Bot extends BaseModel {
    ...
    async generateNickname(role) {
        //TODO: bigpipe and his other gay boyfriends nickname and use a nameDB
        if (["usec", "exusec", "pmcbot", "bear"].includes(role)) {
            return database.core.botNames.followergluhar[await generateRandomInt(0, database.core.botNames.followergluhar.length)];
        } else if (["followergluharsecurity", "followergluharsnipe", "followergluharscout", "followergluharassault"].includes(role)) {
            return database.core.botNames.followergluhar[await generateRandomInt(0, database.core.botNames.followergluhar.length)];
        } else if (["marksman", "playerscav", "cursedassault", "assaultgroup", "assault"].includes(role)) {
