...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        ...
        //const weapons = await this.generateWeapons(parentId, role);
        const weapon = await this.createWeaponForSlot(parentId, role);
        ...
        const cartridge = await this.generateCartridge(weapon[0], ammo, role);
        ...
        return [...weapon, patronInWeapon, ...cartridge, scabbard, ...head, ...body];
    }
    ...
    async generateBodyAccessory(parentId, cartridge, role) {
        const output = []; // array of body accessories
        const conflicts = []; // array of item ids that conflict with the current item

        const securedContainer = await this.generateSecuredContainer(parentId, cartridge, role);
        ...
        if (database.bot.bots[role].loadout) {
            ...
            // TODO: handle armor x tactical vest conflict for loadout, probably need to define before hand if the AI will have a armor, if he does we remove the armored vest from the available picks
            // TODO: handle backpacks
    ...
    async generateSecuredContainer(parentId, cartridges, role) {
    ...
    async generateWeapons(parentId, role) {
        const output = [];

        const slotId = "FirstPrimaryWeapon";

        if (slotId) { //weapon[0] is the primary weapon, for now (create for loop later)
            const weapon = await this.createWeaponForSlot(parentId, role, slotId);
            const ammo = await this.generateAmmo(weapon[0], role);
            const patron_in_weapon = await this.generatePatronInWeapon(weapon[0], ammo, role);
            const cartridge = await this.generateCartridge(weapon[0], ammo, role);

            output.push(weapon, ammo, patron_in_weapon, ...cartridge);
        }
        return output;
    }
    ...
    async generateAmmo(gun, role) {
        if (!gun._props) gun = await Item.get(gun._tpl);

        const caliber = await gun.getProperty("ammoCaliber");
        const ammo = await BotUtilities.generateLootBucket("Ammo", role, caliber);

        return getRandomFromArray(ammo);
    }
