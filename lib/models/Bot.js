...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        const output = [];
        output.push(await this.generateScabbard(parentId, role));
        ...
        output.push(...await BotItemGeneration.generateBotAmmo(botRig, securedContainer, compatibleMagazines, compatibleAmmo));
...
class BotItemGeneration {
     ...
     * @param {string} role allowed items for this role
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (!gearEntries.primaryWeapon) {
            logger.error("[BotItemGeneration.generateBotGear.generateBotWeapon] Bot loadout doesn't have any available primaryWeapon.");
            return false;
        }
    ...
    static async generateBotHeadwear(parentId, role) {
        ...
        logger.error("[BotItemGeneration.generateBotGear.generateBotHeadwear] Bot loadout doesn't have any available headwear.");
        return false;
    }
    ...
    static async generateBotRig(parentId, role) {
        ...
        if (gearEntries.armoredVest.length > 0 && gearEntries.vest.length > 0) {
            ...
            logger.error("[BotItemGeneration.generateBotGear.generateBotRig] Bot loadout doesn't have any available vest and amored vest.");
            return false;
        }
