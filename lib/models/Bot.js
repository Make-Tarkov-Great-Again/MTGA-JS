...
class BotItemGeneration {
    ...
    static async generateBotEarpiece(parentId, role, conflictIds, earpieceChance) {
        ...
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() <= earpieceChance) {
            ...
            const earpiece = await this.generateWeaponPart(
                earpieceTemplate,
                "Earpiece",
                parentId
            );
    ...
    static async generateBotFacecover(parentId, role, conflictIds, facecoverChance) {
        ...
        if (facecovers && facecovers.length > 0 && await getRandomInt() <= facecoverChance) {
            ...
            const facecover = await this.generateWeaponPart(
                facecoverTemplate,
                "FaceCover",
                parentId
            );
    ...
    static async generateBotHeadwear(parentId, role, headwearChance) {
        ...
        if (headwears && headwears.length > 0 && await getRandomInt() <= headwearChance) {
            ...
            const headwear = await this.generateWeaponPart(
                headwearTemplate,
                "Headwear",
                parentId
            );
    ...
    static async generateBotBodyArmor(parentId, role) {
        ...
        if (gearEntries && gearEntries.length > 0) {
            ...
            const armor = await this.generateWeaponPart(
                armorTemplate,
                "ArmorVest",
                parentId
            );
    ...
    static async generateBotVest(parentId, role) {
        ...
        if (gearEntries && gearEntries.length > 0) {
            ...
            const botVest = await this.generateWeaponPart(
                vestTemplate,
                "TacticalVest",
                parentId
            );
    ...
    static async generateBotMelee(parentId, role) {
        ...
        const scabbardItem = await this.generateWeaponPart(
            scabbard,
            "Scabbard",
            parentId
        );
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            for (const slot in requiredSlots.required) {
                const required = requiredSlots.required[slot];

                if (required.filter.length === 0) continue;
                ...
                const mod_part = await this.createWeapon(
                    part,
                    required.slotId,
                    firecontrol._id,
                    conflictIds
     ...
     * @returns {<Promise> {}}
     */
    static async generateWeaponPart(item, slotId, parentId, conflictIds) {
    ...
    static async createWeapon(partId, slotId, parentId, conflictIds) {
        ...
        if (Object.keys(required).length > 0) {
            ...
            const parent = await this.generateWeaponPart(
                partTemplate,
                slotId,
                parentId,
                conflictIds
            ...
            for (const id in required) {
                ...
                mod_part = await this.createWeapon(
                    part,
                    require.slotId,
                    parent._id,
                    conflictIds
    ...
    static async generateMagazineInWeapon(magazine, parentId, ammo) {
        ...
        const mod_magazine = await this.generateWeaponPart(
            magazine.id,
            "mod_magazine",
            parentId
        );
    ...
    static async generateCartridgesInMagazine(ammo, parentId) {
        const cartridges = await this.generateWeaponPart(
            ammo,
            "cartridges",
            parentId
        );
    ...
    static async generateBotPockets(parentId, role) {
        ...
        const pockets = await this.generateWeaponPart(
            pocket,
            "Pockets",
            parentId
        );
    ...
    static async generateBotBackpack(parentId, role, backpackChance) {
        ...
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() >= backpackChance) {
            ...
            const backpack = await this.generateWeaponPart(
                backpackTemplate,
                "Backpack",
                parentId
            );
    ...
    static async generateSecuredContainer(parentId) {
        ...
        const securedContainer = await this.generateWeaponPart(
    ...
    static async generatePartList(anamoly) {
        ...
        item._props.Slots.find(slot => {
            ...
            // these are required for proper bot generation
                ...
                const check = ["mod_reciever", "mod_sight_rear", "mod_stock", "mod_mount"].some(
        ...
        const required = ["mod_reciever", "mod_barrel", "mod_gas_block", "mod_charge", "mod_handguard", "mod_pistol_grip", "mod_stock"];
