...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        ...
        // TODO (DONE): generate loot in pockets ? Loose bullets ? Money ? Bandages ? Grenades ?

        const [looseAmmo, money, bandages, grenades] = await BotItemGeneration.generateBotPockets(parentId, role);
        if(looseAmmo){
        ...
        if(bandages){
            output.push(bandages);
        }

        if(grenades){
            output.push(grenades);
        }
