...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        const output = [];
        let generatedMagazines;
        output.push(await this.generateScabbard(parentId));
...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role) {
        ...
        const weaponParts = await BotItemGeneration.generateWeaponParts(weaponTemplate, parentId);
        console.log()
    ...
    static async generateWeaponParts(weaponTemplate, parentId) {
        ...
        if (upd !== "error")
            receiver.upd = upd;
        const fullWeapon = [receiver];
        const requiredSlots = weaponTemplate._props.Slots.filter(slot => slot._required && slot._name !== 'mod_reciever');
        const conflictsId = [];
        for (const slot of requiredSlots) {
            const validMods = slot._props.filters[0].Filter.filter(mod => !conflictsId.includes(mod));
