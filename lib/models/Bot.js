...
class Bot extends BaseModel {
    ...
    async generateVoice(role = null) {
        if (role && database.bot.bots[role].appearance) {
            return database.bot.bots[role].appearance;
        } else {
            logger.logError(`[Bot : generateInfo] Role [${role}] appearance not handled`);
            return await BotUtilities.randomProperty(database.bot.appearance.random.Voice);
        }
    }
    ...
    async generateCustomization(role) {
        ...
        } else if (templateCustomization.Head === "5d5f8ba486f77431254e7fd2") {
            templateCustomization.Body = "609e86926e8078716f600883"
        }
    ...
    async generatePockets(parentId, role = null) {
        ...
        return {
            "_id": await generateMongoID(),
            "_tpl": pockets[await generateRandomInt(0, pockets.length - 1)],
            "parentId": parentId,
            "slotId": "Pockets"
        }
