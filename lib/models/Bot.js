...
class Bot extends BaseModel {
    ...
    async generateLootInPockets(pocketContainer, parentId = this.generateInventoryItems.parentId)
    {
        const output = [];
        ...
        const newItemTemplate = await Item.get(await getRandomFromArray(ItemEntries));
        const itemSize = await newItemTemplate.getSize();
        const newItem = await newItemTemplate.createAsNewItem();
        const upd = await Item.createFreshBaseItemUpd(newItemTemplate);
        const freeSlot = await Item.getFreeSlot(pocketContainer, output, itemSize.width, itemSize.height);
        if(freeSlot)
        if (upd !== "error"){
           newItem.upd = upd;
        }
