...
class BotItemGeneration {
    ...
    static async generateBotVest(parentId, role) {
        ...
        if (gearEntries && gearEntries.length > 0) {
            const vestTemplate = await Item.get(await getRandomFromArray(await BotUtilities.shuffleGearEntries(gearEntries)));
    ...
    static async generateBotMelee(parentId, role) {
        ...
        const scabbard = gearEntries ?
            await getRandomFromArray(await BotUtilities.shuffleGearEntries(database.bot.bots[role].gearEntries.melee)) :
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            if (requiredSlots.mod_reciever.filter.length > 0) {
                ...
                const mod_receiver = await this.generateRecieverAndOrScope(
                    reciever,
    ...
    static async generateAdapterAndOrRearSight(anamoly, slotId, parentId, conflictIds) {
        ...
        const anamolySlots = await BotUtilities.genereateListOfSlots(rearSightTemplate);
    ...
    static async generateMountAndOrScope(anamoly, slotId, parentId, conflictIds) {
        ...
        if (mountedOpticTemplate._parent === "55818b224bdc2dde698b456f") {
            ...
            const opticMountSlots = await BotUtilities.genereateListOfSlots(opticMountTemplate);
    ...
    static async generateAdapterAndOrScope(anamoly, slotId, parentId, conflictIds) {
        ...
        if (scopeTemplate._parent === "55818b224bdc2dde698b456f") {
            ...
            let adapterSlots = await BotUtilities.genereateListOfSlots(adapterTemplate);
     ...
     * @returns {<Promise>{}}
     */
    static async generateRecieverAndOrScope(reciever, slotId, parentId, conflictIds, mountedSightChance) {
        const output = [];

        const recieverTemplate = await Item.get(reciever);
        const recieverSlots = await BotUtilities.genereateListOfSlots(recieverTemplate);
        ...
        if (mountedSightChance && recieverSlots?.mod_scope) {
    ...
    static async generateListOfRequiredPartsForWeapon(props) {
        ...
        slots.find(slot => {
            ...
            // these are required for proper bot generation
                ...
                check = ["mod_reciever", "mod_sight_rear", "mod_stock", "mod_muzzle", "mod_mount"].some(
