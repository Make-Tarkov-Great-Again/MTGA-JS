...
class Bot extends BaseModel {
    ...
    static async generateBots(request, _reply) {
        ...
        for (const botParameter of botsParameters) {
            ...
            // TODO: Isolate that code in a function
            // generate entries per equipment slots
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const weaponTemplate = await Bot.retrieveWeaponTemplateFromEntries(role, "FirstPrimaryWeapon");
        ...
        const ammoUsed = await getRandomFromArray(compatibleAmmo);
        output.push(...await BotItemGeneration.generateBotGear(parentId, compatibleMagazines, ammoUsed, role));
        //const weapon = await this.createWeaponForSlot(parentId, role, "FirstPrimaryWeapon");
        const ammo = await this.generateAmmo(weapon[0], role);
        const patronInWeapon = await this.generatePatronInWeapon(weapon[0], ammo, role);
        ...
        //const body = await this.generateBodyAccessory(parentId, cartridge, role);
        return output;
        return [...weapon, patronInWeapon, ...cartridge, scabbard, ...securedContainer];
    }

    /**
     * Generate gear for the bot: armor, vest (tactical or armored), backpack, securedContainer.
     * Add loot in that gear: magazines, ammo, meds, additionnal loot goes in the backpack (money, food, various items...)
     * @param {string} parentId equipmentID
     * @param {array} compatibleMagazines list of all magazine for the weapon
     * @param {string} ammoUsed id of the ammo the bot will use
     * @param {string} role role of the bot (assault, bossKojany...)
     * @returns {Promise<Array>} list of generated gear and loot
     */
    async generateBodyAccessory(parentId, compatibleMagazines, ammoUsed, role) {
        const output = []; // array of body accessories
        const conflicts = []; // array of item ids that conflict with the current item

        const securedContainer = await BotItemGeneration.generateSecuredContainer(parentId, compatibleMagazines, ammoUsed);
        output.push(...securedContainer); // add secured container and extra munitions to output

        if (database.bot.bots[role].loadout) {
            const loadout = database.bot.bots[role].loadout;
            if (loadout.Pockets) {
                const pockets = await this.generatePockets(parentId, role, loadout.Pockets);
                output.push(pockets); // add pockets and extra munitions/loot to output
            }
            if (loadout.TacticalVest) {
                const vest = await this.generateTacticalVest(parentId, role, loadout.TacticalVest);
                output.push(vest.vest); // add tactical vest and extra munitions/loot to output
                conflicts.push(...vest.conflicts);
            }
        } else {
            const pockets = await this.generatePockets(parentId, role);
            output.push(pockets); // add pockets and extra munitions/loot to output

            const vest = await this.generateTacticalVest(parentId, role);
            output.push(vest.vest); // add tactical vest and extra munitions/loot to output
            conflicts.push(...vest.conflicts);
            if (await getPercentRandomBool(25)) { // 25% chance of generating a backpack
                const backpack = await this.generateBackpack(parentId, role, conflicts);
                if (backpack) {
                    output.push(backpack.backpack); // add backpack and loot output
                    conflicts.push(...backpack.conflicts);
                }
            }
            if (await getPercentRandomBool(25)) { // 25% chance of generating an armorVest
                const armorVest = await this.generateArmorVest(parentId, role, conflicts);
                if (armorVest) {
                    output.push(armorVest.armorVest);
                    conflicts.push(...armorVest.conflicts);
                }
            }
        }
        return output;
    ...
    /**
     * Pick a random weapon from the gearEntries list of the bot database
     * @param {string} botRole Role of the bot. ex: assault...
     * @param {string} weaponSlot SlotId of the weapon. ex: FirstPrimaryWeapon...
     * @returns {Promise<Item>} Weapon template
     */
    static async retrieveWeaponTemplateFromEntries(botRole, weaponSlot) {
        const gearEntries = database.bot.bots[botRole].gearEntries;
        if (weaponSlot === "FirstPrimaryWeapon") {
            return await Item.get(await getRandomFromArray(gearEntries.primaryWeapon));
        } else {
            return await Item.get(await getRandomFromArray(gearEntries.secondaryWeapon)); // pistols
        }
    }
    ...
    async generateTacticalVest(parentId, role, predefinedVests=false) {
        let vests;
        if (!predefinedVests) {
            vests = await BotUtilities.generateLootBucket("TacticalVest", role);
        } else {
            vests = predefinedVests;
        }
        const vest = await Item.get(await getRandomFromArray(vests));

        const data = await this.createTacticalVest(parentId, vest);
        if (vest._props.ConflictingItems) data.conflicts = vest._props.ConflictingItems;

        return data;
    }

    async createTacticalVest(parentId, vest) {
        return {
            "vest": {
                "_id": await generateMongoID(),
                "_tpl": vest._id,
    ...
    async generateArmorVest(parentId, role, conflicts) {
        const armorVests = await BotUtilities.generateLootBucket("ArmorVest", role)
        const armorVest = await Item.get(await getRandomFromArray(armorVests));

        const check = await BotUtilities.checkForConflicts(armorVest._id, conflicts)
        if (!check && armorVest._props.BlocksArmorVest === false) {
            const data = await this.createArmorVest(parentId, armorVest);
            if (armorVest._props.ConflictingItems) data.conflicts = armorVest._props.ConflictingItems;
            return data;
        }
        return false;
    }

    async createArmorVest(parentId, armorvest) {
        return {
            "armorVest": {
                "_id": await generateMongoID(),
                "_tpl": armorvest._id,
    ...
    async generateBackpack(parentId, role, conflicts) {
        const backpacks = await BotUtilities.generateLootBucket("Backpack", role);
        const backpack = await Item.get(await getRandomFromArray(backpacks));

        const check = await BotUtilities.checkForConflicts(backpack._id, conflicts)
        if (!check && backpack._props.BlocksArmorVest === false) {
            const data = await this.createBackpack(parentId, backpack, role);
            if (backpack._props.ConflictingItems) data.conflicts = backpack._props.ConflictingItems;
            return data;
        }
        return false;
    }

    async createBackpack(parentId, backpack, role) {
        return {
            "backpack": {
                "_id": await generateMongoID(),
                "_tpl": backpack._id,
    ...
    async generatePockets(parentId, role, predefinedPocket=false) {
        if (!predefinedPocket) {
...
class BotItemGeneration {
    ...
    static async generateBotGear(parentId, compatibleMagazines, ammoUsed, role) {
        const gearEntries = database.bot.bots[role].gearEntries;
        const generatedGear = [];
        const botRig = await BotItemGeneration.generateBotRig(parentId, gearEntries);
        // TODO: generate loot in the botRig: magazines or loose ammo, meds
        if (botRig) {
            generatedGear.push(botRig);
        }
        if (botRig?.slotId === "TacticalVest") {
            const bodyArmor = await BotItemGeneration.generateBotBodyArmor(parentId, gearEntries);
            if (bodyArmor) {
                generatedGear.push(bodyArmor);
            }
        }
    }
     ...
     */
    static async generateBotBodyArmor(parentId, gearEntries) {
        if (gearEntries.bodyArmor.length > 0) {
            ...
            botBodyArmor.upd = await Item.createFreshBaseItemUpd(armorTemplate);
     ...
     * @returns {Promise<object>} final item generated or false
     */
    static async generateBotRig(parentId, gearEntries) {
        ...
        botRig.upd = await Item.createFreshBaseItemUpd(rigTemplate);
        botRig.slotId = rigSlotId;
