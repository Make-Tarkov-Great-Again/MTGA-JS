...
class BotItemGeneration {
    ...
    static async assembleWeapon(botInfo, choice, slotId, parentId) {
        ...
        if (baseParts?.mod_magazine) {
            ...
            if (choices)
                ...
                logger.error("[assembleWeapon] choices is undefined")
        ...
        for (const slot in baseParts) {
            if (Object.keys(required).includes(slot)) {
                switch (slot) {
                    ...
                    case "mod_stock_000":
                    case "mod_stock":
                        const stock = await this.generateStockSlot(firecontrol._id, slot, baseParts[slot], cachedSlots, required, stockConflicts);
                    ...
                    case "mod_pistol_grip":
                        const pistolgrip = await this.generateRequiredSlot(firecontrol._id, slot, baseParts[slot], cachedSlots, required, stockConflicts);
                    ...
                    default:
                        ...
                        const mod = await this.generateRequiredSlot(firecontrol._id, slot, baseParts[slot], cachedSlots, required, conflicts);
                ...
                await this.generateRequiredSlot(firecontrol._id, slot, baseParts[slot], optional, required, conflicts);
                break;
            }
        }
    ...
    static async generateStockSlot(parentId, slot, choices, cachedSlots, required, conflicts) {
        ...
        if (!selections) {
            logger.error(`[generateStockSlot] const selection for ${slot} is undefined`)
        }

        const selection = await this.selectWeaponPart(selections, conflicts);
        if (!selection) {
            logger.error(`[generateStockSlot] const selection for ${slot} is undefined`);
            return output;
        }
        ...
        const part = await this.createWeaponPart(selection, slot, parentId, conflicts);
        ...
        if (choices[selection].hasOwnProperty("Slots") && await BotUtilities.isBufferTubeOrAdapter(selection)) { //check for children/sub slots and apply them if they're required
    ...
    static async generateRequiredSlot(parentId, slot, choices, cachedSlots, required, conflicts) {
        ...
        const selections = Object.keys(choices);
        if (!selections) {
            logger.error(`[generateRequiredSlot] const selection for ${slot} is undefined`);
        }
    ...
    static async generateModMagazine(ReloadMode, parentId, magazines, ammo) {
        ...
        if (ReloadMode !== "OnlyBarrel") {
            ...
            const choices = Object.keys(magazines);
            if (!choices) {
                logger.error("[generateModMagazine] const choices is undefined");
            }
            ...
            const anamoly = await Item.get(selection);
