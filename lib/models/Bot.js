...
class BotItemGeneration {
    ...
    static async generateBotWeapon(botInfo, parentId, role, slotId, ammos = [], magazines = {}) {
        ...
        const weaponTemplate = botInfo.weaponCache.weapons[choice];
        const modSlots = botInfo.weaponCache.slots;
        ...
        if (weapon._props.weaponUseType === "primary" && slotId == "Holster") {
            logger.info(`Remove ${weapon._id} from weightedGear.primaryWeapon`);
        } else if (weapon._props.weaponUseType !== "secondary" && slotId === ["FirstPrimaryWeapon", "SecondPrimaryWeapon"]) {
            logger.info(`Remove ${weapon._id} from weightedGear.Holster`);
        }
        ...
        const required = await cloneDeep(weaponTemplate.Required);
        const parts = weaponTemplate.Parts;

        required["mod_magazine"] = await this.generateModMagazine(weapon, assembledWeapon, parts.mod_magazine, ammo);
    ...
    static async generateModMagazine(weapon, assembledWeapon, magazines, ammo) {
        if (weapon._props.ReloadMode !== "OnlyBarrel") {
            const selected = await this.selectWeaponPart(magazines.IDs)
            ...
            const mod_magazine = await this.createWeaponPart(
                anamoly, "mod_magazine", assembledWeapon[0]._id);
    ...
    static async selectWeaponPart(filter) {
        const balles = await BotUtilities.mountsToBlockBecauseTheySuck();
        filter = filter.filter(item => !balles.includes(item));
