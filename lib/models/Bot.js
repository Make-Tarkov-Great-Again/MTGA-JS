...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const ammo = Object.keys(magazines).length > 0
            ? await BotItemGeneration.generateAmmoInContainer(botPocket, securedContainer, ammos, magazines)
...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId, ammos = [], magazines = {}) {
        ...
        if (weapon._props.ReloadMode !== "OnlyBarrel") {
            ...
            const mod_magazine = await this.createWeaponPart(anamoly, "mod_magazine", firecontrol._id);
            if (weapon._props.ReloadMode === "InternalMagazine") {
                ...
                for (const camora of camoras) {
                    const cartridges = await this.createWeaponPart(ammo, camora, mod_magazine._id);
        ...
        for (const rid in required) {
            const require = required[rid];

            mod_part = await this.generateRequiredWeaponPart(require, firecontrol._id, conflictIds, filledAttachments);
        ...
        for (const oid in optional) {
            ...
            mod_part = await this.generateOptionalWeaponPart(option, firecontrol._id, conflictIds, filledAttachments);
            assembledWeapon.push(...mod_part);
            continue;
        }
                ...
                for (const id in optional) {
                    ...
                    switch (id) {
                        case "mod_mount":
                            ...
                            roll = count > 0
                                ? await getPercentRandomBool(await getRandomInt(0, 75 - (5 * count)))
                                : await getPercentRandomBool(await getRandomInt(0, 75));
                        ...
                        case "mod_sight_rear":
                            roll = await getPercentRandomBool(await getRandomInt(0, 75));
    ...
    static async generateMagazine(parentId, magazine, freeSlot) {
        const newMag = await magazine.createAsNewItemWithParent(parentId);
            ...
            for (let i = 0; i < randomAmmos.maxTypes; i++) {
                ...
                if (await getPercentRandomBool(randomAmmos.chance)) {
                    ...
                    if (Object.keys(choices).length > 0)
                        choices[await getRandomFromObject(Object.keys(choices))] += await getRandomInt(1, 35);
                    lastchoice = choice;
                    continue;
                }
            }

            if (Object.keys(choices).length === 0) { //incase the bot has shit rolls lol
                choices[lastchoice] = await getRandomInt(35, 55);
            }

            if (magazines) {
                for (let i = 0; i < await getRandomInt(1, 3); i++) {
                    const choice = await Item.get(await getRandomFromArray(magazines))
                    const magazine = await this.generateMagazinesInContainer(
                        mainContainer,
                        await Item.get(await getRandomFromArray(magazines)),
                        1
                    );
                }
            } else {
