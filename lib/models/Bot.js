...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            const firecontrol = await this.generateWeaponPart(weapon, slotId, parentId);
            ...
            if (weaponProps.Chambers.length > 0) {
                ...
                const ammo = await this.generateWeaponPartChoices(ammos);
                ...
                if (weaponProps.ReloadMode === "OnlyBarrel") {
                    ...
                    const magazine = magazines[await this.generateWeaponPartChoices(Object.keys(magazines))];
            ...
            if (requiredSlots?.mod_barrel) {
                ...
                const barrel = await this.generateWeaponPartChoices(requiredSlots.mod_barrel.filter);
                const mod_barrel = await this.generateWeaponPart(
                    barrel,
            ...
            if (requiredSlots.mod_gas_block) {
                ...
                if (requiredSlots.mod_gas_block.filter.length > 0) {
                    const gasblock = await this.generateWeaponPartChoices(requiredSlots.mod_gas_block.filter);
            ...
            if (requiredSlots.mod_handguard) {
                ...
                const handguard = await this.generateWeaponPartChoices(requiredSlots.mod_handguard.filter);
                const mod_handguard = await this.generateWeaponPart(
                    handguard,
            ...
            if (requiredSlots.mod_sight_rear) { //can make roll
                const rearSight = await this.generateWeaponPartChoices(requiredSlots.mod_sight_rear.filter);
            ...
            if (requiredSlots.mod_reciever.filter.length > 0) {
                const reciever = await this.generateWeaponPartChoices(requiredSlots.mod_reciever.filter);
            ...
            if (mountCheck) {
                ...
                const sideRailMount = await this.generateWeaponPartChoices(requiredSlots.mod_mount.filter);
            ...
            // mod_pistol_grip
            const pistolGrip = await this.generateWeaponPartChoices(requiredSlots.mod_pistol_grip.filter);
            const mod_pistol_grip = await this.generateWeaponPart(
                pistolGrip,
            ...
            if (requiredSlots.mod_stock) {
                ...
                if (requiredSlots.mod_stock.filter.length > 0) {
                    const stock = await this.generateWeaponPartChoices(requiredSlots.mod_stock.filter);
    ...
    static async generateAdapterAndOrRearSight(anamoly, slotId, parentId, conflictIds) {
        ...
        if (anamolySlots?.mod_sight_rear) {
            ...
            const mod_adapter = await rearSightAdapterTemplate.createAsNewItemWithParent(parentId);
            mod_adapter.slotId = slotId;
            const upd = await Item.createFreshBaseItemUpd(rearSightAdapterTemplate);
            if (upd !== "error") mod_adapter.upd = upd;

            output.push(mod_adapter);

            const weightedList = await BotUtilities.generateWeightedList(anamolySlots.mod_sight_rear.filter)
            const choices = await BotUtilities.generateGearEntries(weightedList);

            const anamoly = await getRandomFromArray(choices);
            const mod_sight_rear = await this.generateAdapterAndOrRearSight(
                anamoly,
            ...
            const mod_sight_rear = await rearSightTemplate.createAsNewItemWithParent(parentId);
            mod_sight_rear.slotId = slotId;
            const upd = await Item.createFreshBaseItemUpd(rearSightTemplate);
            if (upd !== "error") mod_sight_rear.upd = upd;
    ...
    static async generateMountAndOrScope(anamoly, slotId, parentId, conflictIds) {
        ...
        if (mountedOpticTemplate._parent === "55818b224bdc2dde698b456f") {
            ...
            const mod_mount = await this.generateWeaponPart(
                opticMountTemplate,
                slotId,
                parentId,
                conflictIds
            ...
            const scope = await this.generateWeaponPartChoices(opticMountSlots.mod_scope.filter)
            ...
            const mod_mount_000 = await this.generateWeaponPart(
    ...
    static async generateAdapterAndOrScope(anamoly, slotId, parentId, conflictIds) {
        ...
        if (scopeTemplate._parent === "55818b224bdc2dde698b456f") {
            ...
            const mod_adapter = await this.generateWeaponPart(
                adapterTemplate,
                slotId,
                parentId,
                conflictIds
            ...
            const mod_scope = await this.generateWeaponPart(
                scope,
                adapterSlot.slotId,
                mod_adapter._id,
                conflictIds
            ...
            output.push(await this.generateWeaponPart(
                scopeTemplate,
                slotId,
                parentId,
                conflictIds
    ...
    static async generateRecieverAndOrScope(reciever, slotId, parentId, conflictIds, mountedSightChance) {
        ...
        const mod_receiver = await this.generateWeaponPart(
            recieverTemplate,
            slotId,
            parentId,
            conflictIds
        ...
        if (recieverSlots?.mod_sight_rear) {
            const anamoly = await this.generateWeaponPartChoices(recieverSlots.mod_sight_rear.filter)
            const mod_sight_rear = await this.generateAdapterAndOrRearSight(
                anamoly,
        ...
        if (mountedSightChance && recieverSlots?.mod_scope) {
            ...
            if (recieverSlots.mod_scope.filter.length > 0) {
                ...
                const anomaly = await this.generateWeaponPartChoices(recieverSlots.mod_scope.filter)
                const mod_scope = await this.generateAdapterAndOrScope(
                    anomaly,
    ...
    static async generateGasBlockAndHandguard(gasblock, slotId, parentId, conflictIds) {
        ...
        const mod_gas_block = await this.generateWeaponPart(
            gasblock,
            slotId,
            parentId,
            conflictIds
        ...
        if (gasblockSlots.mod_handguard) {
            const handguard = await this.generateWeaponPartChoices(gasblockSlots.mod_handguard.filter);
            const mod_handguard = await this.generateWeaponPart(
                handguard,
     ...
     * @returns {<Promise> {}}
     */
    static async generateWeaponPart(part, slotId, parentId, conflictIds = null) {
        const partTemplate = part?._props ? part : await Item.get(part);
        const mod_part = await partTemplate.createAsNewItemWithParent(parentId);
        mod_part.slotId = slotId;
        const upd = await Item.createFreshBaseItemUpd(partTemplate);
        if (upd !== "error") mod_part.upd = upd;

        if (conflictIds) conflictIds.push(...partTemplate._props.ConflictingItems);
        return mod_part;
    }
     ...
     */
    static async generateWeaponPartChoices(filter) {
        let weightedList = await BotUtilities.generateWeightedList(filter);
    ...
    static async generateAdapterAndOrStock(anomaly, slotId, parentId) {
        ...
        if (await BotUtilities.isBufferTubeOrAdapter(stockTemplate._id)) {
            ...
            const stock = await this.generateWeaponPartChoices(adapterSlots["mod_stock"].filter);

            const mod_stock = await this.generateWeaponPart(
                stock,
...
class BotUtilities {
    ...
    static async mountsToBlockBecauseTheySuck(itemId) {
        return [
