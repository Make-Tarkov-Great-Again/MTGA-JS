...
class Bot extends BaseModel {
    ...
    static async getBotsForDuplicatedIds(bots) {
        ...
        for (const [key, bot] of Object.entries(bots)) {
            for (const item of bot.Inventory.items) {
                if (cache.includes(item._id)) {
                    await logger.error("Duplicate Found!");
                    ...
                    if (replacement) {
                        await logger.success("Duplicate Replaced!")
                    }
                    bots.splice(key, 1);
        ...
        await logger.debug(`${duplicates} duplicates found`);
        if (output.length === 0)
            return false;
        return output;
    }
    ...
    static async checkAssembledWeapon(assembledWeapon, required, optional, conflictIds) {
        ...
        for (const id in required) {
            ...
            if (part) {
                ...
                await logger.error(`${require.slotId} was missing because your code is shit and you need checks!!!!!!`)
            }
        }
