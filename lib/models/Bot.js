...
class Bot extends BaseModel {
    ...
    async generateHeadAccessory(parentId, role = null) {
        ...
        if (await getPercentRandomBool(25)) {
            ...
            const facecover = await getRandomFromArray(
                await BotUtilities.generateAllowedItems(facecovers, "FaceCover")
            );
            if (facecover) {
        ...
        if (await getPercentRandomBool(25)) {
            ...
            const earpiece = await getRandomFromArray(
                await BotUtilities.generateAllowedItems(headphones, "Earpiece")
            );
            if (earpiece) {
        ...
        if (await getPercentRandomBool(25)) {
            ...
            const visor = await getRandomFromArray(
                await BotUtilities.generateAllowedItems(visors, "Eyewear")
            );
            if (visor) {
        ...
        if (await getPercentRandomBool(25)) {
            ...
            const headwear = await getRandomFromArray(
                await BotUtilities.generateAllowedItems(headwears, "Headwear")
            );
            if (headwear) {
    ...
    async generateBodyAccessory(parentId, role = null) {
        ...
        const vest = await getRandomFromArray(
            await BotUtilities.generateAllowedItems(vests, "TacticalVest")
        );
        ...
        if (await getPercentRandomBool(25)) {
            ...
            const backpack = await getRandomFromArray(
                await BotUtilities.generateAllowedItems(backpacks, "Backpack")
            );
            if (backpack) {
        ...
        if (await getPercentRandomBool(25)) {
            ...
            const armorvest = await getRandomFromArray(
                await BotUtilities.generateAllowedItems(armor, "ArmorVest")
            );
            if (armorvest) {
    ...
    async generateScabbard(parentId, role = null) {
        ...
        const scabbard = await getRandomFromArray(
            await BotUtilities.generateAllowedItems(
                lootbucket, "Scabbard"
            )
        );
        return {
...
class BotUtilities {
        ...
        if (data._props.Slots) {
            for (const slot of data._props.Slots) {
                for (const filter of slot._props.filters) {
                    if (filter.Filter) {
                        conflicts[slot._name] = filter.Filter;
                    }
                }
            }
        }
        //writeFile("./conflicts.json", stringify(conflicts));
        return conflicts;
    }
    ...
    static async generateLootBucket(itemType, role) {
        const data = readParsed("./rarity.json");
