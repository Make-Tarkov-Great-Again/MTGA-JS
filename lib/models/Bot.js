...
class Bot extends BaseModel {
    ...
    static async generateBots(request = null, preload = null) {
        ...
        for (const botParameter of botsParameters) {
            ...
            for (let i = 0; i < Limit; i++) {
                ...
                await newBot.generateInfo(bot, Role, Difficulty);
                await newBot.generateCustomization(bot, Role);
                await newBot.generateHealth(bot, Role, Difficulty);
                await newBot.generateInventory(bot, Role);
    ...
    async generateNickname(botInfo, role) {
        switch (true) {
            ...
            default:
                return botInfo.names[role]
                    ? botInfo.names[role][0]
                    : getRandomFromArray(botInfo.names.scav);
        }
    }

    async generateVoice(botInfo, role) {
        switch (true) {
            case role && botInfo.appearance[role]:
                return typeof botInfo.appearance[role].Voice !== "string"
                    ? getRandomFromObject(botInfo.appearance[role].Voice)
                    : botInfo.appearance[role].Voice;
    ...
    async generateHealth(botInfo, role, difficulty) {
        ...
        switch (true) {
            case botInfo.bots[role].health && Object.keys(botInfo.bots[role].health).length > 1:
                Object.assign(health, botInfo.bots[role].health[difficulty]);
                break;
            case botInfo.bots[role].health:
                Object.assign(health, botInfo.bots[role].health);
                break;
            default:
                //await logger.error(`[Bot : generateHealth] Role [${role}] health not handled`);
                Object.assign(health, botInfo.bots["assault"].health["impossible"]);
                break;
        }
...
class BotItemGeneration {
    ...
    static async generateBotEarpiece(botInfo, parentId, role, conflictIds, earpieceChance) {
        const earpieces = await BotUtilities.cleanPartList(botInfo.bots[role].gearEntries.earpiece, conflictIds)
        if (earpieces && earpieces.length > 0 && await getRandomInt() <= earpieceChance) {
            const earpieceTemplate = await Item.get(await getRandomFromArray(earpieces));
    ...
    static async generateBotFacecover(botInfo, parentId, role, conflictIds, facecoverChance) {
        const facecovers = await BotUtilities.cleanPartList(botInfo.bots[role].gearEntries.facecover, conflictIds);
        if (facecovers && facecovers.length > 0 && await getRandomInt() <= facecoverChance) {
            const facecoverTemplate = await Item.get(await getRandomFromArray(facecovers));
    ...
    static async generateBotHeadwear(botInfo, parentId, role, headwearChance) {
        const headwears = botInfo.bots[role].gearEntries.headwear;
        if (headwears && headwears.length > 0 && await getRandomInt() <= headwearChance) {
            const headwearTemplate = await Item.get(await getRandomFromArray(headwears));
    ...
    static async generateBotBodyArmor(botInfo, parentId, role) {
        const gearEntries = botInfo.bots[role].gearEntries.bodyArmor;
    ...
    static async generateBotVest(botInfo, parentId, role) {
        const gearEntries = botInfo.bots[role].gearEntries.vest;
    ...
    static async generateBotMelee(botInfo, parentId, role) {
        const gearEntries = botInfo.bots[role].gearEntries.melee;
    ...
    static async generateBotWeapon(botInfo, parentId, role, slotId, ammos = [], magazines = {}) {
        ...
        let gearEntries = []
        gearEntries = botInfo.bots[role].gearEntries;

        switch (slotId) {
            case "Holster":
                gearEntries = gearEntries?.holster.length !== 0
                    ? gearEntries.holster
                    : ["583990e32459771419544dd2"];
                break;
            case "FirstPrimaryWeapon":
                gearEntries = gearEntries?.primaryWeapon.length !== 0
                    ? gearEntries.primaryWeapon
                    : gearEntries?.secondaryWeapon.length !== 0
        ...
        // create fallback for weapons and output warning
        ...
        const choice = gearEntries.length === 1 ? gearEntries[0] : await getRandomFromArray(gearEntries);
    ...
    static async selectWeaponPart(filter) {
        ...
        const entries = await BotUtilities.generateGearEntries(weightedList);
        const choices = await shuffleArray(entries);
    ...
    static async generateBotBackpack(botInfo, parentId, role, backpackChance) {
        ...
        const backpacks = botInfo.bots[role].gearEntries.backpack;
