...
class Bot extends BaseModel {
    constructor() {
        super();
        this.template = cloneDeep(botTemplate);
    }
    ...
    static async initializeBotPreload() {
        const { database: { bot: { bots } } } = require('../../app');
    ...
    static async generateBots(request = null, preload = null) {
        ...
        for (const botParameter of botsParameters) {
            ...
            if (!bot.bots[Role].gearEntries) {
                bot.bots[Role].gearEntries = {};
                for (const gearCateg of bot.bots[Role].weightedGear) {
                    const categorie = Object.keys(gearCateg)[0];
                    bot.bots[Role].gearEntries[categorie] = await BotUtilities.generateGearEntries(gearCateg[categorie]);
                }
            }
        ...
        if (debugBots)
            await writeFile("./botweaponcache.json", stringify(bot.weaponCache));
    ...
    async generateNickname(role) {
        ...
        switch (true) {
            case ["exUsec", "pmcBot", "followerGluharSecurity", "followerGluharScout",
                ...
                return getRandomFromArray(bot.names.generalFollower);

            case ["marksman", "cursedAssault", "playerscav", "assault"].includes(role):
                return getRandomFromArray(bot.names.scav);

            case ["followerSanitar", "followerKojaniy", "followerBully"].includes(role):
                return getRandomFromArray(bot.names[role]);

            case ["usec", "bear"].includes(role):
                return getRandomFromArray(bot.names.normal);

            default:
                return bot.names[role]
                    ? bot.names[role][0]
                    : getRandomFromArray(bot.names.scav);
        }
    }
    ...
    async generateVoice(role) {
        ...
        switch (true) {
            case role && bot.appearance[role]:
                return typeof bot.appearance[role].Voice !== "string"
                    ? getRandomFromObject(bot.appearance[role].Voice)
                    : bot.appearance[role].Voice;

            case role && ["assault", "cursedAssault", "marksman"].includes(role):
                return getRandomFromObject(bot.appearance.scav.Voice);
            case role && ["followerGluharSecurity", "followerGluharAssault", "followerGluharScout"].includes(role):
                return getRandomFromObject(bot.appearance.followerGluhar.Voice);
            default:
                //await logger.error(`[Bot : generateInfo] Role [${role}] appearance not handled`);
                return getRandomFromObject(bot.appearance.random.Voice);
        }
    }
    ...
    async generateExperience(role) {
        switch (true) {
            ...
            case ["assault", "cursedAssault", "marksman", "gifter"].includes(role):
                return -1
            default:
                //await logger.info(`[Bot : generateExperience] Role [${role}] settings not handled`);
                return 0;
        }
    }
    ...
    async generateCustomization(role) {
        ...
        switch (true) {
            case bot.appearance[role]:
                templateCustomization.Head = typeof bot.appearance[role].Head !== "string"
                    ? await getRandomFromArray(bot.appearance[role].Head)
                    : bot.appearance[role].Head;

                templateCustomization.Body = typeof bot.appearance[role].Body !== "string"
                    ? await getRandomFromArray(bot.appearance[role].Body)
                    : bot.appearance[role].Body;

                templateCustomization.Hands = typeof bot.appearance[role].Hands !== "string"
                    ? await getRandomFromArray(bot.appearance[role].Hands)
                    : bot.appearance[role].Hands;

                templateCustomization.Feet = typeof bot.appearance[role].Feet !== "string"
                    ? await getRandomFromArray(bot.appearance[role].Feet)
                    : bot.appearance[role].Feet;

                return this.setCustomization(templateCustomization);

            case ["assault", "cursedAssault", "marksman"].includes(role):
                templateCustomization.Head = await getRandomFromArray(bot.appearance.scav.Head);
                templateCustomization.Body = await getRandomFromArray(bot.appearance.scav.Body);
                templateCustomization.Hands = await getRandomFromArray(bot.appearance.scav.Hands);
                templateCustomization.Feet = await getRandomFromArray(bot.appearance.scav.Feet);

                return this.setCustomization(templateCustomization);


            case ["followerGluharSecurity", "followerGluharAssault", "followerGluharScout"].includes(role):
                templateCustomization.Head = await getRandomFromArray(bot.appearance.followerGluhar.Head);
                templateCustomization.Body = await getRandomFromArray(bot.appearance.followerGluhar.Body);
                templateCustomization.Hands = bot.appearance.followerGluhar.Hands;
                templateCustomization.Feet = await getRandomFromArray(bot.appearance.followerGluhar.Feet);
            ...
            default:
                ...
                templateCustomization.Head = await getRandomFromArray(bot.appearance.random.Head);
                templateCustomization.Body = await getRandomFromArray(bot.appearance.random.Body);
                templateCustomization.Hands = await getRandomFromArray(bot.appearance.random.Hands);
                templateCustomization.Feet = await getRandomFromArray(bot.appearance.random.Feet);
    ...
    async generateHealth(role, difficulty) {
        ...
        switch (true) {
            case bot.bots[role].health && Object.keys(bot.bots[role].health).length > 1:
                Object.assign(health, bot.bots[role].health[difficulty]);
                break;
            case bot.bots[role].health:
                Object.assign(health, bot.bots[role].health);
                break;
            default:
                //await logger.error(`[Bot : generateHealth] Role [${role}] health not handled`);
                Object.assign(health, bot.bots["assault"].health["impossible"]);
                break;
        }
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const ammo = Object.keys(magazines).length > 0
            ? await BotItemGeneration.generateAmmoInContainer(
                botVest,
                securedContainer,
                ammos,
                magazines
            )
            : await BotItemGeneration.generateAmmoInContainer( // put loose rounds in either pocket or vest
...
class BotItemGeneration {
    ...
    static async generateBotEarpiece(parentId, role, conflictIds, earpieceChance) {
        ...
        const gearEntries = await BotUtilities.cleanPartList(bot.bots[role].gearEntries.earpiece, conflictIds)
    ...
    static async generateBotFacecover(parentId, role, conflictIds, facecoverChance) {
        ...
        const facecovers = await BotUtilities.cleanPartList(bot.bots[role].gearEntries.facecover, conflictIds);
    ...
    static async generateBotHeadwear(parentId, role, headwearChance) {
        ...
        const headwears = bot.bots[role].gearEntries.headwear;
    ...
    static async generateBotBodyArmor(parentId, role) {
        ...
        const gearEntries = bot.bots[role].gearEntries.bodyArmor;
        if (gearEntries && gearEntries.length > 0) {
            const armorTemplate = await Item.get(await getRandomFromArray(await shuffleArray(gearEntries)));
    ...
    static async generateBotVest(parentId, role) {
        ...
        const gearEntries = bot.bots[role].gearEntries.vest;
        // pick the itemID to generate
        if (gearEntries && gearEntries.length > 0) {
            const choices = await shuffleArray(gearEntries);
            const vestTemplate = await Item.get(await getRandomFromArray(await shuffleArray(choices)));
    ...
    static async generateBotMelee(parentId, role) {
        ...
        const gearEntries = bot.bots[role].gearEntries.melee;
        const scabbard = gearEntries && gearEntries.length > 0
            ? await getRandomFromArray(await shuffleArray(gearEntries))
    ...
    static async generateBotWeapon(parentId, role, slotId, ammos = [], magazines = {}) {
        ...
        let gearEntries = []
        gearEntries = bot.bots[role].gearEntries;

        switch (slotId) {
            case "Holster":
                gearEntries = gearEntries?.holster
                    ? gearEntries?.holster
                    : gearEntries.push("583990e32459771419544dd2");
                break;
            case "FirstPrimaryWeapon":
                gearEntries = gearEntries?.primaryWeapon
                    ? gearEntries.primaryWeapon
                    : gearEntries.push("583990e32459771419544dd2");
                break;
            default:
                gearEntries = gearEntries?.secondaryWeapon
                    ? gearEntries.secondaryWeapon
                    : gearEntries.push("583990e32459771419544dd2");
                break;
        }
        ...
        if (!bot.weaponCache[choice]) {
            bot.weaponCache[choice] = {};
        }
        ...
        if (!bot.weaponCache[choice]["parts"]) {
            const { required, optional } = await BotUtilities.generatePartList(weapon);

            bot.weaponCache[choice]["parts"] = {};
            bot.weaponCache[choice].parts["required"] = required;
            bot.weaponCache[choice].parts["optional"] = optional;
        }

        const [required, optional] = [
            bot.weaponCache[choice].parts.required,
            bot.weaponCache[choice].parts.optional
        ];

        if (weapon._props.weaponUseType === "primary" && slotId == "Holster") {
            await logger.info(`Remove ${weapon._id} from gearEntries.primaryWeapon`);
        } else if (weapon._props.weaponUseType !== "secondary" && slotId === ["FirstPrimaryWeapon", "SecondPrimaryWeapon"]) {
            await logger.info(`Remove ${weapon._id} from gearEntries.Holster`);
        }


        if (!bot.weaponCache[choice]["ammos"]) {
            bot.weaponCache[choice]["ammos"] = await weapon.generateCompatibleAmmoList();
        }

        ammos = bot.weaponCache[choice].ammos;
        ...
        if (weapon._props.ReloadMode !== "OnlyBarrel") {
            if (!bot.weaponCache[choice]["magazines"]) {
                bot.weaponCache[choice]["magazines"] = await weapon.generateCompatibleMagazineList(true);
            }
            magazines = bot.weaponCache[choice].magazines;
    ...
    static async generateRequiredWeaponPart(required, parentId, conflictIds, filledAttachments) {
        ...
        const part = filtered.length === 1
            ? filtered[0]
            : await this.selectWeaponPart(filtered);
    ...
    static async generateWeaponPart(partId, slotId, parentId, conflictIds, filledAttachments = null, partList = null) {
        ...
        const { required, optional } = partList
    ...
    static async generateBotPockets(parentId, role) {
        ...
        const authorizedGear = bot.bots[role].authorizedGear;
        const pocket = authorizedGear?.pocket
            ? await getRandomFromArray(await shuffleArray(authorizedGear.pocket))
    ...
    static async generateBotBackpack(parentId, role, backpackChance) {
        ...
        const gearEntries = bot.bots[role].gearEntries.backpack;
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() >= backpackChance) {
            const backpack = await getRandomFromArray(await shuffleArray(gearEntries))
