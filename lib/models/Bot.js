...
class BotItemGeneration {
    ...
    // TODO: IT IS BROKEN, NEED TO MAKE A FUNCTION TO REGENERATE THE PRESET AND SET CORRECT PARENTS
    static async generateFullWeaponPresetWithoutMagazine(weaponPreset, parentId, slotId) {
        ...
        for (const part of cloneDeep(weaponPreset._items)) {
            if (!part.slotId) {
                part._id = await generateMongoID();
                part.parentId = parentId;
                part.slotId = slotId;
                fullWeapon.push(part);
                partsParent = part._id;
            } else if (part.slotId !== "mod_magazine") {
                part._id = await generateMongoID();
                part.parentId = partsParent;
                fullWeapon.push(part);
            } else {
                continue;
            }
