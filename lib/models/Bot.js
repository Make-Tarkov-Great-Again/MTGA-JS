...
class Bot extends BaseModel {
    ...
    async generateInventory(role = null) {
        ...
        //initiate divide
        const botDefaultInventory = await this.generateInventoryItems(templateItems.shift())
    ...
    async generateInventoryItems(inventory) {
        const parentId = inventory._id;
        const output = (
            [
                inventory,
                await this.generateFirstPrimaryWeapon(parentId),
                await this.generatePockets(parentId),
                await this.generateTacticalVest(parentId),
                await this.generateFaceCover(parentId),
                await this.generateHeadwear(parentId),
                await this.generateScabbard(parentId),
                await this.generateBackpack(parentId),
                await this.generateSecuredContainer(parentId),
            ]
        );
        return output;
    }

    async generateFirstPrimaryWeapon(parentId) {}
    async generatePockets(parentId) {}
}
