...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const [facecover, facecoverConflicts] = await BotItemGeneration.generateBotFacecover(parentId, role);
        if (facecover)
            output.push(facecover);

        const [helmet, headwearConflicts] = await BotItemGeneration.generateBotHeadwear(parentId, role, facecoverConflicts);
        if (helmet)
            output.push(helmet);

        const earpiece = await BotItemGeneration.generateBotEarpiece(parentId, role, [...headwearConflicts, ...facecoverConflicts]);
        if (earpiece)
...
class BotItemGeneration {
     ...
     */
    static async generateBotEarpiece(parentId, role, conflictIds) {
     ...
     * @returns {Promise<array>} final item generate/false & conflicting items
     */
    static async generateBotHeadwear(parentId, role, conflictIds) {
    ...
    static async generateBotBackpack(parentId, role) {
        ...
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() > 30) {
