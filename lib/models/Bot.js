...
class Bot extends BaseModel {
    ...
    static async initializeBotPreload() {
        const {
            database: {
                core: {gameplay: {bots: {preloadParameters: {maxPreloadThreshold, maxBossPreloadThreshold}}}},
                bot: {bots}
            }
        } = require('../../app');
        ...
        for (const bot in bots) {
            ...
            const max = boss.includes(bot) ? maxBossPreloadThreshold : maxPreloadThreshold
            const type = bots[bot];
    ...
    static async regeneratePreloadedBots() {
            ...
            database: {
                ...
                core: {
                    gameplay: {
                        bots: {
                            preloadParameters: {
                                minBossPreloadThreshold,
                                maxBossPreloadThreshold,
                                minPreloadThreshold,
                                maxPreloadThreshold
                            }
                        }
        ...
        for (const bot in preload) {
            ...
            let min, max;
            if (boss.includes(bot)) {
                [min, max] = [minBossPreloadThreshold, maxBossPreloadThreshold];
            } else {
                [min, max] = [minPreloadThreshold, maxPreloadThreshold];
            }
    ...
    async generateNickname(role) {
        switch (true) {
            ...
            default:
                return database.bot.names[role] ? database.bot.names[role][0] : getRandomFromArray(database.bot.names.scav);
        }
    }
...
class BotItemGeneration {
            ...
            for (let i = 0; i < Object.keys(requiredSlots.required).length; i++) {
                ...
                switch (true) {
                    ...
                    case required.includes("mod_reciever"):
                        /**
                         * Recievers/dust covers not spawning on weapons
                         */
                        const mod_reciever = await this.generateWeapon(requiredSlots.required.mod_reciever, firecontrol._id, conflictIds);
                        assembledWeapon.push(...mod_reciever);
                        required = required.filter(part => part !== "mod_reciever");
                        break;

                    case required.includes("mod_gas_block"):
                        const mod_gas_block = await this.generateWeapon(requiredSlots.required.mod_gas_block, firecontrol._id, conflictIds);
                    ...
                    case required.includes("mod_barrel"):
                        /**
                         * Barrels not spawning on weapons that require them
                         */
                        const mod_barrel = await this.generateWeapon(requiredSlots.required.mod_barrel, firecontrol._id, conflictIds);
                        assembledWeapon.push(...mod_barrel);
                        required = required.filter(part => part !== "mod_barrel");
                        break;

                    case required.includes("mod_handguard"):
                        const mod_handguard = await this.generateWeapon(requiredSlots.required.mod_handguard, firecontrol._id, conflictIds);
                    ...
                    case required.includes("mod_charge"):
                        //conflictIds = [];
                        const mod_charge = await this.generateWeapon(requiredSlots.required.mod_charge, firecontrol._id, conflictIds);
                        assembledWeapon.push(...mod_charge);
                        required = required.filter(part => part !== "mod_charge");
                        break;

                    case required.includes("mod_pistol_grip"):
                        const mod_pistol_grip = await this.generateWeapon(requiredSlots.required.mod_pistol_grip, firecontrol._id, conflictIds);
                        assembledWeapon.push(...mod_pistol_grip);
                        required = required.filter(part => part !== "mod_pistol_grip");
                        break;

                    case required.includes("mod_stock"):
                        const mod_stock = await this.generateWeapon(requiredSlots.required.mod_stock, firecontrol._id, conflictIds);
    ...
    static async generateWeapon(required, parentId, conflictIds) {
        const output = [];
        ...
        const part = await this.selectWeaponPart(filtered);
     ...
     * @returns {<Promise> {}}
     */
    static async createWeaponPart(item, slotId, parentId, conflictIds) {
    ...
    static async generateWeaponPart(partId, slotId, parentId, conflictIds) {

        if (["mod_mount", "mod_scope", "mod_tactical"].includes(slotId)) {
            const randomInt = await getRandomInt();
            if (!await getPercentRandomBool(randomInt)) {
                return [];
            }
        }

        const {required, optional} = await BotUtilities.generatePartList(partId);
        const output = [];

        const partTemplate = await Item.get(partId);
        let mod_part;

        /*
        * turn below into switch
        */
        ...
        if (Object.keys(required).length > 0) {
            ...
            requires.push(parent);

            for (const id in required) {
                const require = required[id];

                if (require.filter.length === 0) break; // first break
                const filtered = await BotUtilities.cleanPartList(require.filter, conflictIds);
                if (filtered.length === 0) break; // second break

                const part = await this.selectWeaponPart(filtered);

                mod_part = await this.generateWeaponPart(
                    part,
                    require.slotId,
                    parent._id,
                    conflictIds
                ...
                requires.push(...mod_part);
            }

            output.push(...requires);
        } else if (Object.keys(optional).length > 0) {
            const options = [];
            const parent = await this.createWeaponPart(
                partTemplate,
                slotId,
            ...
            for (const id in optional) {
                const option = optional[id];

                if (option.filter.length === 0) break;
                const filtered = await BotUtilities.cleanPartList(option.filter, conflictIds);
                if (filtered.length === 0) break; // second break

                const part = await this.selectWeaponPart(filtered);

                if (["mod_mount", "mod_scope", "mod_tactical"].includes(option.slotId)) {
                    const randomInt = await getRandomInt();
                    if (!await getPercentRandomBool(randomInt)) {
                        break;
                    }
                ...
                options.push(...mod_part);
            }
            if (options.length !== 0) {
                options.push(parent, ...options);
            }
        } else {
            mod_part = await this.createWeaponPart(
                partTemplate,
                slotId,
                parentId,
                conflictIds
    ...
    static async generateBotPockets(parentId, role) {
        ...
        const pockets = await this.createWeaponPart(
            pocket,
            "Pockets",
            parentId
        );
    ...
    static async generateBotBackpack(parentId, role, backpackChance) {
        ...
        if (gearEntries && gearEntries.length > 0 && await getRandomInt() >= backpackChance) {
            ...
            const backpack = await this.createWeaponPart(
                backpackTemplate,
                "Backpack",
                parentId
            );
    ...
    static async generateSecuredContainer(parentId) {
        ...
        const securedContainer = await this.createWeaponPart(
...
class BotUtilities {
    ...
    static async cleanPartList(list, conflictIds) {
        list = list.filter(item => !conflictIds.includes(item));
        return list;
    }

    static async generatePartList(anamoly) {
        ...
        item._props.Slots.find(slot => {
            if (slot._name === "mod_magazine") return;
            ...
            // these are required for proper bot generation
                ...
                const check = ["mod_reciever", "mod_sight_rear", "mod_stock"].some(
                ...
                if (check) {
                    ...
                    if (slot._name.includes("mod_mount")) name = "mod_mount"
                    else name = slot._name;
