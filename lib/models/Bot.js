...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
         ...
         * 583990e32459771419544dd2 // AKS-74UN  // mod_mount_000 (side rail)
        ...
        if (weapon._props) {
            ...
            if (requiredSlots.mod_sight_rear) { //can make roll
                ...
                const mod_sight_rear = await this.generateRearSight(
                    rearSight,
            ...
            const mountCheck = requiredSlots?.mod_mount ? await getPercentRandomBool(50) : false;
            ...
            if (mountCheck) {
                ...
                weightedList = await BotUtilities.generateWeightedList(requiredSlots.mod_mount.filter);
                choices = await BotUtilities.generateGearEntries(weightedList);

                const sideRailMount = await getRandomFromArray(choices);
            ...
            const mod_pistol_grip = await this.generatePistolGrip(
                pistolGrip,
    ...
    static async generateMountAndOrScope(anamoly, slotId, parentId, conflictIds) {
        ...
        if (mountedOpticTemplate._parent === "55818b224bdc2dde698b456f") {
            ...
            const mod_mount = await opticMountTemplate.createAsNewItemWithParent(parentId);
            mod_mount.slotId = slotId;
            let upd = await Item.createFreshBaseItemUpd(opticMountTemplate);
            if (upd !== "error") mod_mount.upd = upd;

            conflictIds.push(...opticMountTemplate._props.ConflictingItems);
            output.push(mod_mount);

            const weightedList = await BotUtilities.generateWeightedList(opticMountSlots.mod_scope.filter);
            ...
            const mod_mount_000 = await mountedOpticTemplate.createAsNewItemWithParent(parentId);
            mod_mount_000.slotId = slotId;
            let upd = await Item.createFreshBaseItemUpd(mountedOpticTemplate);
            if (upd !== "error") mod_mount_000.upd = upd;

            conflictIds.push(...mountedOpticTemplate._props.ConflictingItems);
    ...
    static async generateAdapterAndOrScope(anamoly, slotId, parentId, conflictIds) {
        ...
        if (scopeTemplate._parent === "55818b224bdc2dde698b456f") {
            ...
            const upd = await Item.createFreshBaseItemUpd(adapterTemplate);
            if (upd !== "error") mod_adapter.upd = upd;

            conflictIds.push(...adapterTemplate._props.ConflictingItems);
            ...
            const scope = await getRandomFromArray(choices);
            const mod_scope = await this.generateScope(
                scope,
                adapterSlot.slotId,
                mod_adapter._id,
                conflictIds
    ...
    static async generateScope(scope, slotId, parentId, conflictIds) {
        const scopeTemplate = scope?._props ? scope : await Item.get(scope);
        const mod_scope = await scopeTemplate.createAsNewItemWithParent(parentId);
        mod_scope.slotId = slotId;
        let upd = await Item.createFreshBaseItemUpd(scopeTemplate);
        if (upd !== "error") mod_scope.upd = upd;

        conflictIds.push(...scopeTemplate._props.ConflictingItems);
        return mod_scope;
    }
     ...
     * @param {string} parentId 
     * @param {[string]} conflictIds 
     * @param {bool} sightChance
     * @returns {<Promise>{}}
     */
    static async generateRecieverAndOrScope(reciever, slotId, parentId, conflictIds, sightChance) {
        const output = [];

        const recieverTemplate = await Item.get(reciever);
        const mod_receiver = await recieverTemplate.createAsNewItemWithParent(parentId);
        mod_receiver.slotId = slotId;
        const upd = await Item.createFreshBaseItemUpd(recieverTemplate);
        if (upd !== "error") mod_receiver.upd = upd;
        ...
        if (sightChance) {
            ...
            if (recieverSlots.mod_scope) {
                const weightedList = await BotUtilities.generateWeightedList(recieverSlots.mod_scope.filter)
                const choices = await BotUtilities.generateGearEntries(weightedList);
                const anomaly = await getRandomFromArray(choices);
    ...
    static async generateGasBlockAndHandguard(gasblock, slotId, parentId, conflictIds) {
        ...
        if (gasblockSlots.mod_handguard) {
            ...
            const mod_handguard = await this.generateHandguard(
                handguard,
