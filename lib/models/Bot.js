...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role) {
        ...
        const weaponParts = await BotItemGeneration.generateWeaponParts(weaponTemplate);
    ...
    static async generateWeaponParts(weaponTemplate) {
        const fullWeapon = [];
        const requiredSlots = weaponTemplate._props.Slots.filter(slot => slot._required);
        const optionalSlots = weaponTemplate._props.Slots.filter(slot => !slot._required);
        for (const slot of requiredSlots) {
            const weightedListPart = await BotUtilities.generateWeightedList(slot._props.filters[0].Filter);
            console.log()
        }
    }
