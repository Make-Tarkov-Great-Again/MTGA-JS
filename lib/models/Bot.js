...
class Bot extends BaseModel {
    ...
    async generatePocketLoot(parentId){
        const pocketLoot = {
             "salewa":{
                "_id": await generateMongoID(),
                "_tpl": "544fb45d4bdc2dee738b4568",
                "parentId": parentId,
                "slotId": "Medical Supplies"
             },
             "conflicts": [],

             "core_medical_surgical_kit":{
                 "_id": await generateMongoID(),
            ...
            this.getRandomItemFromArray(pocketLoot);

            if(pocketLoot[randomIndex] == salewa){
                this.getSlot(salewa, 1, 2);
                output.push(salewa);
            }

            if(pocketLoot[randomIndex] == core_medical_surgical_kit){
               this.getSlot(core_medical_surgical_kit, 2, 1);
                output.push(core_medical_surgical_kit);
            }

            if(pocketLoot[randomIndex] == item_food_mre){
               this.getSlot(item_food_mre, 1, 2);
               output.push(item_food_mre);
            }
        
    }

    async getSlot(_item, _width, _height){
        await Item.getFreeSlot(_item, _width, _height);
    }

    async getRandomFromArray(itemArray){
        // get random index value
        const randomIndex = Math.floor(Math.random() * itemArray.length);

       // get random item
       const item = pocketLoot[randomIndex];
