...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const scabbard = await this.generateScabbard(parentId, role);
        if (scabbard)
        ...
        const ammo = await BotItemGeneration.generateBotAmmo(botRig, securedContainer, compatibleMagazines, compatibleAmmo);
        if (ammo)
            output.push(...ammo);
        ...
        const helmet = await BotItemGeneration.generateBotHeadwear(parentId, role);
        if (helmet)
            output.push(helmet);
...
class BotItemGeneration {
    ...
    /**
     * Generate a weapon preset based on the gear entries for bot role, load the magazine, create a list of compatible mags & ammo
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (!gearEntries.primaryWeapon) {
            logger.error("[BotItemGeneration.generateBotGear.generateBotWeapon] Bot loadout doesn't have any primary weapon available.");
            return false;
        }
        ...
        const fullWeapon = await BotItemGeneration.generateFullWeaponPresetWithoutMagazine(weaponPreset, parentId, slotId);
        return [fullWeapon, compatibleMagazines, compatibleAmmo];
    }

    static async generateFullWeaponPresetWithoutMagazine(weaponPreset, parentId, slotId) {
        const fullWeapon = [];
        for (const part of cloneDeep(weaponPreset._items)) {
            if (!part.slotId) {
                part.parentId = parentId;
                part.slotId = slotId;
                fullWeapon.push(part);
    ...
    static async generateFullWeapon(weaponTemplate, parentId) {
        const receiver = await weaponTemplate.createAsNewItem();
        receiver.parentId = parentId;
        receiver.slotId = "FirstPrimaryWeapon";
    ...
    static async generateWeaponParts(part, conflictIds) {
        const parts = [];
    ...
    static async generateBotFacecover(parentId, role) {
     ...
     */
    static async generateBotHeadwear(parentId, role) {
        const gearEntries = database.bot.bots[role].gearEntries.headwear;
        if (gearEntries && gearEntries.length > 0) {
    ...
    static async generateBotBodyArmor(parentId, role, rigTpl) {
        ...
        if (gearEntries && gearEntries.length > 0 && !rig._props.BlocksArmorVest) {
        ...
        logger.error("[BotItemGeneration.generateBotGear.generateBotBodyArmor] Bot loadout doesn't have any available body armor.");
        return false;
    }
    ...
    static async generateBotRig(parentId, role) {
        ...
        if (gearEntries.armoredVest.length > 0 && gearEntries.vest.length > 0) {
            ...
            logger.error("[BotItemGeneration.generateBotGear.generateBotRig] Bot loadout doesn't have any vest and amored vest available.");
            return false;
        }
    ...
    static async generateBotPockets(parentId, role) {
        ...
        if (authorizedGear.pocket && authorizedGear.pocket.length > 0)
            return {
                "_id": await generateMongoID(),
                "_tpl": await getRandomFromArray(authorizedGear.pocket),
                "parentId": parentId,
                "slotId": "Pockets"
            };
    ...
    static async generateBotBackpack(parentId, role) {
        ...
        if (gearEntries.backpack.length > 0 && await getRandomInt() > 25) {
...
class BotUtilities {
    ...
    static async generateWeightedList(gearList) {
        ...
        for (const item of itemsWithPrices) {
            percentageList[item.itemId] = await Math.round(item.price / totalPrice * 100);
        }
