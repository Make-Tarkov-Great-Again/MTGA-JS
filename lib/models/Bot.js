...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(inventory, role) {
        ...
        //const head = await generateHeadAccessory(parentId, role),
        //const body = await generateBodyAccessory(parentId, role),
        //const backpack = await generateBackpack(parentId, role),
        //const securedContainer = await generateSecuredContainer(parentId, role),

        //const weapons = await this.generateWeaponInSlot(parentId),

        finalResult.push(...weapons, pockets, inventory);
        return finalResult;
    ...
    async generateChildren(parent, children, parentId) {
        ...
        for (const child of children) {
            if (child.parentId === parent._id) {
                ...
                input.push(newChild);
                if (grandchildren) input.push(...grandchildren);
            }
        }
