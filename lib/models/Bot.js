...
class BotItemGeneration {
    ...
    static async generateBotBodyArmor(botInfo, parentId, role) {
        ...
        if (bodyArmor && bodyArmor.length > 0) {
            const armorTemplate = await Item.get(await getRandomFromArray(bodyArmor));
    ...
    static async generateBotVest(botInfo, parentId, role) {
        ...
        if (vest && vest.length > 0) {
            const vestTemplate = await Item.get(await getRandomFromArray(vest));
    ...
    static async generateBotMelee(botInfo, parentId, role) {
        ...
        const scabbard = melee && melee.length > 0
            ? await getRandomFromArray(melee)
    ...
    static async generateBotWeapon(botInfo, parentId, role, slotId, ammos = [], magazines = {}) {
        ...
        const choice = weightedGear.length === 1 ? weightedGear[0] : await getRandomFromArray(weightedGear);
        ...
        if (!botInfo.weaponCache[choice]["parts"]) {
            ...
            const cache = await readParsed(`./assets/database/bot/weaponcache.json`);
            Object.assign(cache, botInfo.weaponCache[choice]);
