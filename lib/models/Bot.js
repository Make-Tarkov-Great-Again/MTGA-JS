...
const inventoryConflicts = readParsed("./conflicts.json");
...
class Bot extends BaseModel {
    ...
    async generateInventory(role) {
        ...
        //initiate divide
        const conflicts = await BotUtilities.defaultInventoryConflicts();
        const botDefaultInventory = await this.generateInventoryItems(
            templateInventory.equipment, role, conflicts);
    ...
    async generateInventoryItems(parentId, role, conflicts) {
        const finalResult = [];

        // independent slots //

        //const dogtag = await this.generateDogtag(parentId, conflicts);
        //const armband = await this.generateArmband(parentId, conflicts);
        const scabbard = await this.generateScabbard(parentId, conflicts);

        // depends on a dynamic item generation function
        const pockets = await this.generatePockets(parentId, role, conflicts); // createContainerMap()
        const head = await this.generateHeadAccessory(parentId, role, conflicts);
        const body = await this.generateBodyAccessory(parentId, role, conflicts);
        const securedContainer = await this.generateSecuredContainer(parentId, role, conflicts);

        const weapons = await this.generateWeaponInSlot(parentId, conflicts);
    ...
    async generateHeadAccessory(parentId, role = null, conflicts) {
        const output = [];
    ...
    async generateBodyAccessory(parentId, role = null, conflicts) {
        const output = [];
    ...
    async generateWeaponInSlot(parentId, slot = null) {
        ...
        const weapon = await this.generateChildren(
            test[0],
            test,
            newId
        );
...
class BotUtilities {
    ...
    static async defaultInventoryConflicts() {
        return [
    ...
    static async getFilters(item) {
        const conflicts = [];
        const data = await Item.get(item);
        if (data._props.Slots) {
            for (const slot of data._props.Slots) {
                for (const filter of slot._props.filters) {
                    if (filter.Filter) {
                        conflicts.push(...filter.Filter);
                    }
                }
            }
        }
