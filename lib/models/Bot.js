...
class Bot extends BaseModel {
    ...
    static async getBotsForDuplicatedIds(bots) {
        const cache = [];
        const output = [];
        ...
        for (const [key, bot] of Object.entries(bots)) {
            for (const item of bot.Inventory.items) {
                if (cache.includes(item._id)) {
                    logger.error("Duplicate Found!");
                    const replacement = await this.generateBots({
                        Role: bot.Info.Settings.Role,
                        Limit: 1,
                        Difficulty: bot.Info.Settings.BotDifficulty
                    })
                    if (replacement) {
        ...
        logger.warn(`${duplicates} duplicates found`);
        if (output.length === 0)
            return false;
        return output;
    }
...
class BotItemGeneration {
    ...
    static async gearEntriesFail(role, slotId) {
        logger.warn(`[generateBotWeapon] Bot loadout doesn't have any ${slotId} weapon available., using "assault" as fallback`);
        return
    }
    ...
    static async generateBotWeapon(botInfo, parentId, role, slotId) {
        ...
        const choice = weightedGear.length === 1
            ? weightedGear[0]
            : await getRandomFromArray(weightedGear);


        if (!botInfo.weaponCache.weapons[choice]) {
            console.log("im going to say it")
        }
    ...
    static async assembleWeapon(botInfo, choice, slotId, parentId) {
        ...
        const required = await cloneDeep(botInfo.weaponCache.weapons[choice].Required);
        ...
        assembledWeapon.push(firecontrol, ...patron_in_weapon);

        // generate mod magazine

        const conflicts = [];
        const optional = [];
        for (const slot in baseParts) {
            if (Object.keys(required).includes(slot)) {
                switch (slot) {
                    case "mod_magazine":
                        const [mod_magazine, magazine] = await this.generateModMagazine(base._props.ReloadMode, firecontrol._id, baseParts.mod_magazine, ammo);

                        assembledWeapon.push(...mod_magazine);
                        if (cachedSlots[slot][magazine].hasOwnProperty("ConflictingItems")) {
                            conflicts.push(cachedSlots[slot][magazine].ConflictingItems);
                        }
                        required[slot] = true;
                        break;

                    case "mod_stock":
                        console.log("we have to do some niggering here");
                        break;

                    default:
                        // if slot in required = make it, else push into optional?????
                        const cachedSlotData = await this.slotTranslator(cachedSlots, slot);
                        const mod = await this.generateRequiredSlot(firecontrol._id, slot, baseParts[slot], cachedSlotData, required, conflicts);
                ...
                const cachedSlotData = await this.slotTranslator(cachedSlots, slot);
                await this.generateRequiredSlot(firecontrol._id, slot, baseParts[slot], cachedSlotData, optional, required, conflicts);
                break;
            }
    ...
    static async generateRequiredSlot(firecontrolID, slot, choices, cachedSlotData, required, conflicts) {
        const output = []
        const selection = await this.selectWeaponPart(Object.keys(choices), conflicts);

        const part = await this.createWeaponPart(selection, slot, firecontrolID, conflicts);
        if (required.hasOwnProperty(slot) && required[slot] === false) {
            required[slot] = true;
        }

        if (cachedSlotData[selection]) {
            if (cachedSlotData[selection].hasOwnProperty("ConflictingItems")) {
                conflicts.push(...cachedSlotData[selection].ConflictingItems);
            }
            if (cachedSlotData[selection].hasOwnProperty("Parts")) {
    ...
    static async slotTranslator(slots, slot) {
        switch (slot) {
            case "camora_000":
            case "camora_001":
            case "camora_002":
    ...
    static async getAmmos(weapon, cachedWeapon) {
        ...
        if (!cachedWeapon?.Ammo) {
            ...
            if (!read[weapon._id].hasOwnProperty("Ammo")) {
                read[weapon._id].Ammo = [];
                if (cachedWeapon.Ammo) {
                    read[weapon._id].Ammo.push(...cachedWeapon.Ammo);
    ...
    static async generateModMagazine(ReloadMode, parentId, magazines, ammo) {
        ...
        if (ReloadMode !== "OnlyBarrel") {
            const selected = await this.selectWeaponPart(Object.keys(magazines))

            // add magazine to weapon, and fill with ammo
            const anamoly = await Item.get(selected);
            const mod_magazine = await this.createWeaponPart(
                anamoly, "mod_magazine", parentId);
            ...
            if (anamoly?._props?.ReloadMagType === "InternalMagazine") {
                ...
                if (camoras.length !== 0) {
                    ...
                    cartridges.upd.StackObjectsCount = anamoly._props.Cartridges[0]._max_count;
                    output.push(cartridges);
                }
    ...
    static async checkAssembledWeapon(assembledWeapon, required, optional, conflictIds) {

        const missingParts = [];
        for (const id in required) {
            const require = required[id]
            const part = assembledWeapon.find(x => x.slotId === require.slotId);
            if (part) {
                continue;
            } else {
                const parent = assembledWeapon.find(x => ["FirstPrimaryWeapon", "Holster", "SecondaryWeapon"].includes(x.slotId));
                const requiredPart = await this.selectWeaponPart(require.filter);
                const missingRequired = await this.createWeaponPart(
                    requiredPart,
                    require.slotId,
                    parent._id,
                    conflictIds
                );
                missingParts.push(missingRequired);
                logger.error(`${require.slotId} was missing because your code is shit and you need checks!!!!!!`)
            }
        }
        return [...assembledWeapon, ...missingParts];
    }
    ...
    static async createWeaponPart(itemId, slotId, parentId) {
        ...
        const item = itemId?._props ? itemId : await Item.get(itemId);
        if (!item) {
     ...
     * @returns 
     */
    static async generateWeaponPart() {
        const output = [];
     ...
     */
    static async selectWeaponPart(filter, conflicts = null) {
        if (conflicts)
            filter = filter.filter(item => !conflicts.includes(item));
