...
class Bot extends BaseModel {
    ...
    static async getBotsForDuplicatedIds(bots) {
        ...
        logger.debug(`${duplicates} duplicates found`);
        if (output.length === 0)
            return false;
        return output;
    }
...
class BotItemGeneration {
    ...
    static async generateBotWeapon(botInfo, parentId, role, slotId, ammos = [], magazines = {}) {
        ...
        if (!assembledWeapon) {
            logger.error("ballsack")
        }

        await this.checkAssembledWeapon(assembledWeapon, required, optional, conflictIds);
            ...
            if (slot._required === true) {
                switch (true) {
                    ...
                    case slot._name.includes("mod_stock"):
                        name = "mod_stock";
                        break;
                    case slot.name.includes("mod_pistolgrip"):
                        name = "mod_pistolgrip"
                        break;
                    default:
                        name = slot._name;
