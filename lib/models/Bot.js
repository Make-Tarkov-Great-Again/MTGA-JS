...
class Bot extends BaseModel {
    ...
    static async generateBots(request, _reply) {
        ...
        const generatedBots = [];
        let aid = 0;
        for (const botParameter in botsParameters) {
            const { Role, Limit, Difficulty } = botsParameters[botParameter];
            
            for (let i = aid; i < Limit; i++) {
                const newBot = new Bot();
                
                    await newBot.setIds(await generateMongoID(), i);
