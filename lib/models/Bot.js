...
class Bot extends BaseModel {
    ...
    async generateBodyAccessory(parentId, role) {
        return this.generateVest(
            parentId,
            await ItemNode.getNodeChildren("5448e5284bdc2dcb718b4567"),
            role
        )
    }

    async generateVest(parentId, vests, role = null) {
        const output = [];
        const vest = await getRandomFromArray(vests);

        if (vest && await getPercentRandomBool(25)) {
            const armorVest = await getRandomFromArray(
                await ItemNode.getNodeChildren("5448e54d4bdc2dcc718b4568")
            );

            if (!vest._props.ConflictingItems.includes(armorVest._id) &&
    ...
    async generateScabbard(parentId, role = null) {
        const item = await ItemNode.get("5447e1d04bdc2dff2f8b4567")
        const scabbard = await getRandomFromArray(
            await item.getAllItemsChildren()
        );
        return {
            "_id": await generateMongoID(),
            "_tpl": scabbard._id,
