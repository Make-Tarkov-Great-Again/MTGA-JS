...
class Bot extends BaseModel {
    ...
    static async generateBots(request, _reply) {
        ...
        writeFile("../../generatedBots.json", stringify(generatedBots), false);
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const balle = await BotItemGeneration.generateBotWeapon(parentId, role, "FirstPrimaryWeapon");
        return output;
    }
}
...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            // mod_reciever
            requiredSlots["mod_reciever"].filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
            ...
            /*             const recieverSlots = await this.genereateListOfSlots(await Item.get(reciever));
                        if (recieverSlots) {
                            // do rolls for slots
            ...
            if (requiredSlots["mod_gas_block"]) {
                requiredSlots["mod_gas_block"].filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
                ...
                const mod_gas_block = await this.generateGasBlock(gasblock, requiredSlots["mod_gas_block"].slotId, firecontrol._id, conflictIds);
            ...
            if (requiredSlots["mod_handguard"]) {
                requiredSlots["mod_handguard"].filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list

                weightedList = await BotUtilities.generateWeightedList(requiredSlots["mod_handguard"].filter);
                choices = await BotUtilities.generateGearEntries(weightedList);

                const handguard = await getRandomFromArray(choices);
                const mod_handguard = await this.generateHandguard(handguard, requiredSlots["mod_handguard"].slotId, firecontrol._id, conflictIds);
            ...
            /* const pistolGripSlots = await this.genereateListOfSlots(await Item.get(reciever));
            if (pistolGripSlots) {
                // do rolls for slots
                // assembledWeapon.push(...slots);
            } */
            ...
            // mod_stock
            requiredSlots["mod_stock"].filter.filter(mod => !conflictIds.includes(mod)); // remove conflictIds from requiredSlots list
            ...
            const mod_stock = await this.generateStock(stock, requiredSlots["mod_stock"].slotId, firecontrol._id);
    ...
    static async generatePatronInWeapon(patronNames, parentId, ammoId) {
        ...
        for (const name of patronNames) {
            ...
            patron.upd = await Item.createFreshBaseItemUpd(patronTemplate);
            patron.slotId = name;
    ...
    /**
     * Generate gasblock for weapon, return object and updated conflictIds
     ...
     * @returns {<Promise>{}}
     */
    static async generateGasBlock(gasblock, slotId, parentId, conflictIds) {
        const gasblockTemplate = await Item.get(gasblock);
        const output = await gasblockTemplate.createAsNewItemWithParent(parentId);
        output.slotId = slotId;
    ...
    /**
     * Generate stock and/or buffer-tube/adapter, return array of objects
     ...
     * @returns {<Promise>[{}]}
     */
    static async generateStock(anomaly, slotId, parentId) {
        const output = [];

        let stockTemplate = await Item.get(anomaly);
        let adapterTemplate;

        if (["_adapter", "_lock", "_stock_tube", "com_spec_std"].includes(anomaly._name || anomaly._props.Name)) {
            ...
            const adapterSlots = await this.genereateListOfSlots(adapterTemplate);
            let weightedList = await BotUtilities.generateWeightedList(adapterSlots["mod_stock_000"].filter);
            ...
            mod_stock.slotId = adapterSlots["mod_stock_000"].slotId
    ...
    static async generateListOfRequiredPartsForWeapon(props) {
        ...
        slots.find(slot => {
            if (slot._required === true) {
                if (["mod_pistol_grip"].includes(slot._name)) name = "mod_pistol_grip"
            ...
            else if (["mod_reciever", "mod_sight_rear", "mod_stock", "mod_muzzle"].includes(slot._name)) {
                if (["mod_stock"].includes(slot._name)) name = "mod_stock"
                else name = slot._name;
