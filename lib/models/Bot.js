...
class BotItemGeneration {
    ...
    static async generateBotWeapon(parentId, role, slotId) {
        ...
        if (weapon._props) {
            ...
            const mod_stock = await this.generateStockAndOrAdapter(stock, requiredSlots.mod_stock.slotId, firecontrol._id);
    ...
    static async generateGasBlockAndHandguard(gasblock, slotId, parentId, conflictIds) {
        ...
        if (gasblockSlots.mod_handguard){
            weightedList = await BotUtilities.generateWeightedList(gasblockSlots.mod_handguard.filter);
            choices = await BotUtilities.generateGearEntries(weightedList);
     ...
     * @returns {<Promise>[{}]}
     */
    static async generateStockAndOrAdapter(anomaly, slotId, parentId) {
        const output = [];

        let stockTemplate = await Item.get(anomaly);
        let adapterTemplate;

        const check = stockTemplate._name.indexOf(
            "_adapter" || "_lock" || "_stock_tube" || "_com_spec_std" || "caa_akts"
        )
    ...
    static async generateListOfRequiredPartsForWeapon(props) {
        ...
        slots.find(slot => {
            if (slot._required === true) {
                if (slot._name.indexOf("mod_pistol_grip") !== -1) name = "mod_pistol_grip"
            ...
            else if (slot._name.indexOf(["mod_reciever", "mod_sight_rear", "mod_stock", "mod_muzzle"]) !== -1) {
                if (slot._name.indexOf("mod_stock") !== -1) name = "mod_stock"
                else name = slot._name;
