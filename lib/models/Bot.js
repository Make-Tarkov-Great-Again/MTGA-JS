...
class Bot extends BaseModel {
    ...
    async generateInventoryItems(parentId, role) {
        ...
        const weapons = await this.generateWeaponInSlot(parentId, role);
    ...
    async generateWeaponInSlot(parentId, role, slot = null) {
        ...
        const gun = await Item.get(test[0]._tpl); // primary weapon information
        ...
        // get ammo for the weapon // use ammoCaliber for ammo selection   
        const ammo = await this.generateAmmo(gun, role);
        const chambers = await BotUtilities.getChambers(gun);
        const insertMagazine = await this.generateCartridge(test[0], ammo)
        ...
        } else weapon.push(...insertMagazine);
    ...
    async generateCartridge(gun, ammo, role = null) {
        const loadedCartridge = [];

        const cartridges = await BotUtilities.getSlots(gun._tpl, "mod_magazine");
        // need to create lootbucket for magazines/cartridges
        let cartridge = await getRandomFromArray(cartridges);
        ...
        cartridge = await BotUtilities.generateCartridgeData(cartridge);
        const loadCartridge = await this.generateLoadedCartridge(cartridge, ammo, data._id);

        loadedCartridge.push(data, loadCartridge);
     ...
     * @returns 
     */
    async generateCartridges(cartridge, ammo) {
        const output = [];
        // create loop to create multiple cartridges
        const data = {
            "_id": await generateMongoID(),
            "_tpl": cartridge,
            "parentId": gun._id,
        ...
        cartridge = await BotUtilities.generateCartridgeData(cartridge);
        const loadCartridge = await this.generateLoadedCartridge(cartridge, ammo, data._id);
        output.push(data, loadCartridge);
        return output;
    }

    async generateLoadedCartridge(data, ammo, parentId) {
        return {
            "_id": await generateMongoID(),
            "_tpl": ammo,
            "parentId": parentId,
