const { logger, generateMongoID, wipeDepend, getRandomInt } = require("../utilities");
 ...
 */
  ...
  async Helper_AmmoBoxFiller(template, generatedBsgLootSpawnStruct){
    // is that ammo box?
    if(template._parent == "543be5cb4bdc2deb348b4568")
    {
      const ammoTplName = template._props.StackSlots[0]._props.filters[0].Filter[0];
      const ammoTpl = await Item.get(ammoTplName);
      if(ammoTpl != false){
        let ammoItem = await ammoTpl.createAsNewItem();
        // generate random stack from 0 to max possible
        const genAmmoCount = await getRandomInt(1, template._props.StackSlots[0]._max_count); // use server config to change that behaviour later on!!!
        ammoItem["upd"] = { "StackObjectsCount": genAmmoCount };
        ammoItem["parentId"] = generatedBsgLootSpawnStruct.Items[0]._id;
        ammoItem["slotId"] = "cartridges";
        generatedBsgLootSpawnStruct.Items.push(ammoItem);
      }
    }
  }
/**
 ...
 */
  async generateContainerContents(data) {
    ...
    const itemToSpawn = await getRandomInt(0, containerHeight); // for now lets use the height as max items to spawn
    ...
    for (let i = 0; i < itemToSpawn; i++) {
      const roll = await getRandomInt(0, 50);
      let smallList = [];
      ...
      if (smallList.length > 0) {
        const itemToAdd = smallList[await getRandomInt(0, smallList.length - 1)];
        ...
        await this.Helper_AmmoBoxFiller(SpawnList[itemToAdd], data);
      }
    }
    return data;
  }
 ...
 */
  ...
  async gll_loopWalker(lootSpawn, output) {
    ...
    const chooseRandomIndex = await getRandomInt(0, ItemSpawnTable.length - 1); // generate random location of spawned item from table above
    ...
    if (generatedBsgLootSpawnStruct.Items.length == 0) {
      ...
      await this.Helper_AmmoBoxFiller(template, generatedBsgLootSpawnStruct);
