 ...
 */
  async generateContainerContents(data) {
    ...
    if(containerTable == null){
      logger.error(`Container: ${containerId} not found in conrainersSpawnData.json`);
      return;
    }
    ...
    for(let i = 0; i < itemToSpawn; i++)
    {
      ...
      if(smallList.length > 0){
        ...
        if (!freeSlot) {
            await logger.debug(`Unable to add item ${itemToAdd}. No space. Already added ${i}`);
            break;
        }
 ...
 */
  ...
  async generateStaticLoot(output) {
    ...
    logger.debug(">>FINISHED CONTAINERS GENERATION");
    //return output;
  }
/**
 ...
 * @param {*} output response to client
 */
//TODO: do same like loose loot and containers move forloops insides into the function and promise it all
 ...
 */
  async generateLooseLoot(output) {
    ...
    {
      spawnPromises.push(this.gll_loopWalker(lootSpawn, output));
    }
    await Promise.all(spawnPromises)
    logger.debug(">>FINISHED DYNAMIC GENERATION");
