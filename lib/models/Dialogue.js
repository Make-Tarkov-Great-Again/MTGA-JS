...
class Dialogue extends BaseModel {
    ...
    static async generateMailDialogListDialogue(dialogue) {
        ...
        if (output.type === 2) output["message"] = this.traderMessage(message);
        if (output.type === 1) output["message"] = this.playerMessage(message);


        return output;
    }

    static async traderMessage(dialogue) {
        return {
            dt: message.dt,
            type: message.type,
    ...
    static async playerMessage(dialogue) {
        return {
            dt: message.dt,
            type: 1,
            text: message.text,
