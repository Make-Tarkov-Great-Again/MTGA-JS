...
const { logger, getCurrentTimestamp, generateMongoID, round, min } = require("../../utilities");
...
class Character extends BaseModel {
    ...
    async solve() {
        ...
        if (typeof this.Customization !== undefined) {
        ...
        switch (containerType) {
            case "hideout":
                ...
                if (containerID === stashContainer._id) {
                    ...
                    logger.error(`Move request failed: Invalid container with ID ${containerID}`);
                    return false;
                }
            ...
            default:
                logger.debug(`Trying to move item to equipment slot ${containerType}`);
                if (await this.getInventoryItemByID(containerID)) {
    ...
    async moveItemUsingSlotID(itemId, locationData, slotId, containerID) {
        if (!itemId) {
            logger.error("Move request failed: No itemId")
            return false;
        }

        const itemSearch = await this.getInventoryItemByID(itemId);
        if (itemSearch) {
            logger.debug(`Located item with item ID ${itemId}`);
        ...
        logger.debug(`Unable to locate item with item ID ${itemId}`);
        return false;
    }
    ...
    async mergeItem(mergedStackId, destinationStackId) {
        ...
        if (destinationItem) {
            ...
            logger.error(`Merge request: couldn't find merged stack ${mergedStackId}`);
            return false;
        }
        logger.error(`Merge request: couldn't find destination stack ${destinationStackId}`);
        return false;
    }
    ...
    // Hideout - consumption rate is per second //
    async hideoutTick(specialData) {
        ...
        return !!(await this.hideoutCalculateFuelUsage(cyclicUpdateDifference));
    }
        ...
        if(await this.isGeneratorActive()) {
            ...
            if(fuelContainer) {
                ...
                if(typeof fuelContainer.upd.Resource === "undefined") {
                    ...
                    if(templateItem) {
                        fuelContainer.upd.Resource = templateItem._props.Resource;
                    } else {
                ...
                //logger.debug(`[hideoutTick] Calculating fuel consumption: Available Fuel [${fuelContainer.upd.Resource}] - time since last update in seconds [${cyclicUpdateDifference}] * (generator fuel flow rate [${database.core.hideoutSettings.generatorFuelFlowRate}] * consumption multiplier [${consumptionMultiplier}])`)
                
                let fuelAfterConsumption = fuelContainer.upd.Resource - cyclicUpdateDifference * (database.core.hideoutSettings.generatorFuelFlowRate * consumptionMultiplier);
                if(fuelAfterConsumption > 0) {
                    fuelContainer.upd.Resource = fuelAfterConsumption;
                } else {
    ...
    async removeHideoutProductionById(recipeId) {
        this.Hideout.Production.splice(recipeId, 1)
    }
