...
const { logger, getCurrentTimestamp, generateMongoID, round, min, floor, cloneDeep, stringify } = require("../utilities");
...
class Character extends BaseModel {
    ...
    async getPlayerSkill(skillId) {
        const playerSkill = this.Skills.Common.find(iteration => iteration.Id === skillId);
        return playerSkill ? playerSkill : false;
    }
    ...
    async isPlayerSkillLevelElite(skillId) {
        return !!(await this.getPlayerSkillLevel(skillId) === 51);
    }
    ...
    async saveCharacterRaidProgression(raidData) {
        await this.setStats(raidData.profile.Stats);
        this.SurvivorClass = raidData.profile.SurvivorClass;
        this.Info.Level = raidData.profile.Info.Level;
        await this.setExperience(raidData.profile.Info.Experience);
        await this.setQuests(raidData.profile.Quests);
        await this.setConditionCounters(raidData.profile.ConditionCounters);
        await this.setEncyclopedia(raidData.profile.Encyclopedia);
        await this.setTradersInfo(raidData.profile.TradersInfo);

        await this.setUnlockedInfo(raidData.profile.UnlockedInfo);

        await this.setHideout(raidData.profile.Hideout);
        await this.setSkills(raidData.profile.Skills);
    ...
    async moveItem(item) {
        ...
        Item ID: ${item.item}
        Location Data:`);
        logger.debug(stringify(item.to.location));
    ...
    async updateCharacter() {
        ...
        const check = (hideout.wallUnlockInSeconds + this.Info.RegistrationDate) < await getCurrentTimestamp()
        if (check) {
