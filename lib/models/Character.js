...
class Character extends BaseModel {
    ...
    async questsFix(quests) {
        ...
        for (const quest of quests) {
            output.push(await Quest.generateQuestModel(quest));
        }
        return output;
    }
    ...
    async solve() {
        ...
        if (this.Quests.length > 0 ){
            for (const [index, quest] of Object.entries(this.Quests)) {
                this.Quests[index] = await UtilityModel.createModelFromParse("HideoutArea", quest);
            }
        }
    ...
    async moveItem(item) {
        ...
        Item ID: ${item.item}
        Location Data:`);
        await logger.debug(stringify(item.to.location));
