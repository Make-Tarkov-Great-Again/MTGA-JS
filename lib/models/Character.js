...
class Character extends BaseModel {
    ...
    async setHealth(healthData) {
        ...
        for (const bodyPart in this.Health.BodyParts) {
            this.Health.BodyParts[bodyPart].Health.Current = await round(healthData.Health[bodyPart].Current);
    ...
    async setExperience(experiencePoints) {
        return this.Info.Experience = experiencePoints;;
    }
    ...
    async solve() {
        ...
        if (this._id)
            logger.debug(`Solving Character with ID: ${this._id}`);
    ...
    async dissolve() {
        //logger.debug(`Dissolving Character with ID: ${this._id}`);
    ...
    async addItem(container, itemId, children = undefined, amount = 1, foundInRaid = false, customUpd = false) {
        ...
        for (let itemsToAdd = 0; itemsToAdd < stackAmount; itemsToAdd++) {
            if (amount > 0) {
                ...
                if (freeSlot) {
                    ...
                    noSpace = true;
                    logger.debug(`Unable to add item ${itemId}. No space.`);
                    break;
                }
            }
        }
        ...
        if (noSpace) {
            ...
            logger.debug(`[addItem] Unable to add item ${itemId}. Unknown cause.`);
            return false;
        }
    }
    ...
    async moveItem(item) {
        logger.debug(`Move request with params:
        Container ID: ${item.to.id}
        Container Type: ${item.to.container}
        Item ID: ${item.item}
        Location Data:`);
        logger.debug(item.to.location);

        switch (item.to.container) {
            case "hideout":
                logger.debug(`Trying to move item to/in hideout`);
            ...
            case "main":
                logger.debug(`Trying to move item to/in main`);
            ...
            case "cartridges":
                logger.debug(`Trying to move item to/in cartridges`);
                if (await this.getInventoryItemByID(item.to.id)) {
            ...
            default:
                logger.debug(`Trying to move item to equipment slot ${item.to.container}`);
                if (await this.getInventoryItemByID(item.to.id)) {
    ...
    async moveItemWithinProfile(item) {
        ...
        if (itemSearch) {
            logger.debug(`Located item with item ID ${item.item}`);
        ...
        logger.debug(`Unable to locate item with item ID ${item.item}`);
        return false;
    }
    ...
    async splitItem(itemId, splitStackCount, slotId, containerId, location) {
        logger.debug(`Split request with params:
    ...
    async mergeItems(item) {
        ...
        let mergedItem;
        if (item?.fromOwner?.type === "Mail") {
            mergedItem = await this.mergeItem(item, ...await this.retrieveRewardItems(item));
        } else mergedItem = await this.mergeItem(item)
    ...
    async mergeItem(item, reward = null) {
        ...
        if (destinationItem) {
            ...
            let mergedItem;
            if (reward) mergedItem = reward; // need to account for mail
            else mergedItem = await this.getInventoryItemByID(item.item);
    ...
    async hideoutTick(specialData) {
        ...
        if (typeof specialData.lastCyclicUpdate !== "undefined") {
            cyclicUpdateDifference = (Date.now() - specialData.lastCyclicUpdate) / 1000;
        }
            ...
            if (fuelContainer) {
                ...
                if (typeof fuelContainer.upd.Resource === "undefined") {
                    ...
                    if (templateItem) {
                        ...
                        logger.debug(fuelContainer);
                    }
                }

                let consumptionMultiplier = 1
                const solarPower = await this.getHideoutAreaByType(18)
                if (solarPower?.level > 0) {
                    consumptionMultiplier = 0.5
                }

                const { database: { core: { hideoutSettings: { generatorFuelFlowRate } } } } = require("../../app");
                //logger.debug(`[hideoutTick] Calculating fuel consumption: Available Fuel [${fuelContainer.upd.Resource.Value}] - time since last update in seconds [${cyclicUpdateDifference}] * (generator fuel flow rate [${database.core.hideoutSettings.generatorFuelFlowRate}] * consumption multiplier [${consumptionMultiplier}])`)
                const cycleConsumption = cyclicUpdateDifference * (generatorFuelFlowRate * consumptionMultiplier);
    ...
    async hideoutBitcoinProduction(cyclicUpdateDifference) {
        ...
        const maxBitcoins = await this.isPlayerSkillLevelElite("HideoutManagement") ? 5 : 3;

        const [graphicCards, storedBitcoins] = [bitcoinFarm.slots.length, bitcoinProduction.Products.length];
    ...
    async applyHideoutBonus(bonus) {
        ...
        logger.debug(`Bonus ${bonus.type} added to character ${this._id}.`)
        logger.debug(bonus);
