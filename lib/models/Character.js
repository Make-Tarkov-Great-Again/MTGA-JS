...
class Character extends BaseModel {
    ...
    async setHealth(healthData) {
        ...
        for (const bodyPart in this.Health.BodyParts) {
            this.Health.BodyParts[bodyPart].Health.Current = healthData.Health[bodyPart].Current;
    ...
    async generateListItemsInStash() {
        const listItemsStash = [];
        const stashID = await this.getStashContainer();
        let parentItems = await this.getInventoryItemsByParent(stashID._id);
        while (parentItems.length > 0) {
            const childrenList = [];
            listItemsStash.push(...parentItems);
