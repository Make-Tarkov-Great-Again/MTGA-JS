...
class RagfairOffer extends BaseModel {
    ...
    async loadOffer(offerNumber, itemData, user, offerEnd, buyRestriction = false) {
        ...
        await this.setCosts(Number(itemData.barter[0].count));
        await this.setOfferDuration(await getCurrentTimestamp(), offerEnd);
    ...
    async refreshTraderOffer(trader) {
        await this.setOfferDuration(await getCurrentTimestamp(), trader.assort.nextResupply);
    }
}
