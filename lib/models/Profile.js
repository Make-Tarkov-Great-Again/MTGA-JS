...
class Profile extends BaseModel {
    ...
    */
    ...
    async createMessage(id, message, rewards, stashItems) {
        const output = {
     ...
     */
    async addDialogue(id, contents, sessionID, rewards = null) {

        let dialogue = await this.getDialogue(id);
        if (!dialogue) dialogue = await this.createDialogue(id)
        ...
        const message = await this.createMessage(id, contents, rewards, stashItems);
        if (!message) logger.error(`createMessage failed`);
    ...
    async addInsuranceNotification(message) {
    ...
    async processMailbox() {
        if (await this.checkNotificationQueue) {
            ...
            for (const notification of notifications) {
                if (await notificationHandler.sendNotification(notification)) {
    ...
    async processInsuranceReturn() {
        if (await this.checkInsuranceNotifications()) {
            ...
            for (const insured of insurance) {
                ...
                await this.removeNotificationFromQueue();
                await this.addDialogue(insured.tid, insured.messageContent, this.id, insured.items);
