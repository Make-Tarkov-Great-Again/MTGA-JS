...
const {
    ...
    writeFile, getCurrentTimestamp, getRandomFromArray, getFileUpdatedDate
} = require("../utilities");
...
class Profile extends BaseModel {
    ...
    */
    static async ifAvailableNickname(nickname) {
        ...
        for (const profile of profiles) {
            ...
            if (Object.keys(profile.character).length === 0) {
                await logger.debug(`[PROFILE] Character for ${profile.id} has not been created.`);
                break;
            }
    ...
    async save() {
        ...
        //.then((results) => results.forEach((result) => await logger.info(`${result.status}, ${result.reason}`)));
                ...
                if (statsPreSave.mtimeMs === fileAge[this.id].character) {
                    ...
                    if (currentProfile !== savedProfile) {
                       ...
                       await writeFile(characterPath, currentProfile);
                        //await logger.success(`[PROFILE] Character for AID ${this.id} was saved.`);
                    } else {
                        await logger.warn(`[PROFILE] Skipped saving character for AID ${this.id}.`);
                        // Skip save ?
                    }
                } else {
                    await logger.warn(`[PROFILE] Character for AID ${this.id} requires reload.`);
                    // Recreate reload
                }
            } else {
                //await logger.success(`[PROFILE] Character for AID ${this.id} was saved in a newly created file.`);
                // Save the PMC character from memory to disk.
                await writeFile(characterPath, currentProfile);
            }
            // Update the savedFileAge stored in memory for the character.json.
            const statsAfterSave =await getFileUpdatedDate(characterPath);
            fileAge[this.id].character = statsAfterSave.mtimeMs;
        } else {
            await logger.error(`[PROFILE] Character for AID ${this.id} does not exist.`);
        }
    ...
    async saveStorage() {
        ...
        if (this.storage) {
            ...
            if (await fileExist(storagePath)) {
                ...
                if (statsPreSave.mtimeMs === fileAge[this.id].storage) {
                    ...
                    if (currentStorage !== savedProfile) {
                        ...
                        await logger.success(`[PROFILE] Storage for AID ${this.id} was saved.`);
                    } else {
                        // Skip save ?
                    }
                } else {
            ...
            let statsAfterSave =await getFileUpdatedDate(storagePath);
        ...
        if (await fileExist(dialoguePath)) {
            ...
            if (statsPreSave.mtimeMs === fileAge[this.id].dialogues) {
                ...
                if (currentDialogues !== savedDialogues) {
                    ...
                    const stats =await getFileUpdatedDate(dialoguePath);
                    fileAge[this.id].dialogues = stats.mtimeMs;
                    await logger.success(`[PROFILE] Dialogues for AID ${this.id} was saved.`);
                }
            } else {
           ...
           await writeFile(dialoguePath, currentDialogues);
            ...
            const stats =await getFileUpdatedDate(dialoguePath);
            fileAge[this.id].dialogues = stats.mtimeMs;
            await logger.success(`[PROFILE] Dialogues for AID ${this.id} was created and saved.`);
        }
    }
            ...
            if (statsPreSave.mtimeMs === fileAge[this.id].special) {
                ...
                if (currentSpecialData !== savedSpecialData) {
                   ...
                   await writeFile(specialDataPath, currentSpecialData);
                    ...
                    const stats =await getFileUpdatedDate(specialDataPath);
    ...
    async getLoyalty(traderID) {
        ...
        let playerSaleSum = pmcData.TradersInfo[traderID] ? pmcData.TradersInfo[traderID].salesSum : 0;
        let playerStanding = pmcData.TradersInfo[traderID] ? pmcData.TradersInfo[traderID].standing : 0;
        let playerLevel = pmcData.Info.Level;
