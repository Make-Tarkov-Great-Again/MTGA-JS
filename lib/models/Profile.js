...
const {
    ...
    writeFile, generateMongoID, getCurrentTimestamp, getRandomFromArray
} = require("../../utilities");
...
class Profile extends BaseModel {
    ...
    async tick() {
        ...
        let hideoutTick = await pmc.hideoutTick(specialData);
        if (hideoutTick) {
            changesMade = true;
        }
    ...
    */
    ...
    async createDialogue(traderID) {
        let dialogue;
        if (traderID in this.dialogues) {
            dialogue = this.dialogues[traderID];
        } else {
            dialogue = {
                _id: traderID,
                messages: [],
            ...
            this.dialogues[traderID] = dialogue;
        }
        return dialogue;
    }

    async createMessageContents(traderID, message, rewards, stashItems) {
        const output = {
     ...
     */
    async generateDialogue(traderID, contents, sessionID, rewards = null) {

        let dialogue = await this.getDialogue(traderID);
        if (!dialogue) dialogue = await this.createDialogue(traderID)
        ...
        if (rewards !== null && rewards.length > 0) {
            ...
            for (const reward of rewards) {
                ...
                if (!("slotId" in reward) || reward.slotId === "hideout") {
                    reward.parentId = stashItems.stash;
                    reward.slotId = "main"
                }
        ...
        const message = await this.createMessageContents(traderID, contents, rewards, stashItems);
        ...
        if (check) {
            ...
            for (const notification of notifications) {
                if (time >= notification.message.dt) {
                    await notificationHandler.sendNotification(notification, this.id);
                }
            }
        }
    }
    ...
    async processInsuranceReturn() {
        ...
        if (check) {
            ...
            for (const insured of insurance) {
                ...
                const dialogue = await this.generateDialogue(insured.traderId, insured.messageContent, this.id, insured.items);
                await this.removeInsuranceNotification();
                await this.sendNotificationMessage(dialogue, this.id);
