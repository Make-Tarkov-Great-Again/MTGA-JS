    ...
    */
    ...
    async saveCharacter() {
        ...
        if (this.character) {
            ...
            const statsAfterSave = fs.statSync(characterPath);
    ...
    async saveStorage() {
        ...
        if (this.storage) {
            ...
            let statsAfterSave = fs.statSync(storagePath);
    ...
    async saveDialogue() {
        ...
        if (fileExist(dialoguePath)) {
            ...
            if (statsPreSave.mtimeMs === fileAge[this.id].dialogues) {
                ...
                const savedDialogues = stringify(readParsed(dialoguePath));
                if (currentDialogues !== savedDialogues) {
                    // Save the dialogues stored in memory to disk.
                    writeFile(dialoguePath, currentDialogues);
    ...
    async saveSpecial() {
        ...
        if (fileExist(specialDataPath)) {
            ...
            if (statsPreSave.mtimeMs === fileAge[this.id].special) {
                ...
                const savedSpecialData = stringify(readParsed(specialDataPath));
                if (currentSpecialData !== savedSpecialData) {
                    // Save the special stored in memory to disk.
                    writeFile(specialDataPath, currentSpecialData);
    ...
    async processInsuranceReturn() {
        ...
        if (check) {
            ...
            for (const insured of insurance) {
                ...
                const generatedDialogue = await dialogue.generateDialogue(insured.traderId, insured.messageContent, this.id, insured.items);
