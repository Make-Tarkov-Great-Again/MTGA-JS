const { generateItemId, getRandomInt } = require("../../utilities");
...
class HideoutScavcase extends BaseModel {
    ...
    async generateRewards() {
        ...
        for (const rarity in availableRarity) {
            ...
            const amountProducts = await getRandomInt(availableRarity[rarity].min, availableRarity[rarity].max);
            const items = await Item.getAllWithoutKeys();
            for (const item of items) {
                if (await item.getRarityByPrice() === rarity){
                    possibleRewards.push(item);
                }
            }
            for (let i = 0; i < amountProducts; i++) {
                const randomIndex = await getRandomInt(0, possibleRewards.length - 1);
                rewards.push({
