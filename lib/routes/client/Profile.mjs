...
export default async function profileRoutes(app, _opts) {
    ...
    app.post(`/client/game/profile/items/moving`, async (request, reply) => {
        await RichPresenseController.onStash(); //i hate /items/moving, but you can tell basically anything you're doing on this
        await MoveController.clientGameProfileItemsMoving(request, reply); //from what's in this controller
    });
