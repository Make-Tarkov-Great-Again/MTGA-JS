const { ClientController, MenuController, HideoutController } = require("../../controllers");
...
module.exports = async function clientRoutes(app, _opts) {
    ...
    app.post(`/client/hideout/qte/list`, async (_request, reply) => {
        const QTE = await HideoutController.clientHideoutQTEList();
        return Response.zlibJsonReply(
            reply,
            await Response.applyBody(QTE)
        );
    });
