...
const { Response, logger, getCurrentTimestamp, readParsed, stringify, writeFile } = require("../../utilities");
...
module.exports = async function clientRoutes(app, _opts) {
    ...
    app.post(`/client/items/prices/:trader`, async (request, reply) => {
        const { database: {
            core: { resupply },
            templates: { priceTable } } } = require(`../../../app`);
        ...
        if (resupply[trader.base._id]) {
            if (resupply[trader.base._id] <= currentTime) {
                await trader.refreshTrader();
                resupplyTime = (currentTime + refreshTimeInMinutes * 60);
                resupply[trader.base._id] = resupplyTime;

                const original = stringify(await readParsed(resupplyPath));
                const current = stringify(resupply);
                if (original !== current) {
                    await writeFile(resupplyPath, current);
                }
            }
            else {
                resupplyTime = resupply[trader.base._id];
            }
        }
        else {
            resupplyTime = (currentTime + refreshTimeInMinutes * 60);
            resupply[trader.base._id] = resupplyTime;
            trader.assort.nextResupply = resupplyTime;
