...
export default async function mailRoutes(app, _opts) {
    ...
    app.post(`/client/mail/dialog/list`, async (request, reply) => {
        await ClientController.clientMailDialogList(Response.getSessionID(request), reply);
    });
    ...
    app.post(`/client/mail/msg/send`, async (request, reply) => {
        if (request.body.dialogId !== Response.getSessionID(request))
            await FriendController.clientMailMessageSend(request, reply)
        else {
