...
import { zlibJsonReply, getSessionID, applyEmpty, logger } from "../../utilities/_index.mjs";
...
export default async function friendRoutes(app, _opts) {
    ...
    app.post(`/client/friend/list`, async (request, reply) => {
        await FriendController.clientFriendRequestList(await getSessionID(request), reply);
    });
    ...
    app.post(`/client/friend/request/list/inbox`, async (request, reply) => {
        await FriendController.clientFriendRequestListInbox(await getSessionID(request), reply);
    });

    app.post(`/client/friend/request/list/outbox`, async (request, reply) => {
        await FriendController.clientFriendRequestListOutbox(await getSessionID(request), reply);
    });
