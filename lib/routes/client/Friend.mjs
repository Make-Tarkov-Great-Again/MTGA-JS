...
export default async function friendRoutes(app, _opts) {
    ...
    app.post(`/client/friend/list`, async (request, reply) => {
        await FriendController.clientFriendRequestList(Response.getSessionID(request), reply);
    });
    ...
    app.post(`/client/friend/request/list/inbox`, async (request, reply) => {
        await FriendController.clientFriendRequestListInbox(Response.getSessionID(request), reply);
    });

    app.post(`/client/friend/request/list/outbox`, async (request, reply) => {
        await FriendController.clientFriendRequestListOutbox(Response.getSessionID(request), reply);
    });
