...
export default async function taurilauncherRoutes(app, _opts) {
    ...
    // }
    app.post('/tauri/get/config', async (request, reply) => {
        return LauncherControllerinator.getConfig(request, reply)
    }) // 

    app.post('/tauri/update/config', async (request, reply) => {
        return LauncherControllerinator.updateConfig(request, reply)
    }) // 
