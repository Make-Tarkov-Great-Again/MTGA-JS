const fs = require('fs');
const { Ragfair, Preset, Item } = require('../models');
    ...
    */
    ...
    static async loadLocales() {
        ...
        for (const locale in localeKeys) {
            ...
            const currentLocalePath = `${databaseLocalesPath}${localeIdentifier}/`;
    ...
    static async loadTraders() {
        ...
        for (const traderID of traderKeys) {
            const path = `${databaseTraderPath}${traderID}/`;
    ...
    static async loadAccounts() {
        if (!fileExist("/user/profiles/")) {
            createDirectory(getAbsolutePathFrom("/user/profiles/"));
        }

        for (const profileID of getDirectoriesFrom(getAbsolutePathFrom('/user/profiles/'), false)) {
            if (fileExist(getAbsolutePathFrom("/user/profiles/" + profileID + "/account.json"))) {
