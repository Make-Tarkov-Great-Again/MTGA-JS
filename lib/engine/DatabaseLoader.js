...
const {
    ...
    read, parse, round } = require('../../utilities');
...
class DatabaseLoader {
    ...
    static async generateTemplates() {
        ...
        if (typeof template.data != "undefined") { template = template.data; }
        database.templates = {
            Categories: [],
            Items: [],
            priceTable: [],
        ...
        if (database.templates.Categories) database.templates.Categories = template.Categories;
        if (database.templates.Items) database.templates.Items = template.Items;
        if (database.templates.priceTable) database.templates.priceTable = Item.generatePriceTable(template.Items);
        if (database.templates.TplLookup) database.templates.TplLookup = await DatabaseUtils.generateTplLookup(template.Items, template.Categories);
    }
    ...
    static async generateProfiles() {
        for (const profileID of getDirectoriesFrom(getAbsolutePathFrom('/user/profiles'), false)) {
            ...
            //tasker.addTask(profile.id + "_tick", profile.tick)
        }
    }
...
class DatabaseUtils {
    ...
    /* These need to be moved into their own script
        ...
        static async formatAndWriteNewLocationDataToDisk() {
            logger.warn("Loading new locations into proper format...");
            ...
            if (files.length > 0) {
                for (let file of files) {
                    ...
                    if (fileExist(`${locationPath}/${filename}.json`)) {
                        ...
                        logger.debug(`${directoryName}${filename}`);
