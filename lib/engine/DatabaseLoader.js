...
const {
    ...
    getFilesFrom, generateMongoID, getAbsolutePathFrom,
    getFileUpdatedDate,
    generateUniqueId,
    read } = require('../../utilities');
...
class DatabaseLoader {
    ...
    static async loadDatabase() {
        ...
        await this.loadBots();
        await this.loadOrderedItemsPerCategories();
    }
    ...
    static async loadRagfair() {
        ...
        for (const trader of traders) {
            if (!await trader.isFence() && !await trader.isRagfair()) {
                ragfair.offers = ragfair.offers.concat(await trader.getRagfairOffers());
                Object.assign(categories, trader.assort.loyal_level_items);
            }
        }
        ragfair.offersCount = ragfair.offers.length;
        ragfair.categories = await Ragfair.formatCategories(categories, ragfair.offers);
    ...
    /**
     * Create a database view of items ordered by categories (ex: {"Armor": [...], "Weapon": [...]})
     */
    static async loadOrderedItemsPerCategories() {
        const itemsNodes = await Item.getAllItemsNodes();
        const formatedNodes = {};
        database.nodes = {};
        database.nodes.orderedItemsPerCategories = await Item.generateFormatedNodes(itemsNodes, formatedNodes);
        console.log()
    }
}
    ...
    static async formatAndWriteNewBotDifficultyDataToDisk() {
        ...
        if (files.length > 0) {
            for (let file of files) {
                ...
                const input = read(path).toString();
                const output = input.replace(/(?<=(true|false|null|["\d}\]])\s*),(?=\s*[}\]])/g, '');
                const difficultyData = JSON.parse(output);
