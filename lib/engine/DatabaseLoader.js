    ...
    static async generateTraders() {
        ...
        for (const traderID of traderDirectory) {
            ...
            trader.suits = await fileExist(`${path}suits.json`)
                ? await readParsed(`${path}suits.json`)
                : []

            trader.dialogue = await fileExist(`${path}dialogue.json`)
                ? await readParsed(`${path}dialogue.json`)
                : [];
        ...
        for (const profileID of accountDirectory) {
            ...
            if (await fileExist(`${profile}account.json`)) {
                ...
                if (Object.keys(account).length === 0) {
                    ...
                    await logger.success(`Loaded account data for profile: ${profileID}`);

                const stats =await getFileUpdatedDate(path);
        ...
        for (const profileID of profileDirectory) {
            ...
            if (await fileExist(`${path}character.json`)) {
                ...
                if (Object.keys(profile.character).length === 0) {
                    ...
                    await logger.success(`Loaded character data for profile: ${profileID}`);


                stats =await getFileUpdatedDate(`${path}character.json`);
            ...
            if (await fileExist(`${path}storage.json`)) {
                ...
                if (!profile.storage._id) {
                    ...
                    await logger.success(`Loaded storage data for profile: ${profileID}`);

                stats =await getFileUpdatedDate(`${path}storage.json`);
            ...
            if (await fileExist(`${path}dialogue.json`)) {
                ...
                if (!profile.dialogues) {
                    ...
                    await logger.success(`Loaded dialogue data for profile: ${profileID}`);

                stats =await getFileUpdatedDate(`${path}dialogue.json`);
            ...
            if (await fileExist(`${path}special.json`)) {
                ...
                if (!profile.special?.lastCyclicUpdate) {
                    ...
                    await logger.success(`Loaded special data for profile: ${profileID}`);

                stats =await getFileUpdatedDate(`${path}special.json`);
