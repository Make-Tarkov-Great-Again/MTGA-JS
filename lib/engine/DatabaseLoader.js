...
class DatabaseLoader {
    ...
    static async generateLocations() {
        ...
        for (const map of mapDirectory) {
            ...
            for(let fileName of await getFilesFrom(waves_path))
            {
                ...
                await logger.info(`Additional waves from: ${fileName}`);
            }
            ...
            for(let fileName of await getFilesFrom(bossWaves_path))
            {
                ...
                await logger.info(`Additional boss waves from: ${fileName}`);
            }
    ...
    static async generateTraders() {
        ...
        if (!fileExist(`./assets/database/configs/resupply.json`)) {
            database.core.resupply = {};
            await writeFile(`./assets/database/configs/resupply.json`, database.core.resupply);
        } else
