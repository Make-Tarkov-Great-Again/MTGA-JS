...
class DatabaseLoader {
    static async setDatabase() {
        ...
        if (database.core.gameplay.bots.preload) {
            const bots = await Bot.initializeBotPreload();
            database.bot.preload = bots;
            if (database.core.gameplay.development.debugBots) {
                const preload = stringify(bots);
                writeFile("/bots.json", preload)
            };
        }
    }
    ...
    static async generateLocations() {
        ...
        for (const map of maps) {
            ...
            for (const [index, variant] of Object.entries(variants)) {
                ...
                if (database.core.gameplay.location.changeExfiltrationTime && pathData?.exits.length > 0) {
                    for (const exit of pathData.exits) {
                        exit.ExfiltrationTime = database.core.gameplay.location.exfiltrationTime >= 0 ? database.core.gameplay.location.exfiltrationTime : 0;
                    }
                }
                // programatically add missing parameters
    ...
    static async generateItems() {
        ...
        for (const [index, item] of Object.entries(items)) {
            ...
            if (gameplay.items.noArmorRestrictions) {
                if (item._props.BlocksArmorVest) {
                    item._props.BlocksArmorVest = false;
                }
            }

            if (gameplay.items.Weight.enabled) {
