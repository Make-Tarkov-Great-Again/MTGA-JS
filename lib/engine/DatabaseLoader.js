...
class DatabaseLoader {
    ...
    static async generateCore() {
        database.core = {
            serverConfig: readParsed(getAbsolutePathFrom(`./assets/database/configs/server.json`)),
            matchMetrics: readParsed(getAbsolutePathFrom(`./assets/database/configs/matchMetrics.json`)),
            botTemplate: readParsed(getAbsolutePathFrom(`./assets/database/configs/schema/botTemplate.json`)),
            clientSettings: readParsed(getAbsolutePathFrom(`./assets/database/configs/client.settings.json`)).data,
            gameplay: readParsed(getAbsolutePathFrom(`./assets/database/configs/gameplay.json`)),
            globals: readParsed(getAbsolutePathFrom(`./assets/database/configs/globals.json`)).data,
            locations: readParsed(getAbsolutePathFrom(`./assets/database/configs/locations.json`)),
            hideoutSettings: readParsed(getAbsolutePathFrom(`./assets/database/hideout/settings.json`)).data,
            blacklist: readParsed(getAbsolutePathFrom(`./assets/database/configs/blacklist.json`)),
            metrics: readParsed(getAbsolutePathFrom(`./assets/database/configs/matchMetrics.json`)),
            connections: {
                webSocket: {},
                webSocketPings: {}
            },
        };
        database.core.mongoIds = new MongoId(syncGetRandomIntInc(1, 16777215)) // set id factory at core start
        database.core.globals = await this.adjustGlobals(database.core.globals, database.core.gameplay);
    }
    ...
    static async generateBots() {
        ...
        database.bot = {
            names: readParsed(`./assets/database/bot/names.json`),
            core: readParsed(`./assets/database/bot/__BotGlobalSettings.json`),
            appearance: readParsed("./assets/database/bot/appearance.json"),
            playerScav: readParsed("./assets/database/bot/playerScav.json")
        };

        database.bot.bots = {};
        const botType = getDirectoriesFrom(`./assets/database/bot/bots`);
        for (const bot of botType) {
            database.bot.bots[bot] = {
                difficulty: {}
            };

            if (fileExist(`./assets/database/bot/bots/${bot}/health.json`)) {
                database.bot.bots[bot].health = {};
                const health = readParsed(`./assets/database/bot/bots/${bot}/health.json`);
            ...
            if (fileExist(`./assets/database/bot/bots/${bot}/loadout.json`)) {
                database.bot.bots[bot].authorizedGear = readParsed(`./assets/database/bot/bots/${bot}/loadout.json`);
                database.bot.bots[bot].weightedGear = await DatabaseUtils.generateWeightedList(database.bot.bots[bot].authorizedGear);
            }

            const botDifficulty = getFilesFrom(`./assets/database/bot/bots/${bot}/difficulties`);
            for (const difficulty of botDifficulty) {
                database.bot.bots[bot].difficulty[difficulty.replace(".json", "")] = readParsed(`./assets/database/bot/bots/${bot}/difficulties/${difficulty}`);
            }
        }
    }
    ...
    static async generateHideout() {
        ...
        let hideoutAreas = readParsed(getAbsolutePathFrom('./assets/database/hideout/areas.json'));
        ...
        let hideoutProductions = readParsed(getAbsolutePathFrom('./assets/database/hideout/productions.json'));
        ...
        let hideoutScavcase = readParsed(getAbsolutePathFrom('./assets/database/hideout/scavcase.json'));
    ...
    static async generateWeather() {
        database.weather = readParsed(getAbsolutePathFrom('./assets/database/weather.json'))
        if (database.weather.data) { database.weather = database.weather.data; }
    }

    static async generateTemplates() {
        let template = readParsed(getAbsolutePathFrom('./assets/database/templates.json'));
    ...
    static async generateLocations() {
        ...
        const maps = getDirectoriesFrom('./assets/database/locations');
        for (const map of maps) {
            const location = await UtilityModel.createModelFromParseWithID('Location', map, {});
            const variants = getFilesFrom(getAbsolutePathFrom(`./assets/database/locations/${map}`), false);
            for (const [index, variant] of Object.entries(variants)) {
                let pathData = readParsed(getAbsolutePathFrom(`./assets/database/locations/${map}/${variant}`));
    ...
    static async generateCustomization() {
        ...
        let customizations = readParsed(getAbsolutePathFrom("./assets/database/customization.json"));
    ...
    static async generateQuests() {
        let quests = readParsed(getAbsolutePathFrom("./assets/database/quests.json"));
    ...
    static async generateEditions() {
        const editionKeys = getDirectoriesFrom('./assets/database/editions/');
        for (const editionType of editionKeys) {
            const path = getAbsolutePathFrom(`./assets/database/editions/${editionType}/`);
    ...
    static async generateItems() {
        ...
        let itemdata = readParsed(getAbsolutePathFrom('./assets/database/items.json'));
    ...
    static async generateLanguages() {
        let languages = readParsed(getAbsolutePathFrom(`./assets/database/locales/languages.json`));
    ...
    static async generateLocales() {
        const locales = getDirectoriesFrom(`./assets/database/locales/`);
        this.locales = {};
        for (const key in locales) {
            const localeIdentifier = locales[key];
            const currentLocalePath = `./assets/database/locales/${localeIdentifier}/`;
    ...
    static async generateTraders() {
        const traderKeys = getDirectoriesFrom('./assets/database/traders/');
        for (const traderID of traderKeys) {
            ...
            trader.suits = fileExist(`${path}suits.json`)
                ? readParsed(getAbsolutePathFrom(`${path}suits.json`))
                : []

            trader.dialogue = fileExist(`${path}dialogue.json`)
                ? readParsed(getAbsolutePathFrom(`${path}dialogue.json`))
                : [];
...
class DatabaseUtils {
    ...
    static async addVoicesToLocale(locales) {
        const extraVoices = readParsed(`./assets/database/locales/extras.json`)
    ...
    static async addHeadsToLocale(locales) {
        const extraHeads = readParsed(`./assets/database/locales/extras.json`);
    ...
    static async generateCategorizedCustomization() {
        ...
        const appearance = readParsed(getAbsolutePathFrom("./assets/database/bot/appearance.json"));
        ...
        writeFile("./assets/database/bot/appearance.json", stringify(appearance));
    }
    ...
    static async generateWeigthedWeaponItemLists() {
        let scavWeaponsFileExists = !!fileExist("./assets/database/bot/scav_weapons.json");
        let scavAttachmentsFileExists = !!fileExist("./assets/database/bot/scav_attachments.json");
        let raiderWeaponsFileExists = !!fileExist("./assets/database/bot/raider_weapons.json");
        let raiderAttachmentsFileExists = !!fileExist("./assets/database/bot/raider_attachments.json");
        ...
        if (
            ...
            if (!scavWeaponsFileExists) {
                writeFile("./assets/database/bot/scav_weapons.json", stringify(scavWeapons));
            }

            if (!scavAttachmentsFileExists) {
                writeFile("./assets/database/bot/scav_attachments.json", stringify(scavAttachements));
            }

            if (!raiderWeaponsFileExists) {
                writeFile("./assets/database/bot/raider_weapons.json", stringify(raiderWeapons));
            }

            if (!raiderAttachmentsFileExists) {
                writeFile("./assets/database/bot/raider_attachments.json", stringify(raiderAttachments));
            }
        }
    }
