    ...
    static async generateProfiles() {
        for (const profileID of getDirectoriesFrom('/user/profiles')) {
            ...
            if (fileExist(`user/profiles/${profileID}/special.json`)) {
                logger.logWarning(`Loading special data for profile: ${profileID}`);

                let parsedSpecialData = readParsed(`${path}special.json`);
                if (typeof parsedSpecialData.data != "undefined") { parsedSpecialData = parsedSpecialData.data; }
            ...
            tasker.addTask(profile.id + "_profile_tick", profile.tick, profile, 10000)
        }
    }
