...
class Database {
    ...
    async setCoreLocations() {
        ...
        this.core.map = {
            locations: map?.data ? map.data.locations : map.locations,
            paths: map?.data ? map.data.paths : map.paths,
            legend: {
                index: {},
                locations: {}
            }
        ...
        for (const id in this.core.map.locations) {
            const locationData = this.core.map.legend.locations[id] = {};
            const location = this.core.map.locations[id];

            this.core.map.legend.index[id] = location.Id;
            locationData.OpenZones = location.OpenZones.split(",");
            locationData.Infiltration = [];

            for (const point in location.SpawnPointParams) {
                const spawnPoint = location.SpawnPointParams[point];
                if (spawnPoint.Infiltration.length === 0)
                    continue;

                locationData.Infiltration.push(spawnPoint.Infiltration);
            }
        }
    }
