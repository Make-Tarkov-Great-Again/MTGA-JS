...
class Database {
    ...
    async initialize() {
        ...
        await Promise.all([
            DatabaseUtils.adjustGlobals(this.core.globals, this.core.gameplay),
            DatabaseUtils.adjustItems(this.core.gameplay, this.items),
            DatabaseUtils.adjustLocales(this.core.gameplay, this.locales),
            DatabaseUtils.adjustHideout(this.core.gameplay, this.hideout),
            DatabaseUtils.adjustLocations(this.core.gameplay, this.core.map.locations),
            DatabaseUtils.adjustCustomizations(this.core.gameplay, this.customization),
            DatabaseUtils.adjustProfiles(this.profiles),
        ]);
    }
    ...
    async setCore() {
        ...
        if (!await fileExist(`./assets/database/configs/resupply.json`)) {
            this.core.resupply = {};
            await write(`./assets/database/configs/resupply.json`, stringify(this.core.resupply));
        } else
    ...
    async getTplLookup(items, categories) {
        ...
        for (const item of items.reverse()) {
            const { Id, Price, ParentId } = item;
            lookup.items.byId[Id] = Price;
            lookup.items.byParent[ParentId] ??= [];
            lookup.items.byParent[ParentId].push(Id);
        }

        for (const category of categories.reverse()) {
            const { Id, ParentId } = category;
            lookup.categories.byId[Id] = ParentId ?? null;
            if (ParentId) {
                lookup.categories.byParent[ParentId] ??= [];
                lookup.categories.byParent[ParentId].push(Id);
            }
        }
    ...
    async setBot() {
        ...
        for (let i = 0, dirLength = botDirectory.length; i < dirLength; i++) {
            const botType = botDirectory[i];
            const bot = this.bot.bots[botType] = {};
            const path = `./assets/database/bot/bots/${botType}`;
            ...
            if (await fileExist(`${path}/health.json`)) {
                ...
                if (Object.keys(health).length > 0) {
                    for (const [index, value] of Object.entries(health)) {
                        bot.health[index] = value;
                    }
                } else bot.health = health;
            }

            if (await fileExist(`${path}/loadout.json`)) {
                bot.loadout = await readParsed(`${path}/loadout.json`);
            }
            ...
            for (let d = 0, difLength = botDifficulty.length; d < difLength; d++) {
                ...
                bot.difficulty[difficulty.replace(".json", "")] = await readParsed(`${path}/difficulties/${difficulty}`);
            }
        }
    }
    ...
    async setTemplatesPriceTable() {
        ...
        for (const item of items) {
            const price = fleaPrices && liveFlea[item.Id] ? liveFlea[item.Id] : item.Price;
            this.templates.priceTable[item.Id] = price;
        }
    }
    ...
    async setTraders() {
        ...
        for (let i = 0, length = traderDirectory.length; i < length; i++) {
            ...
            if (await fileExist(`${traderPath}assort.json`)) {
                let assort = await readParsed(`${traderPath}assort.json`);
                traderData.baseAssort = assort?.data ? assort.data : assort;
            }
    ...
    async setLocations() {
        ...
        for (let i = 0, length = mapDirectory.length; i < length; i++) {
            ...
            await Promise.allSettled([
                this.setLocationLootSpawns(path, location),
                this.setLocationWaves(path, location),
                this.setLocationBossWaves(path, location),
                this.setLocationPresets(path, location),
            ])
        }
    ...
    async setProfiles() {
        ...
        for (let i = 0, length = profileDirectory.length; i < length; i++) {
            ...
            await Promise.all([
                this.setCharacter(profilePath, profileID),
                this.setAccount(profilePath, profileID),
                this.setStorage(profilePath, profileID),
                this.setDialogues(profilePath, profileID),
                this.setSpecial(profilePath, profileID),
            ]);
        }
    }
...
class DatabaseUtils {
    ...
    static async adjustItems(gameplay, items) {
        ...
        for (let i = 0, length = values.length; i < length; i++) {
            ...
            if (shotguns.includes(item._id) && item?._props?.ShotgunDispersion) {
                item._props.shotgunDispersion = item._props.ShotgunDispersion;
            }
    ...
    static async adjustLocales(gameplay, locales) {
        ...
        const changes = {
            "Prepare for Escape": "MAKE TARKOV GREAT AGAIN",
            "Prepare to Escape": "MAKE TARKOV GREAT AGAIN",
            "Game settings": "Raid settings",
            "Co-op mode settings": "Raid settings",
            "ECoopBlock/GameVersionUnavailable": "Disabled until someone makes fucking co-op!!!!!",
            "Enable OFFLINE mode for this raid": "Auto-Enabled For Dummies",
            "Offline raid description": "Below are the Raid/Game Settings that can be adjusted to your preferences.",
            "Offline raid test mode": "Raid Settings Summary",
        };
    ...
    static async adjustHideout(gameplay, hideout) {
        const { fastProduction, fastScavcase } = gameplay.hideout;

        if (!fastProduction && !fastScavcase) {
            return;
        }

        const productions = Object.values(hideout.productions);
        const scavcases = Object.values(hideout.scavcase);
    ...
    static async adjustLocations(gameplay, locations) {
        const { changeRaidTime, raidTimerMultiplier, changeExfiltrationTime, exfiltrationTime } = gameplay.location;

        if (!changeRaidTime && !changeExfiltrationTime) {
            return;
        }
        ...
        for (const id in locations) {
            ...
            if (changeRaidTime && raidTimerMultiplier)
                location.EscapeTimeLimit *= raidTimerMultiplier;

            if (changeExfiltrationTime && location.exits.length !== 0) {
                for (const exit of location.exits) {
                    exit.ExfiltrationTime = exfiltrationTime;
                }
            }
        }
    }
