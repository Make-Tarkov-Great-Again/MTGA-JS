...
class Account {
    ...
    reloadAccountByLogin = async (info) => {
        ...
        for (const accountID in this.accounts) {
            ...
            if (info.email === account.email && info.password === account.password) {
                ...
                if (stats.mtimeMs != this.accountFileAge[accountID]) {
                    logWarning(`[CLUSTER] Account file for account ${accountID} was modified, reloading.`);
        ...
        for (const id in profileIDs) {
            if (!fileExist(`/user/profiles/${profileIDs[id]}/account.json`)) {
                logWarning(`[CLUSTER] Account file for account ${profileIDs[id]} does not exist.`);
            } else {
                ...
                if (info.email === account.email && info.password === account.password) {
                    ...
                    logSuccess(`[CLUSTER] User ${account.email} with ID ${profileIDs[id]} logged in successfully.`);
    ...
    */
    reloadAccountBySessionID = async (sessionID) => {
        if (!fileExist(`./user/profiles/${sessionID}/account.json`)) {
            logWarning(`[CLUSTER] Account file for account ${sessionID} does not exist.`);
        } else {
            // Does the session exist?
            if (!this.accounts[sessionID]) {
                logWarning(`[CLUSTER] Tried to load session ${sessionID} but it wasn't cached, loading.`);
                // Reload the account from disk.
                ...
                if (stats.mtimeMs != this.accountFileAge[sessionID]) {
                    logWarning(`[CLUSTER] Account file for account ${sessionID} was modified, reloading.`);
    ...
    saveToDisk = async (sessionID = 0) => {
        ...
        if (sessionID == 0) {
            ...
            for (const id in this.accounts) {
                ...
                if (stats.mtimeMs == this.accountFileAge[id]) {
                    ...
                    if (stringify(currentAccount) !== stringify(savedAccount)) {
                        ...
                        logSuccess(`[CLUSTER] Account file for account ${id} was saved to disk.`);
                    }
                } else {
                    logWarning(`[CLUSTER] Account file for account ${id} was modified, reloading.`);
            ...
            if (!fileExist(`/user/profiles/${sessionID}/account.json`)) {
                ...
                logSuccess(`[CLUSTER] New account ${sessionID} registered and was saved to disk.`);
            } else {
                // Check if the file was modified by another cluster member using the file age.
                const stats = fs.statSync(`./user/profiles/${sessionID}/account.json`);
                if (stats.mtimeMs == this.accountFileAge[sessionID]) {
                    ...
                    if (stringify(currentAccount) !== stringify(savedAccount)) {
                        ...
                        logSuccess(`[CLUSTER] Account file for account ${sessionID} was saved to disk.`);
                    }
                } else {
                    logWarning(`[CLUSTER] Account file for account ${sessionID} was modified, reloading.`);
        ...
        for (const account in accounts) {
            if (account == sessionID) {
                if (!fileExist("/user/profiles/" + sessionID + "/character.json")) {
                    logSuccess(`[CLUSTER] New account ${sessionID} logged in!`);
                }
                return true
            }
        }
    ...
    */
    ...
    find = async (sessionID) => {
        logInfo(`[CLUSTER] Account ${sessionID} requested.`);
        ...
        for (const accountID in this.accounts) {
            ...
            if (account.id === sessionID) {
                logSuccess(`[CLUSTER] Account ${sessionID} found.`);
                return account;
            }
        }
        return undefined;
    ...
    changeEmail = async (info) => {
        logWarning(`[CLUSTER] Changing email for account ${info.id}`);
        const accountID = this.reloadAccountBySessionID(info);

        if (accountID !== "") {
            this.accounts[accountID].email = info.change;
            this.saveToDisk(accountID);
            logSuccess(`[CLUSTER] Changed email for account ${accountID}`);
        }
    ...
    changePassword = async (info) => {
        logWarning(`[CLUSTER] Changing password for account ${info.id}`);
        const accountID = this.reloadAccountBySessionID(info);

        if (accountID !== "") {
            this.accounts[accountID].password = info.change;
            this.saveToDisk(accountID);
            logSuccess(`[CLUSTER] Password for account ${accountID} changed.`);
        }
     ...
     */
    wipe = async (info) => {
        logWarning(`[CLUSTER] Wiping account ${info.id}`);
        const accountID = this.reloadAccountBySessionID(info);
